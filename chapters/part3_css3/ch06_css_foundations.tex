...
/* Cu phap RGBA bổ sung kenh Alpha (4 tham so) */
color: rgba(R, G, B, A);
\end{lstlisting}
\end{codeWindow}
...
\begin{tikzpicture}[scale=0.92, transform shape]
  % 1. Thẻ .basic-text: Nền xanh lá đặc, chữ đỏ đặc
  \node[
    draw=none,
    fill=green!70!black,
    text=red!90!black,
    ...
    .basic-text: Chữ màu Đỏ \codeinline{rgb(255,0,0)} trên nền Xanh lá \codeinline{rgb(0,255,0)}
  };
  
  % 2. Lớp văn bản phía dưới (Background Content)
  \node[
    text=gray!60,
    ...
    Dòng văn bản phía dưới --- Lớp giao diện nền trang web nằm phía đằng sau...
  };
  
  % 3. Thẻ .translucent-box: Nền xanh dương mờ 50% phủ lên lớp văn bản phía dưới
  \node[
    draw=none,
    fill=blue!90!black,
    fill opacity=0.45,
    ...
    .translucent-box: Nền Xanh dương mờ 50\% \codeinline{rgba(0,0,255,0.5)}\\
    \footnotesize\normalfont(Vẫn nhìn xuyên qua được nội dung văn bản nền phía sau)
  };
\end{tikzpicture}
\end{center}
\end{browserWindow}
...
\begin{center}
\begin{tikzpicture}[scale=0.92, transform shape]
  % 1. Khung nền toàn bộ trang web (Background Web Canvas)
  \draw[draw=gray!30, fill=primaryBlue!5, rounded corners=6pt, line width=0.8pt] (-6.2, -1.8) rectangle (6.2, 1.8);
  
  % Giao diện trang web đằng sau (Navbar & Text)
  \fill[primaryBlue!20, rounded corners=3pt] (-5.8, 1.1) rectangle (5.8, 1.5);
  \node[font=\sffamily\bfseries\color{primaryBlue!80!black}] at (0, 1.3) {Giao Diện Trang Web Phía Dưới (Navbar \& Content)};
  \node[font=\sffamily\color{gray!70}, text width=10.5cm, align=center] at (0, 0.2) {
    Nội dung danh sách bài viết, banner quảng cáo và nút bấm phía dưới trang web...
  };

  % 2. Lớp phủ RGBA Overlay (0, 0, 0, 0.6) che phủ toàn bộ trang web
  \fill[black, fill opacity=0.6, rounded corners=6pt] (-6.2, -1.8) rectangle (6.2, 1.8);
  
  % 3. Cửa sổ Modal nổi bật nằm ở trung tâm trên cùng
  \node[
    draw=white!40,
    fill=white,
    text=darkText,
    ...
    {\color{primaryBlue}\bfseries\large Cửa Sổ Modal / Dialog Popup}\\[5pt]
    {\small Lớp phủ xung quanh: \codeinline{background-color: rgba(0, 0, 0, 0.6);}}\\[3pt]
    {\footnotesize\color{gray!90} Lớp mờ RGBA giúp làm nổi bật Modal mà vẫn quan sát được ngữ cảnh phía sau.}
  };
\end{tikzpicture}
\end{center}
\end{browserWindow}
...
\begin{lstlisting}[language=HTML5]
<!-- Component HTML -->
<div class="note">Thông báo lỗi hệ thống</div>
<div class="gradient-banner">Banner chuyển màu RGBA</div>

<!-- CSS Định dáng -->
<style>
.note {
  padding: 15px 20px;
  background-color: rgba(255, 0, 0, 0.1); /* Đỏ nhạt 10% làm nền */
  border: 1px solid rgb(255, 0, 0);       /* Viền đỏ đặc 100% */
  color: rgb(200, 0, 0);                   /* Chữ đỏ sẫm dễ đọc */
  border-radius: 6px;
}

/* Kết hợp RGBA với dải màu chuyển tiếp */
.gradient-banner {
  padding: 15px 20px;
...
\begin{tikzpicture}[scale=0.92, transform shape]
  ...
  \node[
    ...
    \footnotesize\color{red!70!black}\codeinline{background: rgba(255,0,0,0.1);} (Nền đỏ mờ 10\%) \quad|\quad \codeinline{border: 1px solid rgb(255,0,0);}
  };

  % 2. Linear Gradient Banner Component - Draw rectangle shading first then text node!
  \shade[left color=red!60, right color=blue!60, rounded corners=5pt] (-5.75, -1.1) rectangle (5.75, -0.1);
  \node[
    draw=none,
    font=\sffamily\bfseries\small\color{white},
    inner sep=6pt,
    text width=11cm,
    align=center
  ] at (0, -0.6) {
