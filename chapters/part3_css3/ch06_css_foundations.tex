...
\textbf{4. Ví Dụ Sử Dụng Trong HTML \& Kết Quả Hiển Thị Trực Quan:}

\begin{codeWindow}[Ví Dụ Định Kiểu Trực Tiếp Trong HTML Bằng Color Name]
\begin{lstlisting}[language=HTML5]
<h1 style="color: crimson;">Chao ban!</h1>
<p style="background-color: lightyellow;">Day la doan van demo.</p>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Kết Quả Hiển Thị Của Tên Màu Định Danh]
\vspace{2pt}
\begin{tcolorbox}[
    enhanced,
    colback=white,
  ...
  {\sffamily\bfseries\large\color{red!70!black} Chào bạn! (Chữ màu Crimson - \codeinline{color: crimson;})}\\[6pt]
  \begin{tcolorbox}[colback=yellow!15, colframe=yellow!60!black, arc=3pt, boxrule=0.4pt, top=6pt, bottom=6pt, left=8pt, right=8pt, fontupper=\sffamily\color{black}]
    Đây là đoạn văn demo có nền vàng nhạt (\codeinline{background-color: lightyellow;}).
  \end{tcolorbox}
\end{tcolorbox}
\end{browserWindow}
...
\begin{prettyTableBox}[Tóm Tắt Tổng Kết Nhanh Các Dạng Khai Báo Mã Màu]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}p{3cm}|>{\bfseries\color{accentOrange}}p{2.5cm}|>{\centering\arraybackslash}p{2.2cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{primaryBlue} \thd{\sffamily Mã màu} & \thd{\sffamily Dạng biểu diễn} & \thd{\sffamily Có Alpha?} & \thd{\sffamily Hỗ trợ viết gọn (Shorthand)?} \\
\hline
\#RRGGBB & Hệ HEX (6 ký tự) & \textcolor{red!80!black}{\textbf{Không}} & \textcolor{green!60!black}{\textbf{Có}} (rút gọn thành 3 ký tự \#ABC). \\
\hline
rgb(r, g, b) & Hệ RGB thập phân & \textcolor{red!80!black}{\textbf{Không}} & \textcolor{red!80!black}{\textbf{Không}} (bắt buộc viết đủ 3 đối số). \\
\hline
rgba(r, g, b, a) & Hệ RGBA có Alpha & \textcolor{green!60!black}{\textbf{Có}} & \textcolor{red!80!black}{\textbf{Không}} (bắt buộc viết đủ 4 đối số). \\
\hline
\end{tabularx}
\end{prettyTableBox}
...
\paragraph{3.4. Hệ Màu HSL Và HSLA (Hue, Saturation, Lightness, Alpha):}
HSL là hệ thống biểu diễn màu sắc dựa trên góc màu và cảm nhận tự nhiên của mắt người:
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Hue (Góc màu - H)}: Góc trên vòng tròn màu sắc ($0^\circ \rightarrow 360^\circ$). Trong đó: $0^\circ$ là Đỏ, $120^\circ$ là Xanh lá, $240^\circ$ là Xanh dương.
    \item \textbf{Saturation (Độ bão hòa - S\%)}: Mức độ tươi/xám của màu ($0\%$ là màu xám xịt, $100\%$ là màu tươi nguyên bản).
    \item \textbf{Lightness (Độ sáng - L\%)}: Mức độ sáng/tối ($0\%$ là đen hoàn toàn, $50\%$ là màu chuẩn, $100\%$ là trắng hoàn toàn).
    \item \textbf{Alpha (A)}: Kênh độ trong suốt từ $0.0 \rightarrow 1.0$.
\end{itemize}

\begin{codeWindow}[Ví Dụ Cú Pháp HSL \& HSLA]
\begin{lstlisting}[language=CSS3]
.brand-red   { color: hsl(0, 100%, 50%); }     /* Mau do chuan */
.soft-blue   { color: hsl(200, 80%, 70%); }    /* Mau xanh duong nhat diu */
.dark-green  { color: hsl(120, 100%, 25%); }    /* Mau xanh la dam */
.transparent-purple { color: hsla(280, 70%, 50%, 0.5); } /* Mau tim mo 50% */
\end{lstlisting}
\end{codeWindow}

\begin{tipBox}[Lợi Thế Vượt Trội Của HSL Trong Lập Trình Hệ Màu Động]
HSL cực kỳ hữu ích khi bạn muốn tạo dải màu đồng bộ (Palette) hoặc các biến thể màu sáng/tối (\term{Lighten / Darken}) cho giao diện, vì bạn chỉ cần giữ nguyên góc Hue và điều chỉnh chỉ số \codeinline{Lightness (L\%)} hoặc \codeinline{Saturation (S\%)}.
\end{tipBox}
...
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{\codeinline{currentColor}}: Là một giá trị từ khóa đặc biệt trong CSS, đại diện cho giá trị màu sắc hiện tại của thuộc tính \codeinline{color} đang áp dụng trực tiếp hoặc kế thừa lên phần tử đó.
    \item \textbf{Nguyên lý cốt lõi}: $\mathbf{\text{currentColor} \equiv \text{giá trị hiện tại của thuộc tính color}}$.
    \item \textbf{Công dụng chính}: Được dùng để tự động kế thừa và đồng bộ màu sắc cho các thuộc tính định phong cách khác như: \codeinline{border-color}, \codeinline{background-color}, \codeinline{box-shadow}, \codeinline{outline-color}, và thuộc tính tô màu đồ họa SVG \codeinline{fill} / \codeinline{stroke}.
\end{itemize}

\textbf{2. Cơ Chế Hoạt Động Nội Bộ Của Trình Duyệt}:
Trình duyệt sẽ tự động lấy giá trị của thuộc tính \codeinline{color} hiện tại (cho dù được gán trực tiếp hay kế thừa từ thẻ cha trong cây DOM) và gán trực tiếp vào những nơi khai báo từ khóa \codeinline{currentColor}.
\end{conceptBox}

\textbf{3. Các Ví Dụ Sử Dụng currentColor Từ Cơ Bản Đến Nâng Cao:}

\begin{codeWindow}[Ví Dụ 1: Đồng Bộ Màu Viền Card Đơn Giản]
\begin{lstlisting}[language=CSS3]
.box {
  color: red;
  border: 2px solid currentColor; /* Vien tu dong mang mau do sang theo color */
}
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Kết Quả Hiển Thị Đồng Bộ Màu Viền Vẫn Theo Màu Chữ Red]
\vspace{2pt}
\begin{tcolorbox}[
    enhanced,
    colback=red!5,
  ...
  .box: Văn bản màu Đỏ và viền tự động mang màu Đỏ chuẩn nhờ \codeinline{border: 2px solid currentColor;}
\end{tcolorbox}
\end{browserWindow}

\begin{codeWindow}[Ví Dụ 2: Hiệu Ứng Hover Nút Bấm Đổi Màu Toàn Bộ Linh Hoạt]
\begin{lstlisting}[language=CSS3]
/* Trang thai binh thuong */
button.btn-sync {
  color: #007bff;                /* Mau chu xanh duong */
...
/* Khi di chuot (Hover): Chi can doi duy nhat thuoc tinh color */
button.btn-sync:hover {
  color: #ff9900;                /* Vien tu dong chuyen sang mau cam! */
}
\end{lstlisting}
\end{codeWindow}

\begin{codeWindow}[Ví Dụ 3: Đồng Bộ Màu Sắc Đồ Họa Vector SVG]
\begin{lstlisting}[language=HTML5]
<!-- Thiet lap icon SVG tu dong to mau theo currentColor -->
...
<style>
svg {
  color: darkgreen; /* Icon SVG se tu dong duoc to mau darkgreen */
}
</style>
\end{lstlisting}
\end{codeWindow}

\textbf{4. Tại Sao currentColor Lại Cực Kỳ Hữu Ích Trong Lập Trình UI/UX?}

\begin{prettyTableBox}[Bảng Phân Tích Lợi Ích Của Từ Khóa currentColor]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}p{3.2cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{primaryBlue} \thd{\sffamily Lý do nên dùng} & \thd{\sffamily Ý nghĩa \& Giá trị thực tế} \\
\hline
Đồng bộ màu dễ dàng & Dùng $1$ giá trị màu duy nhất cho nhiều thuộc tính (\codeinline{color}, \codeinline{border}, \codeinline{box-shadow}, SVG \codeinline{fill}). \\
\hline
Dễ bảo trì mã nguồn & Khi đổi màu giao diện, bạn chỉ cần thay đổi duy nhất $1$ dòng thuộc tính \codeinline{color}, toàn bộ viền và icon liên quan tự động cập nhật theo. \\
\hline
Hỗ trợ Pseudo-classes & Phối hợp hoàn hảo với trạng thái \codeinline{:hover}, \codeinline{:focus}, \codeinline{:active} và từ khóa kế thừa \codeinline{inherit}. \\
\hline
Hỗ trợ Dark / Light Theme & Cực kỳ linh hoạt khi xây dựng hệ thống chuyển đổi chủ đề sáng/tối (Theme switch) nhờ tính tự động kế thừa màu chữ của phần tử cha. \\
\hline
\end{tabularx}
\end{prettyTableBox}
...
\paragraph{4. Mẹo Chọn Màu Sắc Chuyên Nghiệp Cho Giao Diện Web (UI/UX Best Practices):}
...
\paragraph{5. Ứng Dụng Màu Động Với CSS Variables \& JavaScript:}
...
\paragraph{6. Bảng So Sánh Tổng Quát Các Hệ Màu Trong CSS:}
...
\paragraph{7. Kết Luận – Khi Nào Nên Dùng Hệ Màu Nào?}
...
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    ...
    \begin{codeWindow}[Nội Dung Dropdown Xuất Hiện]
...
\begin{conceptBox}[4. Ví Dụ Định Kiểu Tương Tác Với Thẻ Liên Kết (Link)]
...
\end{lstlisting}
\end{codeWindow}
\textbf{Giải thích chi tiết các trạng thái}:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
