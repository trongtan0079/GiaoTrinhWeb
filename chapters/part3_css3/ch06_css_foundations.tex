...
\paragraph{\textcolor{accentOrange}{\textbf{[Mục Đích Cốt Lõi]}} Mục Đích Của CSS:}
...
\paragraph{a. Cấu Trúc Cơ Bản Của CSS:}
...
\paragraph{b. Ví Dụ Cụ Thể \& Giải Thích:}
...
\paragraph{1. Inline CSS (CSS Trong Thẻ HTML):}
...
\paragraph{2. Internal CSS (CSS Nội Bộ):}
...
\paragraph{3. External CSS (CSS Bên Ngoài):}
...
\paragraph{a. Bảng Tra Cứu Một Số Thuộc Tính CSS Cơ Bản:}
...
\paragraph{3.1. Color Keywords (Tên Màu Định Sẵn):}
CSS định nghĩa sẵn khoảng \textbf{147 tên màu từ khóa chuẩn} (ví dụ: \codeinline{red}, \codeinline{blue}, \codeinline{green}, \codeinline{lightgray}, \codeinline{darkcyan}, \codeinline{tomato}, \codeinline{transparent}).

\begin{codeWindow}[Ví Dụ Sử Dụng Tên Màu Định Sẵn]
\begin{lstlisting}[language=CSS3]
h1 {
  color: tomato;             /* Mau do ca chua */
  background-color: lightgray; /* Mau nen xam nhat */
}
\end{lstlisting}
\end{codeWindow}

\begin{conceptBox}[Đánh Giá Tên Màu Từ Khóa]
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Ưu điểm}: Dễ nhớ, dễ viết tay cho bài tập nhỏ hoặc thử nghiệm nhanh.
    \item \textbf{Hạn chế}: Không linh hoạt, bị giới hạn trong 147 màu định sẵn và không hỗ trợ tùy chỉnh tinh tế hay độ mờ trong suốt.
\end{itemize}
\end{conceptBox}

\paragraph{3.2. Hexadecimal (Mã Thập Lục Phân \#RRGGBB):}
Mã Hex là hệ thống biểu diễn màu sắc phổ biến nhất trên Web. Cú pháp bắt đầu bằng dấu \codeinline{\#} theo sau bởi 6 ký tự đại diện cho 3 cặp kênh màu: \codeinline{\#RRGGBB} (Red, Green, Blue).
Mỗi cặp nhận giá trị từ \codeinline{00} ($0$) đến \codeinline{FF} ($255$).

\begin{codeWindow}[Ví Dụ Cú Pháp Mã Hex]
\begin{lstlisting}[language=CSS3]
.red-box   { color: #ff0000; } /* Do thuan (R=255, G=0, B=0) */
.green-box { color: #00ff00; } /* Xanh la thuan (R=0, G=255, B=0) */
.blue-box  { color: #0000ff; } /* Xanh duong thuan (R=0, G=0, B=255) */
.white-box { color: #ffffff; } /* Trang tinh */
.black-box { color: #000000; } /* Den tuyen */

/* Dang rut gon (Shorthand Hex): khi cac cap ky tu trung nhau */
.short-red { color: #f00; }    /* Tuong duong #ff0000 */
.short-gray { color: #ccc; }   /* Tuong duong #cccccc */
\end{lstlisting}
\end{codeWindow}

\begin{tipBox}[Tại Sao Mã Hex Lại Chuẩn Trong Thiết Kế UI/UX?]
...
\textbf{1. Khái Niệm Cơ Bản Về RGB \& RGBA:}
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{RGB (Red - Green - Blue)}: Là hệ màu dựa trên \term{Mô hình ánh sáng cộng} (\term{Additive Color Model}). Trong mô hình này, các màu sắc được tạo ra bằng cách kết hợp 3 chùm ánh sáng màu cơ bản: Đỏ (Red), Xanh lá (Green) và Xanh dương (Blue). Đây là tiêu chuẩn mô hình màu được sử dụng phổ biến nhất trên các thiết bị điện tử như màn hình máy tính, TV, điện thoại và giao diện Web.
    \item \textbf{Thang giá trị}: Mỗi kênh màu $R, G, B$ nhận giá trị số nguyên từ \codeinline{0} đến \codeinline{255} ($0 \rightarrow 255$), tương ứng với $256$ mức cường độ phát sáng của từng kênh màu.
    \item \textbf{RGBA (Red - Green - Blue - Alpha)}: Là phiên bản mở rộng của RGB, bổ sung thêm kênh \textbf{Alpha ($A$)} cho phép thiết lập \term{độ trong suốt} (\term{Opacity}) của màu nền, đường viền, chữ hoặc hiệu ứng đổ bóng. Kênh Alpha nhận giá trị số thực từ \codeinline{0.0} (trong suốt tuyệt đối $0\%$) đến \codeinline{1.0} (đặc nguyên bản $100\%$).
\end{itemize}

\textbf{2. Cú Pháp Khai Báo Trong CSS \& Bảng Tham Số Chi Tiết:}
\begin{codeWindow}[Cú Pháp Định Dạng RGB \& RGBA Trong CSS]
\begin{lstlisting}[language=CSS3]
/* Cu phap RGB truyen thong (3 tham so) */
color: rgb(R, G, B);

/* Cu phap RGBA co kenh Alpha (4 tham so) */
color: rgba(R, G, B, A);
\end{lstlisting}
\end{codeWindow}
...
\textbf{3. Ví Dụ Cụ Thể Về Cú Pháp RGB \& RGBA:}

\begin{codeWindow}[Ví Dụ Sử Dụng RGB Cơ Bản Và RGBA Trong Suốt]
\begin{lstlisting}[language=CSS3]
/* RGB co ban (Mau dac 100%) */
p {
  color: rgb(255, 0, 0);          /* Mau do thuan (R=255, G=0, B=0) */
  background-color: rgb(0, 255, 0); /* Mau nen xanh la (R=0, G=255, B=0) */
}

/* RGBA co do trong suot */
.box {
  background-color: rgba(0, 0, 255, 0.5); /* Xanh duong mo 50% (Alpha = 0.5) */
}
\end{lstlisting}
\end{codeWindow}

\textbf{4. Nguyên Lý Phối Màu Tuỳ Chỉnh Bằng RGB:}
...
\textbf{5. Đánh Giá Ưu Điểm Và Hạn Chế Của Hệ Màu RGB:}
...
\textbf{6. Hiệu Ứng Khi Thay Đổi Kênh Alpha ($A$) Trong RGBA:}
...
\textbf{7. Ứng Dụng Phổ Biến Của RGBA Trong Thiết Kế Giao Diện UI/UX:}
...
\begin{browserWindow}[Kết Quả Mô Phỏng Lớp Phủ Trong Suốt RGBA Trên Nền Web]
\sffamily\small
\begin{center}
\begin{tikzpicture}[scale=0.9, transform shape]
  % Nền trang web (Background Content)
  \fill[primaryBlue!15] (-0.5,-0.5) rectangle (10.5, 2.5);
  \node[font=\sffamily\bfseries\Large, color=primaryBlue!80!black] at (5, 1.0) {Nội Dung Nền Trang Web Phía Dưới};
  
  % Lớp phủ RGBA (0,0,0,0.7)
  \fill[black, fill opacity=0.7] (1.0, -0.2) rectangle (9.0, 2.2);
  \node[font=\sffamily\bfseries\color{white}] at (5, 1.3) {Cửa Sổ Modal (Overlay background: rgba(0,0,0,0.7))};
  \node[font=\sffamily\small\color{accentOrange}] at (5, 0.6) {Vẫn nhìn xuyên qua được nội dung nền phía sau!};
\end{tikzpicture}
\end{center}
\end{browserWindow}
...
\begin{lstlisting}[language=HTML5]
<!-- Component HTML -->
<div class="note">Thuoc tinh va thong bao loi</div>

<!-- CSS Dinh dang -->
<style>
.note {
  padding: 20px;
  background-color: rgba(255, 0, 0, 0.1); /* Do nhieu 10% tao nen nhat diu */
  border: 1px solid rgb(255, 0, 0);       /* Vien do dac 100% */
  color: rgb(200, 0, 0);                   /* Mau chu do sdt de doc */
}

/* Ket hop RGB/RGBA voi linear-gradient */
.gradient-banner {
...
\textbf{2. Nguyên Lý Xử Lý Alpha Blending Trên Phần Cứng Card Đồ Họa (GPU):}
...
\]
Quá trình này được tăng tốc phần cứng (\term{GPU Hardware Acceleration}) thông qua việc tạo ra một lớp tổng hợp (\term{Compositor Layer}) riêng biệt.

\textbf{3. Lưu Ý Khả Năng Truy Cập (Accessibility \& WCAG Contrast):}
...
\paragraph{3.5. Từ Khóa Đặc Biệt: transparent Và currentColor:}

\begin{conceptBox}[Ý Nghĩa Của 2 Từ Khóa Đặc Biệt]
\begin{itemize}[leftmargin=1.5em, itemsep=4pt, parsep=0pt]
    \item \textbf{\codeinline{transparent}}: Tạo thuộc tính hoàn toàn trong suốt, thường dùng để ẩn đường viền hoặc làm mờ màu nền ban đầu.
    \item \textbf{\codeinline{currentColor}}: Tự động lấy giá trị màu chữ (\codeinline{color}) hiện tại của phần tử. Giúp đồng bộ màu viền (\codeinline{border}), màu đổ bóng (\codeinline{box-shadow}) hoặc màu icon biểu tượng SVG với màu chữ mà không cần khai báo lại mã màu.
\end{itemize}
\end{conceptBox}

\begin{codeWindow}[Ví Dụ Ứng Dụng Từ Khóa currentColor]
\begin{lstlisting}[language=CSS3]
.card {
  color: #007bff;                /* Mau chu xanh duong */
  border: 2px solid currentColor; /* Vien tu dong co mau xanh duong #007bff */
  box-shadow: 0 4pt 8pt currentColor;
}
\end{lstlisting}
\end{codeWindow}
