...
\begin{tikzpicture}[font=\sffamily]
  % Khung cha .parent (chiều cao tự co)
  \draw[fill=gray!20, draw=gray!60, line width=1pt, rounded corners=4pt] (0,0) rectangle (9.5, 1.4);
  \node[anchor=north west, font=\bfseries\small, color=darkText] at (0.2, 1.3) {.parent \textmd{(Không có thuộc tính height $\rightarrow$ chiều cao co theo nội dung)}};
  
  % Khối con .child (bị co theo chữ)
  \draw[fill=pink!60, draw=pink!90!black, line width=1pt, rounded corners=3pt] (0.3, 0.15) rectangle (9.2, 0.85);
  \node[anchor=center, font=\bfseries\footnotesize, color=purple!90!black] at (4.75, 0.5) {.child \{ height: 50\%; \} $\rightarrow$ \textcolor{red!80!black}{\textbf{Bị co rút thành 26px (chỉ ôm khít chữ "Hello")}}};

  % Thước đo chiều cao bên trái
  \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.8pt, color=red!80!black] (-0.2, 0) -- (-0.2, 1.4)
    node[midway, left=6pt, font=\scriptsize\bfseries, color=red!80!black, align=right] {Chiều cao cha\\bằng nội dung\\($\approx 35\text{px}$)};
\end{tikzpicture}
\par\vspace{4pt}
...
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung cha .parent (tổng chiều cao 400px ~ 3.4cm)
  \draw[fill=gray!15, draw=gray!50, line width=1pt, rounded corners=4pt] (0,0) rectangle (9.5, 3.4);
  \node[anchor=north west, font=\bfseries\small, color=darkText] at (0.2, 3.3) {.parent \{ height: 400px; background: \#e0e0e0; \}};
  
  % Khối con .child (chiếm đúng 50% = 200px ~ 1.5cm trên)
  \draw[fill=pink!65, draw=purple!80!black, line width=1.1pt, rounded corners=3pt] (0.3, 1.7) rectangle (9.2, 2.9);
  \node[anchor=center, font=\bfseries\small, color=purple!90!black] at (4.75, 2.3) {\textcolor{green!60!black}{\textbf{[ĐẠT]}} .child \{ height: 50\%; \} $= \mathbf{200\text{px}}$ (Chiếm trọn vẹn $50\%$ nửa trên)};
  
  % Vùng trống 50% còn lại của cha
  \draw[dashed, draw=gray!40, fill=white!60, rounded corners=3pt] (0.3, 0.15) rectangle (9.2, 1.55);
  \node[anchor=center, font=\scriptsize\color{codeComment}] at (4.75, 0.85) {Vùng trống $50\%$ còn lại của phần tử cha ($200\text{px}$)};

  % Thước đo tổng thể bên trái (400px)
  \draw[decorate, decoration={brace, amplitude=5pt}, line width=0.9pt, color=primaryBlue] (-0.2, 0) -- (-0.2, 3.4)
  ...
  \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.8pt, color=purple!80!black] (9.7, 2.9) -- (9.7, 1.7)
...
\begin{tikzpicture}[font=\sffamily]
  ...
  \draw[fill=white, draw=red!40, line width=0.9pt, rounded corners=4pt] (0,0) rectangle (4.5, 3.4);
  \node[anchor=north, font=\scriptsize\bfseries, color=red!80!black] at (2.25, 3.3) {[CHƯA KHAI BÁO] Chưa set <html>};
  \draw[fill=pink!60, draw=red!70!black, line width=0.8pt, rounded corners=2pt] (0.2, 2.2) rectangle (4.3, 2.9);
  \node[font=\tiny\bfseries\color{purple!90!black}] at (2.25, 2.55) {.child (Bị co rút $\approx 30\text{px}$)};
  \draw[dashed, draw=gray!30] (0.2, 0.2) rectangle (4.3, 2.0);
  \node[font=\tiny\color{gray!60}, align=center] at (2.25, 1.1) {Khoảng trắng thừa\\(body không cao $100\%$)};

  % Cửa sổ phải (Đúng - Đã set html, body 100%)
  \draw[fill=white, draw=primaryBlue!40, line width=0.9pt, rounded corners=4pt] (5.0,0) rectangle (9.5, 3.4);
  \node[anchor=north, font=\scriptsize\bfseries, color=primaryBlue] at (7.25, 3.3) {[ĐÃ KHAI BÁO] html, body \{ height: 100\%; \}};
  \draw[fill=pink!70, draw=purple!80!black, line width=1pt, rounded corners=2pt] (5.2, 0.2) rectangle (9.3, 2.9);
  \node[font=\scriptsize\bfseries\color{purple!90!black}, align=center] at (7.25, 1.55) {.child \{ height: 100\%; \}\\\textbf{Phủ kín $100\%$ màn hình!}};
\end{tikzpicture}
\end{browserWindow}
