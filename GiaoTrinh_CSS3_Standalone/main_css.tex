...
\subparagraph{1. Absolute Units (Đơn Vị Tuyệt Đối -- Không Thay Đổi Theo Bối Cảnh):}
...
\subparagraph{1. Pixel (px) Là Gì?}

\begin{conceptBox}[Khái Niệm Pixel Và Pixel Tham Chiếu Trong CSS]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item $1\text{px} = 1$ \textbf{điểm ảnh tham chiếu trên màn hình}.
    \item Là \textbf{đơn vị cố định}, không thay đổi theo kích thước của phần tử cha hay kích thước màn hình (tương tự như centimet trong thực tế -- độc lập, không phụ thuộc vào ai).
    \item \textbf{Pixel vật lý (Physical Pixel)}: Trong bối cảnh màn hình kỹ thuật số, một pixel vật lý là điểm ảnh nhỏ nhất trên tấm nền hiển thị có thể được điều khiển riêng lẻ để phát màu sắc. Màn hình được tạo thành từ hàng triệu pixel vật lý xếp cạnh nhau.
    \item \textbf{Pixel trong CSS (CSS Reference Pixel)}: Trong CSS, \codeinline{px} đại diện cho một \term{Pixel tham chiếu (Reference Pixel)}, không nhất thiết phải tương ứng tỷ lệ $1:1$ với một pixel vật lý trên màn hình.
\end{itemize}
\end{conceptBox}

\subparagraph{2. Sự Khác Biệt Giữa Pixel Trong CSS Và Pixel Vật Lý (HiDPI / Retina Display):}
Ban đầu, $1\text{px}$ trong CSS thường tương đương với $1$ pixel vật lý trên màn hình. Tuy nhiên, với sự ra đời của các màn hình có mật độ điểm ảnh cao (\term{Retina Display} của Apple, màn hình \term{HiDPI}), sự tương quan này đã thay đổi:
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Màn hình tiêu chuẩn (Standard DPI)}: $1\text{px}$ CSS thường tương đương với $1$ pixel vật lý.
    \item \textbf{Màn hình mật độ cao (HiDPI / Retina)}: Để đảm bảo nội dung hiển thị sắc nét, trình duyệt sẽ tự động điều chỉnh. Ví dụ, trên màn hình Retina, $1\text{px}$ CSS có thể tương ứng với $2 \times 2$ (4 pixel vật lý) hoặc $3 \times 3$ (9 pixel vật lý).
    \item \textbf{Ví dụ minh họa}: Một phần tử có \codeinline{width: 100px;} trong CSS sẽ chiếm $100$ "pixel tham chiếu" trên màn hình, nhưng được trình duyệt render bằng $200$ hoặc $300$ pixel vật lý để đường nét trông mịn màng và sắc nét hơn.
    \item \textbf{Mục đích cốt lõi}: Đảm bảo kích thước các phần tử trên trang web trông ổn định về mặt vật lý trên các thiết bị khác nhau, bất kể mật độ điểm ảnh. Đoạn văn $16\text{px}$ sẽ có kích thước vật lý tương đương trên màn hình tiêu chuẩn và màn hình Retina.
\end{itemize}

\subparagraph{3. Đơn Vị Tuyệt Đối Là Gì?}
...
\subparagraph{4. Ví Dụ Đơn Giản Khai Báo px Trong CSS:}
...
\subparagraph{5. Đặc Điểm Của Đơn Vị px (Ưu Điểm \& Nhược Điểm):}
...
\subparagraph{6. So Sánh px Với Các Đơn Vị Khác (em, rem, \%, vw, vh):}
...
\subparagraph{7. Các Trường Hợp Sử Dụng Phổ Biến Của Pixel (px):}
\begin{itemize}[leftmargin=1.5em, itemsep=4pt, parsep=0pt]
    \item \textbf{Độ dày đường viền (\codeinline{border-width})}: Thích hợp nhất cho viền cố định.
\begin{lstlisting}[language=CSS3]
.element { border: 1px solid #ccc; }
\end{lstlisting}
    \item \textbf{\codeinline{padding} và \codeinline{margin} khoảng cách nhỏ/cố định}:
\begin{lstlisting}[language=CSS3]
.button { padding: 10px 20px; margin-top: 15px; }
\end{lstlisting}
    \item \textbf{Kích thước Icon, Logo và hình ảnh nhỏ}:
\begin{lstlisting}[language=CSS3]
.icon { width: 24px; height: 24px; }
\end{lstlisting}
    \item \textbf{Giới hạn kích thước (\codeinline{min-width}, \codeinline{max-width}, \codeinline{min-height}, \codeinline{max-height})}:
\begin{lstlisting}[language=CSS3]
.content { max-width: 960px; /* Noi dung khong bao gio rong hon 960px */ }
\end{lstlisting}
    \item \textbf{Hiệu ứng viền \& bóng đổ}: \codeinline{border-radius}, \codeinline{box-shadow}, \codeinline{text-shadow}.
    \item \textbf{\codeinline{font-size} cố định}:
\begin{lstlisting}[language=CSS3]
h1 { font-size: 32px; }
...
\subparagraph{8. Khi Nào Nên Dùng px Và Khi Nào Nên Dùng Đơn Vị Tương Đối?}
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
...
\subparagraph{2. Relative Units (Đơn Vị Tương Đối -- Thay Đổi Theo Bối Cảnh):}
Đơn vị tương đối tính toán kích thước dựa trên một giá trị tham chiếu khác (như phông chữ của phần tử cha, phông chữ gốc \codeinline{<html>}, hoặc kích thước khung hình trình duyệt \term{Viewport}).
