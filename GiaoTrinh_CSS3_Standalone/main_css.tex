...
\paragraph{2. Công Thức Quy Đổi Toán Học:}
Điểm tựa toán học cốt lõi của \codeinline{rem}:
\[
\mathbf{1\text{rem} = \text{font-size của thẻ gốc \codeinline{<html>}}}
\]

Theo mặc định tiêu chuẩn của hầu hết mọi trình duyệt web:
\begin{lstlisting}[language=CSS3]
html {
  font-size: 16px; /* Mac dinh cua moi trinh duyet */
}
\end{lstlisting}

Từ đó, ta có các giá trị quy đổi tức thời:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item $1\text{rem} = 1 \times 16\text{px} = \mathbf{16\text{px}}$
    \item $2\text{rem} = 2 \times 16\text{px} = \mathbf{32\text{px}}$
    \item $0.5\text{rem} = 0.5 \times 16\text{px} = \mathbf{8\text{px}}$
\end{itemize}

\begin{conceptBox}[Công Thức Chuyển Đổi Đơn Vị rem Sang Pixel]
\[
\text{Giá Trị Thực Tế (px)} = \text{Giá Trị rem} \times \text{font-size của thẻ gốc \codeinline{<html>} (Mặc định: } 16\text{px)}
\]
\end{conceptBox}
