...
\begin{conceptBox}[2.1. Bản Chất Toán Học Của Đơn Vị vh]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Định nghĩa}: \codeinline{vh} viết tắt của \term{Viewport Height} (chiều cao khung nhìn trình duyệt).
    \item \textbf{Quy đổi chuẩn}: $\mathbf{1\text{vh} = 1\%}$ chiều cao của cửa sổ trình duyệt hiện tại.
    \item \textbf{Ví dụ}: Nếu trình duyệt cao $800\text{px}$ thì $1\text{vh} = 8\text{px}$, $50\text{vh} = 400\text{px}$, $100\text{vh} = 800\text{px}$.
\end{itemize}
\end{conceptBox}

\begin{codeWindow}[Ứng Dụng Thực Chiến: Tạo Hero Banner Phủ Kín 100\% Chiều Cao Màn Hình]
\begin{lstlisting}[language=HTML5]
<style>
.hero-fullscreen {
  width: 100%;
...
\begin{browserWindow}[Mô Phỏng Trực Quan: Hero Section 100vh Phủ Toàn Màn Hình]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung trình duyệt Viewport (rộng 9cm, cao 3.2cm)
  \draw[fill=blue!85!black, draw=primaryBlue!80!black, line width=1pt, rounded corners=4pt] (0,0) rectangle (9.2, 3.2);
  
  % Nội dung Hero căn giữa
  \node[anchor=center, font=\bfseries\normalsize, color=white] at (4.6, 2.0) {Chào Mừng Đến Với Khóa Học CSS Masterclass};
  \node[anchor=center, font=\scriptsize, color=white!80] at (4.6, 1.45) {Giao diện tự động căn giữa toàn màn hình với height: 100vh};
  \draw[fill=orange!90!black, draw=orange, rounded corners=3pt] (3.3, 0.45) rectangle (5.9, 0.95);
  \node[anchor=center, font=\scriptsize\bfseries\color{white}] at (4.6, 0.7) {Bắt Đầu Học Ngay};

  % Thước đo chiều cao 100vh bên trái
  \draw[decorate, decoration={brace, amplitude=5pt}, line width=0.9pt, color=cyan!80!white] (-0.3, 0) -- (-0.3, 3.2)
    node[midway, left=7pt, font=\scriptsize\bfseries, color=primaryBlue, align=right] {Chiều cao\\$100\text{vh}$ ($100\%$)};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: Phần tử luôn lấp đầy đúng $100\%$ chiều cao của cửa sổ trình duyệt trên mọi thiết bị).}
\end{browserWindow}

\begin{warnBox}[Cạm Bẫy Kinh Điển: Lỗi Xuất Hiện Thanh Cuộn Dọc Khi Dùng 100vh]
Khi bạn thiết lập \codeinline{height: 100vh;} nhưng phần tử lại có thêm \codeinline{padding} hoặc \codeinline{border} mà quên khai báo \codeinline{box-sizing: border-box;}, tổng chiều cao thực tế sẽ bị cộng dồn: $\text{Tổng Chiều Cao} = 100\text{vh} + 2 \times \text{Padding}$. Việc này làm trang web dài hơn màn hình và xuất hiện thanh cuộn dọc không mong muốn!
\end{warnBox}
