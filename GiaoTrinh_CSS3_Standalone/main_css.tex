...
\begin{prettyTableBox}[Bảng So Sánh Chi Tiết 5 Nhóm Generic Font Families]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
...
\paragraph{Mở Rộng: Chuyên Đề System Font Stack (system-ui Masterclass):}
System Font (phông chữ hệ thống) là phông chữ mặc định cài sẵn trên hệ điều hành (macOS: San Francisco, Windows: Segoe UI, Android: Roboto, Linux: Ubuntu). 
\begin{codeWindow}[Chuỗi Khai Báo System Font Stack Hoàn Chỉnh]
\begin{lstlisting}[language=CSS3]
/* Su dung font-family he dieu hanh giup trang web tai cuc nhanh va hoa nhap UI */
body {
...
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}p{3.2cm}|>{\ttfamily\color{accentOrange}\bfseries}p{2.8cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{primaryBlue} \thd{\sffamily Phân loại đơn vị} & \thd{\sffamily Ký hiệu / Từ khóa} & \thd{\sffamily Mô tả kỹ thuật \& Đặc tính} \\
\hline
\multirow{3}{*}{\textbf{Đơn vị tuyệt đối}} & px (pixels) & Kích thước cố định, không thay đổi theo thiết lập của người dùng hoặc kích thước màn hình. \\
\cline{2-3}
 & pt (points) & Thường dùng trong in ấn (\codeinline{1pt = 1/72 inch}). \\
\cline{2-3}
 & in, cm, mm & Đơn vị độ dài vật lý, ít dùng trong thiết kế web. \\
\hline
\multirow{6}{*}{\textbf{Đơn vị tương đối}} & em & Tương đối với kích thước font của phần tử cha trực tiếp. Ví dụ: Nếu thẻ cha có \codeinline{font-size: 16px}, thì \codeinline{1.5em} sẽ là \codeinline{24px}. Có thể bị ảnh hưởng bởi hiệu ứng tầng cascading. \\
\cline{2-3}
...
\hline
\multirow{2}{*}{\textbf{Từ khóa tương đối}} & xx-small đến xx-large & Các mức kích thước tương đối so với font mặc định của trình duyệt. \\
\cline{2-3}
...
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}p{3.5cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{primaryBlue} \thd{\sffamily Mục tiêu tối ưu} & \thd{\sffamily Giải pháp \& Kỹ thuật khuyến nghị} \\
\hline
\multirow{4}{*}{\textbf{Hiệu suất (Performance)}} & \textbf{Giảm số lượng font và style}: Chỉ nhúng những font và style (Regular 400, Bold 700, Italic) thực sự cần thiết trong thiết kế. \\
\cline{2-2}
 & \textbf{Sử dụng \codeinline{font-display: swap;}}: Tránh hiện tượng nhấp nháy chữ trắng (\term{FOIT}) và cải thiện điểm chỉ số Core Web Vitals (LCP, CLS). \\
\cline{2-2}
 & \textbf{Tối ưu hóa tệp font (Subsetting)}: Sử dụng công cụ như Font Squirrel Webfont Generator để nén tệp font WOFF2 và loại bỏ các ký tự không dùng đến. \\
\cline{2-2}
 & \textbf{Sử dụng \codeinline{preconnect} / \codeinline{preload}}: \codeinline{<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>} và \codeinline{<link rel="preload" href="..." as="style">}. \codeinline{preconnect} thiết lập kết nối sớm đến máy chủ font, \codeinline{preload} yêu cầu trình duyệt tải tài nguyên sớm hơn. \\
\hline
\multirow{4}{*}{\textbf{Khả năng đọc (Readability)}} & \textbf{Kích thước font phù hợp}: Đảm bảo kích thước chữ đủ lớn trên mọi thiết bị. Kích thước cơ bản tiêu chuẩn trên Desktop thường là \codeinline{16px}. \\
\cline{2-2}
 & \textbf{Chiều cao dòng tiêu chuẩn (\codeinline{line-height})}: Đặt \codeinline{line-height} từ \codeinline{1.5} đến \codeinline{1.8} để tạo khoảng cách đọc thoải mái, tránh mỏi mắt. \\
\cline{2-2}
 & \textbf{Độ tương phản màu sắc}: Đảm bảo màu chữ có đủ độ tương phản (\term{Contrast Ratio}) với màu nền theo tiêu chuẩn WCAG. \\
\cline{2-2}
 & \textbf{Thuộc tính \codeinline{text-rendering}}: Cải thiện hiển thị nét chữ trên một số trình duyệt bằng cách áp dụng \codeinline{text-rendering: optimizeLegibility;} (hoặc \codeinline{geometricPrecision}, \codeinline{optimizeSpeed}). \\
\hline
\multirow{2}{*}{\textbf{Đáp ứng (Responsive Design)}} & \textbf{Đơn vị tương đối}: Sử dụng \codeinline{rem}, \codeinline{em}, \codeinline{vw} cho \codeinline{font-size} để font tự động co giãn linh hoạt theo màn hình. \\
\cline{2-2}
 & \textbf{Media Queries}: Điều chỉnh \codeinline{font-size} cho từng mốc màn hình cụ thể bằng \codeinline{@media (max-width: 768px) \{ body \{ font-size: 14px; \} h1 \{ font-size: 2rem; \} \}}. \\
\hline
\end{tabularx}
\end{prettyTableBox}
...
\textbf{1. Variable Fonts (Font Biến Thể CSS Fonts Level 4)}:
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Khái niệm}: Variable Font cho phép đóng gói toàn bộ các trọng lượng (\codeinline{wght}), độ rộng (\codeinline{wdth}), độ nghiêng (\codeinline{slnt}) vào trong \textbf{chỉ 1 tệp font duy nhất} (.woff2).
    \item \textbf{Cú pháp khai báo trong CSS}:
\begin{lstlisting}[language=CSS3]
h1 {
...
\end{lstlisting}
    \item \textbf{Lợi ích}: Tiết kiệm tới 80\% dung lượng tải mạng so với việc tải 10 tệp font tĩnh riêng biệt!
\end{itemize}

\textbf{2. CSS Font Loading API Trong JavaScript}:
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item Giúp lập trình viên chủ động kiểm soát trạng thái tải font bằng JavaScript thông qua đối tượng \codeinline{document.fonts}:
\begin{lstlisting}[language=JavaScript]
...
\textbf{3. Ngăn Trình Duyệt Tự Sinh Font Giả (\codeinline{font-synthesis})}:
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item Khi font không có bản Bold hoặc Italic thực sự, trình duyệt có thể tự bóp méo ký tự để làm nghiêng/đậm giả. Để ngăn chặn điều này:
\begin{lstlisting}[language=CSS3]
body {
