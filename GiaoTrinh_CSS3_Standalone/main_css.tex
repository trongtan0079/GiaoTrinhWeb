...
\paragraph{1. Đơn Vị Phần Trăm (\%) Là Gì?}

\begin{conceptBox}[Khái Niệm Đơn Vị Tương Đối Phần Trăm (\%)]
\textbf{Đơn vị phần trăm (\codeinline{\%})} là một đơn vị tương đối trong CSS -- giá trị của nó không cố định mà được tính toán dựa trên một thuộc tính tương ứng của phần tử cha (\term{Parent Element}) hoặc một thuộc tính cụ thể của chính phần tử đó.
\par\vspace{4pt}
...
\paragraph{2. Đơn Vị \% Hoạt Động Như Thế Nào? (Quy Tắc Điểm Tham Chiếu):}
Điểm tham chiếu để tính toán giá trị \codeinline{\%} thay đổi tùy thuộc vào thuộc tính CSS mà nó được áp dụng:
...
\hline
\rowcolor{primaryBlue} \thd{Thuộc Tính CSS} & \thd{Phụ Thuộc Vào...} & \thd{Giải Thích Chi Tiết \& Ví Dụ Tính Toán} \\
\hline
width & \codeinline{width} của phần tử cha & Nếu cha rộng $500\text{px}$, con có \codeinline{width: 50\%} $\rightarrow$ con rộng $250\text{px}$. \\
\hline
height & \codeinline{height} của phần tử cha & Cần phần tử cha có chiều cao xác định (nếu cha không set \codeinline{height}, \codeinline{height: \%} không hoạt động). \\
\hline
margin / padding & \textbf{\codeinline{width} của phần tử cha} & \textbf{Lưu ý quan trọng}: Cả chiều ngang (\codeinline{left}/\codeinline{right}) và chiều dọc (\codeinline{top}/\codeinline{bottom}) của \codeinline{padding} và \codeinline{margin} \textbf{đều dựa trên chiều rộng của phần tử cha}. \\
\hline
font-size & \codeinline{font-size} của phần tử cha gần nhất & Nếu cha có \codeinline{font-size: 16px}, con có \codeinline{font-size: 150\%} $\rightarrow$ con đạt $24\text{px}$. \\
\hline
line-height & \codeinline{font-size} của chính phần tử đó & Tính dựa trên kích thước phông chữ thực tế của phần tử hiện tại. \\
\hline
transform & Kích thước của chính phần tử đó & \codeinline{transform: translateX(50\%)} sẽ dịch chuyển phần tử sang phải $50\%$ chiều rộng của chính nó. \\
\hline
...
\paragraph{3. Ví Dụ Trực Quan Khai Báo Đơn Vị \% (Khối .parent \& .child):}

\begin{codeWindow}[Ví Dụ Tính Kích Thước Khối Con Theo \% Của Khối Cha]
\begin{lstlisting}[language=HTML5]
<style>
.parent {
  width: 400px;
...
.child {
  width: 50%;     /* 50% cua 400px = 200px */
  height: 50%;    /* 50% cua 200px = 100px */
  background: #00bcd4;
  color: white;
}
</style>
...
<div class="parent">
  <div class="child">Child Block</div>
</div>
\end{lstlisting}
\end{codeWindow}
...
\begin{tcolorbox}[enhanced, colback=gray!20, colframe=gray!50, width=240pt, height=120pt, arc=6pt, boxrule=1pt, top=8pt, bottom=8pt, left=8pt, right=8pt]
  ...
  \begin{tcolorbox}[colback=cyan!80!black, colframe=cyan!90!black, width=120pt, height=60pt, arc=4pt, boxrule=1pt, top=4pt, bottom=4pt, left=6pt, right=6pt]
    \color{white}\bfseries\small .child ($200\text{px} \times 100\text{px}$)\par
    \tiny (Width 50\% cha, Height 50\% cha)
  \end{tcolorbox}
\end{tcolorbox}
\end{browserWindow}

\paragraph{4. Điểm Đặt Biệt: padding Và margin Theo Đơn Vị \%:}

\begin{codeWindow}[Mã Nguồn Minh Hòa padding: 10\% Theo Chiều Rộng Phủ Khối]
\begin{lstlisting}[language=HTML5]
<style>
.box {
  width: 300px;
  padding: 10%; /* 10% cua 300px (chieu rong cha) = 30px */
  background: #fff3e0;
...
<div class="box">
  Padding-top, bottom, left, right deu la 30px vi tinh theo width (300px)!
</div>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng Trực Quan padding: 10\% Tính Theo Width Khối Cha]
\sffamily\small
\centering
\begin{tcolorbox}[enhanced, colback=orange!8, colframe=orange!80!black, width=220pt, arc=6pt, boxrule=1.5pt, top=14pt, bottom=14pt, left=14pt, right=14pt]
  \centering
  \textcolor{darkText}{\bfseries Khối .box ($300\text{px}$ Width)}\par\vspace{4pt}
  \textcolor{orange!90!black}{\small (Tất cả 4 hướng Padding top/bottom/left/right đều bằng $30\text{px} = 10\%$ của Width $300\text{px}$).}
\end{tcolorbox}
\end{browserWindow}

\paragraph{5. Ưu Điểm Và Nhược Điểm Của Đơn Vị phần trăm (\%):}

\begin{prettyTableBox}[Bảng Tổng Hợp Ưu Điểm Và Nhược Điểm Của Đơn Vị \%]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|Z|Z|}
\hline
\rowcolor{primaryBlue} \thd{Ưu Điểm Của Đơn Vị \%} & \thd{Nhược Điểm \& Cân Nhắc} \\
\hline
\textbf{Responsive Design (Thiết kế đáp ứng)}: Tự động co giãn theo kích thước màn hình và khung cha. & \textbf{Tính kế thừa lồng ghép phức tạp}: Việc lồng nhiều cấp phần tử con dùng \codeinline{\%} dễ gây nhầm lẫn kích thước. \\
\hline
\textbf{Fluid Layouts (Bố cục linh hoạt)}: Tạo giao diện ``dòng chảy'' mềm mại, tự lấp đầy không gian có sẵn. & \textbf{Khó kiểm soát kích thước nhỏ}: Không phù hợp cho các chi tiết tuyệt đối như đường viền, icon nhỏ (nên dùng \codeinline{px}). \\
\hline
\textbf{Dễ bảo trì mã nguồn}: Giảm thiểu số lượng đọan \codeinline{@media} query phải viết cho từng màn hình. & \textbf{Phức tạp cho typography}: Sử dụng \codeinline{\%} cho \codeinline{font-size} khó kiểm soát hơn so với \codeinline{rem} hay \codeinline{em}. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{6. Các Trường Hợp Sử Dụng Phổ Biến Của Đơn Vị \% trong Thực Tế:}

\begin{prettyTableBox}[Bảng Tổng Hợp 4 Ứng Dụng Thực Tế Phổ Biến Của Đơn Vị \%]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash \bfseries\color{primaryBlue}}K{3.8cm}|>{\ttfamily\color{accentOrange}\bfseries}K{3.6cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Trường Hợp Sử Dụng} & \thd{Thuộc Tính CSS} & \thd{Mục Đích \& Hiệu Quả Giao Diện} \\
\hline
Bố cục khung Layout \& Cột & width, margin & Tạo khung container chiếm $80\%$ -- $90\%$ màn hình và chia cột $48\%$ co giãn. \\
\hline
Tỷ lệ khoảng đệm Card & padding & Giữ khoảng đệm \codeinline{padding: 5\%} co giãn hài hòa theo độ rộng khung. \\
\hline
Hình ảnh Responsive & max-width: 100\% & Đảm bảo ảnh không bao giờ tràn ra ngoài khung chứa, tự giữ tỷ lệ hình ảnh. \\
\hline
Tăng giảm cỡ chữ tương quan & font-size: 110\% & Tăng kích thước chữ lớn hơn $10\%$ so với phông chữ của phần tử cha. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{codeWindow}[Mã Nguồn Mẫu Ứng Dụng Thực Tế Đơn Vị \% Tạo Layout \& Ảnh Responsive]
\begin{lstlisting}[language=HTML5]
<style>
/* 1. Thiet lap khung layout trung tam: */
.container {
...
/* 2. Chia cot co gian: */
.column {
    width: 48%;       /* Chiem 48% chieu rong cha */
    float: left;
    margin-right: 2%; /* Khoang cach 2% giua cac cot */
}

/* 3. Padding co gian theo ty le: */
.card {
    padding: 5%;      /* Padding 5% dua tren width cua cha */
}
...
/* 5. Font size tuong quan: */
p.highlight {
    font-size: 110%;  /* Lon hon 10% so voi font-size phan tu cha */
}
</style>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng Trực Quan Kết Quả Layout Chia Cột \& Ảnh Responsive (%)]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!25, arc=6pt, boxrule=0.8pt, top=10pt, bottom=10pt, left=12pt, right=12pt]
  \textcolor{primaryBlue}{\bfseries Khung Container (Width 80\% màn hình, căn giữa margin: 0 auto)}\par\vspace{6pt}
  \begin{tcolorbox}[colback=blue!8, colframe=blue!60!black, arc=4pt, boxrule=1pt, top=6pt, bottom=6pt, left=8pt, right=8pt, width=0.48\linewidth, nobeforeafter]
    \textcolor{primaryBlue}{\bfseries Cột 1 (Width 48\%)}\par
    \textcolor{darkText}{\small Thẻ Card Padding 5\% co giãn.}
  \end{tcolorbox}\hfill
  \begin{tcolorbox}[colback=blue!8, colframe=blue!60!black, arc=4pt, boxrule=1pt, top=6pt, bottom=6pt, left=8pt, right=8pt, width=0.48\linewidth, nobeforeafter]
    \textcolor{primaryBlue}{\bfseries Cột 2 (Width 48\%)}\par
    \textcolor{darkText}{\small Ảnh \codeinline{max-width: 100\%} không tràn khung.}
  \end{tcolorbox}
\end{tcolorbox}
\end{browserWindow}

\begin{noteBox}[Kết Luận Thực Tế Về Đơn Vị Phần Trăm (\%)]
Đơn vị \codeinline{\%} trong CSS là một công cụ thiết yếu để xây dựng các giao diện web linh hoạt và đáp ứng. Hiểu rõ cách nó tính toán dựa trên các thuộc tính khác nhau của phần tử cha hoặc chính nó là chìa khóa để thiết kế bố cục web chuyên nghiệp.
\end{noteBox}
