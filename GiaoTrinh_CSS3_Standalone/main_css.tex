...
\begin{browserWindow}[Kết Quả Hiển Thị: Mô Phỏng Trực Quan Thang Độ Đậm 9 Mức (100 - 900)]
\begin{tcolorbox}[arc=4pt, colback=white, colframe=primaryBlue!30, boxrule=0.8pt, top=10pt, bottom=10pt, left=12pt, right=12pt]
  {\sffamily\bfseries\color{primaryBlue} Minh họa phân cấp độ đậm trực quan 9 mức (Sắc độ \& Nét chữ):}\par\vspace{8pt}
  \begin{tabular}{@{}l l@{}}
    {\sffamily\fontsize{12}{15}\selectfont\color{gray!50} Chữ rất mỏng (Thin 100)} & {\sffamily\small\color{gray!70} \codeinline{font-weight: 100;}} \\ [4pt]
    {\sffamily\fontsize{12}{15}\selectfont\color{gray!70} Chữ cực nhẹ (Extra Light 200)} & {\sffamily\small\color{gray!70} \codeinline{font-weight: 200;}} \\ [4pt]
    {\sffamily\fontsize{12}{15}\selectfont\color{gray!88} Chữ mỏng (Light 300)} & {\sffamily\small\color{gray!70} \codeinline{font-weight: 300;}} \\ [4pt]
    {\sffamily\fontsize{12}{15}\selectfont\color{darkText} Chữ bình thường (Normal 400)} & {\sffamily\small\color{gray!70} \codeinline{font-weight: normal; (400)}} \\ [4pt]
    {\sffamily\fontsize{12}{15}\selectfont\color{black}\special{pdf:literal q 2 Tr 0.2 w}Chữ trung bình (Medium 500)\special{pdf:literal Q}} & {\sffamily\small\color{gray!70} \codeinline{font-weight: 500;}} \\ [4pt]
    {\sffamily\fontsize{12}{15}\selectfont\color{black}\special{pdf:literal q 2 Tr 0.4 w}Chữ đậm vừa (Semi Bold 600)\special{pdf:literal Q}} & {\sffamily\small\color{gray!70} \codeinline{font-weight: 600;}} \\ [4pt]
    {\sffamily\bfseries\fontsize{12}{15}\selectfont\color{black} Chữ đậm (Bold 700)} & {\sffamily\small\color{gray!70} \codeinline{font-weight: bold; (700)}} \\ [4pt]
    {\sffamily\bfseries\fontsize{12}{15}\selectfont\color{black}\special{pdf:literal q 2 Tr 0.6 w}Chữ rất đậm (Extra Bold 800)\special{pdf:literal Q}} & {\sffamily\small\color{gray!70} \codeinline{font-weight: 800;}} \\ [4pt]
    {\sffamily\bfseries\fontsize{12}{15}\selectfont\color{black}\special{pdf:literal q 2 Tr 0.9 w}Chữ siêu đậm (Black 900)\special{pdf:literal Q}} & {\sffamily\small\color{gray!70} \codeinline{font-weight: 900;}} \\
  \end{tabular}\par\vspace{8pt}
  {\sffamily\small\color{gray!80} \textbf{Mẹo thực hành}: Sử dụng các phông chữ đa trọng lượng như \codeinline{Roboto}, \codeinline{Inter}, \codeinline{Poppins} hoặc \codeinline{Open Sans} từ Google Fonts để trình duyệt hiển thị nét chữ siêu mỏng và siêu đậm chuẩn xác nhất.}
\end{tcolorbox}
\end{browserWindow}
...
\subsection*{VII. Chuyên Đề Xử Lý Sự Cố \& Tình Huống Thực Tế Với font-weight}

Trong thực tế phát triển Web, bạn sẽ gặp một số tình huống sự cố liên quan đến \codeinline{font-weight}. Dưới đây là phân tích và giải pháp xử lý triệt để:

\begin{prettyTableBox}[Bảng Phân Tích \& Xử Lý Các Sự Cố Thực Tế Khi Dùng font-weight]
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash\bfseries\color{red!80!black}}p{3.6cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash\bfseries\color{green!60!black}}p{4.0cm}|}
\hline
\rowcolor{primaryBlue} \thd{\sffamily Tình huống sự cố} & \thd{\sffamily Nguyên nhân kỹ thuật cốt lõi} & \thd{\sffamily Giải pháp khắc phục triệt để} \\
\hline
Font chữ không hỗ trợ nhiều độ đậm & Các font hệ thống cũ (Arial, Times New Roman) chỉ có 2 độ đậm mặc định (400 và 700). Đặt 200 hay 900 đều giống 400/700. & Chuyển sang dùng font đa trọng lượng hiện đại: \codeinline{Roboto}, \codeinline{Inter}, \codeinline{Poppins}, \codeinline{Open Sans}. \\
\hline
Trình duyệt tự thay thế font không hỗ trợ & Khi khai báo một mức đậm (VD: 600) mà font không có tệp nhúng, trình duyệt tự động chọn mức gần nhất (700) để thay thế. & Nhúng đầy đủ chuỗi \codeinline{wght} trong URL Google Fonts hoặc tệp \codeinline{@font-face}. \\
\hline
Font quá mỏng hoặc quá dày trên màn hình nhỏ & Trên màn hình độ phân giải thấp hoặc máy mobile nhỏ, các nét chữ quá mỏng (100-300) dễ bị mờ, đứt nét. & Tăng kích thước \codeinline{font-size: 20px;}, chọn font tối ưu (\codeinline{Inter}, \codeinline{Verdana}) và tăng độ tương phản màu sắc. \\
\hline
Cú pháp bị ghi đè CSS (\term{Specificity}) & Thuộc tính \codeinline{font-weight} khai báo nhưng không có hiệu lực do bị quy tắc CSS khác có độ ưu tiên cao hơn ghi đè. & Kiểm tra DevTools (F12) để xem thuộc tính bị gạch ngang. Dùng Selector cụ thể hơn hoặc \codeinline{!important}. \\
\hline
Khoảng cách chữ hoặc line-height làm mờ độ đậm & Khoảng cách ký tự (\codeinline{letter-spacing}) hoặc chiều cao dòng quá lớn/nhỏ khiến mắt khó thấy sự khác biệt độ đậm. & Điều chỉnh chiều cao dòng và khoảng cách chữ chuẩn: \codeinline{letter-spacing: 0.5px; line-height: 1.6;}. \\
\hline
Font có nét chữ ít khác biệt & Một số font (như Times New Roman) được thiết kế nét chữ ít thay đổi giữa các mức 500 và 600. & Chuyển sang phông chữ hiện đại có độ tương phản nét chữ rõ nét như \codeinline{Poppins}, \codeinline{Montserrat}, \codeinline{Lato}. \\
\hline
\end{tabularx}
\end{prettyTableBox}
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item luôn chọn phông chữ hỗ trợ nhiều biến thể trọng lượng (\codeinline{Roboto}, \codeinline{Inter}).
    \item Tăng kích thước chữ lớn hơn (\codeinline{font-size: 24px;}) khi thử nghiệm để dễ quan sát nét chữ.
    \item Thử nghiệm đối soát trên các trình duyệt khác nhau (Chrome, Firefox, Safari).
    \item Sử dụng phím \codeinline{F12} (DevTools) để đảm bảo không bị quy tắc CSS khác ghi đè.
\end{enumerate}
\end{tipBox}
