...
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung màn hình Browser (rộng 9cm, cao 3.2cm)
  \draw[fill=white, draw=gray!40, line width=0.9pt, rounded corners=4pt] (0,0) rectangle (9.0, 3.2);
  \node[anchor=north west, font=\tiny\bfseries\color{gray!70}] at (0.2, 3.05) {Cửa sổ trình duyệt (Tổng chiều cao: 100vh)};
  
  % Khối .box chiếm 50vh nửa trên (cao 1.4cm)
  \draw[fill=teal!70!black, draw=teal!90!black, line width=1pt, rounded corners=3pt] (0.2, 1.4) rectangle (8.8, 2.8);
  \node[anchor=center, font=\small\bfseries\color{white}] at (4.5, 2.1) {Tôi cao 50\% màn hình};
  
  % Khoảng trống 50vh còn lại nửa dưới
  \draw[dashed, draw=gray!30] (0.2, 0.2) rectangle (8.8, 1.25);
  \node[anchor=center, font=\tiny\color{gray!50}] at (4.5, 0.72) {Khoảng trống 50vh còn lại phía dưới};

  % Thước đo 50vh bên trái
  \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.8pt, color=teal!90!black] (-0.2, 1.4) -- (-0.2, 2.8)
    node[midway, left=5pt, font=\scriptsize\bfseries, color=teal!90!black] {$50\text{vh}$};
\end{tikzpicture}
\par\vspace{4pt}
