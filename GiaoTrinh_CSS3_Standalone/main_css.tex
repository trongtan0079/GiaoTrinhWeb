...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    ...
    \item \textbf{Mặc định trình duyệt}:
\begin{lstlisting}[language=CSS3]
html {
  font-size: 16px;
}
...
\end{lstlisting}
    Hoặc để thuận tiện trong việc nhẩm toán, nhiều lập trình viên áp dụng tỷ lệ:
\begin{lstlisting}[language=CSS3]
html {
...
\end{lstlisting}
    \item \textbf{Thiết kế dễ dàng scale toàn bộ website}: Thay đổi kích thước toàn trang linh hoạt chỉ bằng một dòng CSS điều chỉnh phông chữ của thẻ \codeinline{<html>}.
\end{itemize}
