...
% ==============================================================================
\subsection{Tổng Quan \& Phân Loại Các Đơn Vị Trong CSS (CSS Units Classification)}
\label{sec:css_units_classification}
...
CSS Units được chia làm 2 nhóm chính: \textbf{Đơn vị Tuyệt đối (Absolute Units)} và \textbf{Đơn vị Tương đối (Relative Units)}.

\subsubsection{1. Nhóm Đơn Vị Tuyệt Đối (Absolute Units)}
...
% ==============================================================================
\subsection{Chuyên Đề Đơn Vị Pixel (px Masterclass Trong CSS)}
...
% ==============================================================================
\subsection{Chuyên Đề Đơn Vị Phần Trăm (\% Masterclass Trong CSS)}
...
\begin{browserWindow}[Mô Phỏng Trực Quan padding-top 10\% và margin-left 5\% Theo Width Cha]
\sffamily\small
\centering
\begin{tcolorbox}[enhanced, colback=gray!15, colframe=gray!40, width=280pt, arc=6pt, boxrule=1pt, top=8pt, bottom=8pt, left=8pt, right=8pt]
  \textcolor{darkText}{\textbf{Khối Cha (.parent): Width $500\text{px}$}}\par\vspace{5pt}
  \hspace{14pt}\begin{tcolorbox}[colback=orange!10, colframe=orange!80!black, width=220pt, arc=4pt, boxrule=1pt, top=16pt, bottom=8pt, left=8pt, right=8pt]
    \textcolor{orange!90!black}{\bfseries .child (Padding-top $10\% = 50\text{px}$, Margin-left $5\% = 25\text{px}$)}\par
    \tiny Khoảng đệm phía trên và khoảng lề trái đều dựa trên chiều rộng $500\text{px}$ của khối cha.
  \end{tcolorbox}
\end{tcolorbox}
\end{browserWindow}

\paragraph{c) Chuyên Đề Chi Tiết: Thuộc Tính font-size Với Đơn Vị Phần Trăm (\%):}

Tổng hợp chi tiết về việc sử dụng \codeinline{font-size} với đơn vị phần trăm (\codeinline{\%}) trong CSS, đặc biệt phân tích rõ cơ chế hoạt động khi phần tử cha không cài đặt \codeinline{font-size}:

\begin{conceptBox}[1. Cách Hoạt Động Cốt Lõi Của font-size: \%]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item Đơn vị \codeinline{\%} trong \codeinline{font-size} là \textbf{tỉ lệ so với font-size của phần tử cha trực tiếp}.
    \item Nếu phần tử cha không khai báo \codeinline{font-size}, nó sẽ \textbf{tự động kế thừa ngược lên trên}, cho đến khi gặp phần tử có \codeinline{font-size} (thường là thẻ gốc \codeinline{<html>}).
    \item Nếu xuyên suốt cây DOM không có phần tử nào khai báo, thì mặc định \codeinline{font-size} chuẩn của trình duyệt cho \codeinline{<html>} là $\mathbf{16\text{px}}$.
\end{itemize}
\end{conceptBox}

\begin{prettyTableBox}[2. Bảng Ma Trận: Khi Nào font-size: \% Hướng Về Thẻ <html>?]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.8cm}|>{\centering\arraybackslash\bfseries}K{2.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Tình Huống Trong DOM} & \thd{Hướng Về <html>?} & \thd{Giải Thích Cơ Chế Kế Thừa} \\
\hline
Không có phần tử cha nào cài đặt \codeinline{font-size} & \textcolor{green!60!black}{\textbf{[CÓ]}} & Tự động kế thừa thẳng lên thẻ gốc và lấy \codeinline{font-size} mặc định của \codeinline{<html>} ($16\text{px}$). \\
\hline
Phần tử cha gần nhất có cài đặt \codeinline{font-size} & \textcolor{red!80!black}{\textbf{[KHÔNG]}} & Sẽ lấy trực tiếp giá trị của cha gần nhất làm điểm tựa cơ sở để tính \codeinline{\%}. \\
\hline
Nhiều lớp cha lồng nhau, mỗi lớp đều dùng \codeinline{\%} & \textcolor{red!80!black}{\textbf{[KHÔNG]}} & Mỗi lớp sẽ nhân dồn theo từng cấp (Hiệu ứng Cascading Compounding), dễ gây sai lệch kích thước. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{3. Ví Dụ Minh Họa \& Đo Lường Trực Quan Trên Chrome DevTools:}

\paragraph{Trường Hợp 1: Không Có Phần Tử Cha Thiết Lập font-size:}

\begin{codeWindow}[Mã Nguồn: Thẻ Con Dùng font-size: 150\% Khi Cha Không Set Cỡ Chữ]
\begin{lstlisting}[language=HTML5]
<!-- Khong co phan tu cha nao khai bao font-size -->
<div class="child" style="font-size: 150%;">Text</div>
\end{lstlisting}
\end{codeWindow}

\begin{conceptBox}[Công Thức Tính Toán Trường Hợp 1]
Trình duyệt kế thừa cỡ chữ gốc của \codeinline{<html>} ($16\text{px}$):
\[
\text{font-size (.child)} = 150\% \times 16\text{px} = \left(\frac{150}{100}\right) \times 16\text{px} = \mathbf{24\text{px}}
\]
\end{conceptBox}

\begin{browserWindow}[Mô Phỏng Chrome DevTools: font-size = 24px (Khi Cha Không Cài Đặt)]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Highlighted DOM element (Text) with DevTools blue overlay
  \draw[fill=blue!30!cyan!35, draw=blue!40, line width=0.8pt] (0, 1.3) rectangle (7.5, 2.5);
  \node[anchor=west, font=\fontsize{22pt}{26pt}\selectfont\rmfamily, color=primaryBlue!80!black] at (0.3, 1.9) {Text};
  
  % DevTools Tooltip HUD Callout box
  \draw[fill=gray!90!black, draw=gray!70!black, line width=0.8pt, rounded corners=4pt] (0.2, -0.6) rectangle (5.8, 1.0);
  
  % DevTools Header: div.child + dimensions
  \draw[fill=white, rounded corners=3pt] (0.3, 0.45) rectangle (5.7, 0.95);
  \node[anchor=west, font=\footnotesize\bfseries, color=purple!80!black] at (0.45, 0.7) {div.child};
  \node[anchor=east, font=\scriptsize\color{darkText}] at (5.5, 0.7) {420.8 $\times$ 43.2};
  
  % DevTools Properties
  \node[anchor=west, font=\scriptsize\ttfamily, color=cyan!70!white] at (0.45, 0.2) {\textbf{>} display};
  \node[anchor=east, font=\scriptsize\ttfamily, color=orange!80!white] at (5.5, 0.2) {block};
  
  \node[anchor=west, font=\scriptsize\ttfamily, color=cyan!70!white] at (0.45, -0.1) {\textbf{>} font-size};
  \node[anchor=east, font=\scriptsize\ttfamily\bfseries, color=green!70!white] at (5.5, -0.1) {24px};
  
  \node[anchor=west, font=\scriptsize\ttfamily\itshape, color=cyan!40!white] at (0.45, -0.4) {height};
  \node[anchor=east, font=\scriptsize\ttfamily\itshape, color=gray!40] at (5.5, -0.4) {27.2px};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả DevTools Inspector: Kích thước chữ thực tế đạt chuẩn $\mathbf{24\text{px}}$ do lấy mốc $16\text{px}$ của \codeinline{<html>}).}
\end{browserWindow}

\paragraph{Trường Hợp 2: Phần Tử Cha Có Thiết Lập font-size: 20px:}

\begin{codeWindow}[Mã Nguồn: Thẻ Con Dùng font-size: 150\% Khi Cha Có font-size: 20px]
\begin{lstlisting}[language=HTML5]
...
\begin{conceptBox}[Công Thức Tính Toán Trường Hợp 2]
Trình duyệt lấy trực tiếp cỡ chữ của khối cha \codeinline{.parent} ($20\text{px}$) làm điểm tựa:
\[
\text{font-size (.child)} = 150\% \times 20\text{px} = \left(\frac{150}{100}\right) \times 20\text{px} = \mathbf{30\text{px}}
\]
\end{conceptBox}

\begin{browserWindow}[Mô Phỏng Chrome DevTools: font-size = 30px (Khi Cha Có font-size: 20px)]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Highlighted DOM element (Text) with DevTools blue overlay
  \draw[fill=blue!30!cyan!35, draw=blue!40, line width=0.8pt] (0, 1.3) rectangle (7.5, 2.5);
  \node[anchor=west, font=\fontsize{26pt}{30pt}\selectfont\rmfamily, color=primaryBlue!80!black] at (0.3, 1.9) {Text};
  ...
  \draw[fill=white, rounded corners=3pt] (0.3, 0.45) rectangle (5.7, 0.95);
  \node[anchor=west, font=\footnotesize\bfseries, color=purple!80!black] at (0.45, 0.7) {div.child};
  \node[anchor=east, font=\scriptsize\color{darkText}] at (5.5, 0.7) {404.8 $\times$ 34.4};
  ...
  \node[anchor=west, font=\scriptsize\ttfamily, color=cyan!70!white] at (0.45, -0.1) {\textbf{>} font-size};
  \node[anchor=east, font=\scriptsize\ttfamily\bfseries, color=green!70!white] at (5.5, -0.1) {30px};
  
  \node[anchor=west, font=\scriptsize\ttfamily\itshape, color=cyan!40!white] at (0.45, -0.4) {height};
  \node[anchor=east, font=\scriptsize\ttfamily\itshape, color=gray!40] at (5.5, -0.4) {34.4px};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả DevTools Inspector: Kích thước chữ thực tế đạt chuẩn $\mathbf{30\text{px}}$ do lấy mốc $20\text{px}$ của \codeinline{.parent}).}
\end{browserWindow}

\paragraph{4. Bảng Khuyến Nghị Lựa Chọn Đơn Vị Typography (Best Practices):}

\begin{prettyTableBox}[Bảng Hướng Dẫn Chọn Đơn Vị Kích Thước Chữ Trong Thực Tế]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{4.8cm}|>{\ttfamily\color{accentOrange}\bfseries}K{2.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Mục Đích Thiết Kế Giao Diện} & \thd{Nên Dùng Đơn Vị} & \thd{Lý Do \& Lợi Ích Kỹ Thuật} \\
\hline
Kế thừa linh hoạt theo cha gần nhất & \% & Cỡ chữ tự co giãn theo tỷ lệ của khối bao bọc. \\
\hline
Đồng bộ với kích thước gốc toàn trang & rem \textmd{(root em)} & Tránh hoàn toàn lỗi nhân dồn nhiều cấp, dễ quản lý trên toàn bộ hệ thống. \\
\hline
Tương đối theo chính phần tử con & em & Rất thích hợp cho \codeinline{padding}, \codeinline{margin}, bo góc của Component co giãn. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{5. Mẹo Cài Đặt Nhất Quán (Consistent Typography Pattern):}

\begin{codeWindow}[Mã Nguồn Mẫu Thiết Lập Typography Nhất Quán]
\begin{lstlisting}[language=CSS3]
/* 1. Thiet lap font-size goc toan trang: */
html {
  font-size: 16px; /* Goc chung toan bo website */
}

/* 2. Dinh kieu cac the noi dung: */
p {
  font-size: 150%;  /* = 24px neu phan tu cha truc tiep la html */
  /* Hoac su dung rem de dam bao tuyet doi: */
  font-size: 1.5rem; /* Luon luon = 24px bat ke phan tu cha la gi */
}
\end{lstlisting}
\end{codeWindow}
...
\begin{browserWindow}[Mô Phỏng Trực Quan line-height: 150\% (Chiều Cao Dòng 27px Với Cỡ Chữ 18px)]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=codeBg, colframe=primaryBlue!30, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=12pt, right=12pt]
  {\fontsize{13pt}{19.5pt}\selectfont \color{darkText}
  Dòng thứ nhất: Đoạn văn bản có phông chữ $18\text{px}$ và chiều cao dòng $150\%$ ($27\text{px}$).\par
...
\begin{browserWindow}[Mô Phỏng Trực Quan transform: translateX(50\%) Dịch Sang Phải 50px]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Vị trí gốc ban đầu (hộp nét đứt xám, rộng 2.5cm ~ 100px)
  \draw[dashed, gray!60, line width=1pt, fill=gray!8, rounded corners=3pt] (0.5, 0.4) rectangle (3.0, 2.0);
  \node[font=\scriptsize\bfseries\color{gray!70}, align=center] at (1.1, 1.2) {Vị trí\\gốc\\(0px)};
  
  % Vị trí mới sau translateX(50%) (dịch sang phải 1.25cm ~ 50px)
  \draw[primaryBlue!90!black, line width=1.2pt, fill=primaryBlue!18, rounded corners=3pt] (1.75, 0.4) rectangle (4.25, 2.0);
  \node[font=\small\bfseries\color{primaryBlue}, align=center] at (3.5, 1.2) {translateX(50\%)\\\scriptsize\bfseries (+50px)};

  % Mũi tên thể hiện độ dịch chuyển ở trên đỉnh
  \draw[{Stealth[length=4pt]}-{Stealth[length=4pt]}, orange!90!black, line width=1.1pt] (0.5, 2.3) -- (1.75, 2.3)
    node[midway, above=2pt, font=\scriptsize\bfseries\color{orange!90!black}] {Dịch sang phải +50px (50\%)};

  % Thước đo chiều rộng gốc bên dưới
  \draw[decorate, decoration={brace, amplitude=4pt}, color=gray!70, line width=0.8pt] (3.0, 0.2) -- (0.5, 0.2)
    node[midway, below=4pt, font=\tiny\bfseries\color{gray!70}] {Chiều rộng bản thân = 100px};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: Phần tử tự dịch sang phải một khoảng bằng đúng $50\%$ chiều rộng của chính nó ($50\text{px}$)).}
\end{browserWindow}
...
% ==============================================================================
% CHUYÊN ĐỀ LƯU Ý THỰC TẾ VỀ THUỘC TÍNH HEIGHT (%) TRONG CSS
% ==============================================================================
\paragraph{7. Chuyên Đề Lưu Ý Thực Tế: Cơ Chế Hoạt Động Của height Khi Dùng Đơn Vị \% (Vấn Đề HTML, Body \& Khối Con)}
\label{sec:css_percentage_height_masterclass}

Một trong những cạm bẫy kinh điển và thường xuyên gây bối rối nhất cho các lập trình viên Frontend là: \textbf{Nếu không cài đặt chiều cao (\codeinline{height}) cho phần tử cha, nhưng lại khai báo \codeinline{height: \%} cho phần tử con, thì chiều cao phần trăm sẽ KHÔNG HOẠT ĐỘNG như kỳ vọng.}

\paragraph{a) Vì Sao height: \% Không Hoạt Động Khi Cha Không Có Chiều Cao?}

\begin{warnBox}[Nguyên Nhân Cốt Lõi Khiến height: \% Bị Vô Hiệu Hóa]
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item Trong CSS, giá trị \codeinline{height: \%} của phần tử con bắt buộc phải dựa vào \textbf{chiều cao đã được xác định rõ ràng (\term{Explicit Height})} của phần tử cha.
    \item Nếu phần tử cha không có \codeinline{height} cụ thể, trình duyệt sẽ không có cơ sở toán học để tính toán tỷ lệ phần trăm. Kết quả là phần tử con sẽ \textbf{co lại thành $0$ hoặc chỉ vừa khít với nội dung bên trong nó}.
\end{itemize}
\end{warnBox}

\paragraph{b) Ví Dụ Đối Chiếu Trực Quan: Không Hiệu Quả vs Hiệu Quả}

\begin{codeWindow}[Trường Hợp 1: KHÔNG HIỆU QUẢ -- Phần Tử Cha Không Khai Báo height]
\begin{lstlisting}[language=HTML5]
<style>
.parent {
  background: #e0e0e0;
  /* KHONG co height xac dinh */
}
.child {
  height: 50%;   /* KHONG hoat dong dung! */
  background: #f48fb1;
}
</style>

<div class="parent">
  <div class="child">Noi dung Hello</div>
</div>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng: Cha Không Khai Báo height]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung cha .parent (chiều rộng 7.2cm, chiều cao 1.8cm)
  \draw[fill=gray!15, draw=gray!50, line width=1pt, rounded corners=4pt] (0,0) rectangle (7.2, 1.8);
  \node[anchor=north west, font=\scriptsize\bfseries, color=darkText] at (0.2, 1.65) {.parent \textmd{(Không set height $\rightarrow$ co theo chữ)}};
  
  % Khối con .child (bị co theo chữ)
  \draw[fill=pink!60, draw=pink!90!black, line width=1pt, rounded corners=3pt] (0.2, 0.2) rectangle (7.0, 1.05);
  \node[anchor=center, font=\scriptsize\bfseries, color=purple!90!black] at (3.6, 0.62) {.child \{ height: 50\%; \} $\rightarrow$ \textcolor{red!80!black}{\textbf{Bị co thành 26px}}};

  % Thước đo chiều cao bên trái
  \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.8pt, color=red!80!black] (-0.3, 0) -- (-0.3, 1.8)
    node[midway, left=6pt, font=\scriptsize\bfseries, color=red!80!black, align=right] {Chiều cao cha\\bằng chữ\\($\approx 35\text{px}$)};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{red!80!black}{\scriptsize ($\rightarrow$ \textbf{Thất bại}: .child không thể tính ra $50\%$ vì .parent không có kích thước cụ thể để làm mốc).}
\end{browserWindow}

\begin{codeWindow}[Trường Hợp 2: HOẠT ĐỘNG HOÀN HẢO -- Phần Tử Cha Có height Xác Định]
\begin{lstlisting}[language=HTML5]
<style>
.parent {
  height: 400px; /* Cha co chieu cao ro rang: 400px */
  background: #e0e0e0;
}
.child {
  height: 50%;   /* Hoat dong hoan hao: 50% cua 400px = 200px */
  background: #f48fb1;
}
</style>
...
\begin{browserWindow}[Mô Phỏng: Cha Có height: 400px Chuẩn Mực]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung cha .parent (tổng chiều cao 3.8cm, rộng 7.2cm)
  \draw[fill=gray!15, draw=gray!50, line width=1pt, rounded corners=4pt] (0,0) rectangle (7.2, 3.8);
  \node[anchor=north west, font=\scriptsize\bfseries, color=darkText] at (0.2, 3.65) {.parent \{ height: 400px; background: \#e0e0e0; \}};
  
  % Khối con .child (chiếm đúng 50% = 200px nửa trên, cao 1.4cm)
  \draw[fill=pink!65, draw=purple!80!black, line width=1.1pt, rounded corners=3pt] (0.2, 1.75) rectangle (7.0, 3.15);
  \node[anchor=center, font=\scriptsize\bfseries, color=purple!90!black, align=center] at (3.6, 2.45) {\textcolor{green!60!black}{\textbf{[ĐẠT]}} .child \{ height: 50\%; \} $= \mathbf{200\text{px}}$\\[2pt]\textcolor{darkText}{\tiny (Chiếm trọn vẹn $50\%$ nửa trên)}};
  
  % Vùng trống 50% còn lại của cha
  \draw[dashed, draw=gray!40, fill=white!60, rounded corners=3pt] (0.2, 0.2) rectangle (7.0, 1.55);
  \node[anchor=center, font=\scriptsize\color{codeComment}] at (3.6, 0.87) {Vùng trống $50\%$ còn lại của cha ($200\text{px}$)};

  % Thước đo tổng thể bên trái (400px)
  \draw[decorate, decoration={brace, amplitude=5pt}, line width=0.9pt, color=primaryBlue] (-0.3, 0) -- (-0.3, 3.8)
    node[midway, left=7pt, font=\scriptsize\bfseries, color=primaryBlue, align=right] {Chiều cao cha\\$400\text{px}$ ($100\%$)};

  % Thước đo con bên phải (200px)
  \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.8pt, color=purple!80!black] (7.5, 3.15) -- (7.5, 1.75)
    node[midway, right=6pt, font=\scriptsize\bfseries, color=purple!90!black, align=left] {Chiều cao con\\$200\text{px}$ ($50\%$)};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ \textbf{Thành công}: Trình duyệt lấy $50\% \times 400\text{px} = 200\text{px}$ để gán chính xác cho khối con).}
\end{browserWindow}

\paragraph{c) Bảng Hướng Dẫn Cách Xử Lý Chiều Cao Linh Hoạt Trong Thực Tế:}

\begin{prettyTableBox}[Bảng Giải Pháp Xử Lý Khi Cần Chiều Cao Tương Đối]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Tình Huống Giao Diện} & \thd{Cách Xử Lý Chuẩn Mực} \\
\hline
Cần dùng \codeinline{\%} cho \codeinline{height} & Đặt chiều cao rõ ràng cho phần tử cha (bằng \codeinline{px}, \codeinline{vh}, v.v.). \\
\hline
Không muốn dùng \codeinline{px} cố định & Dùng \codeinline{min-height}, mô hình \textbf{Flexbox}, \textbf{CSS Grid}, hoặc đơn vị Viewport (\codeinline{vh}, ví dụ: \codeinline{height: 50vh}). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{tipBox}[Tóm Tắt 3 Phương Án Thay Thế Khi Không Thể Dùng height: \%]
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Sử dụng Flexbox hoặc Grid}: Tự động kéo dãn phần tử con theo chiều cao cột cha bằng thuộc tính \codeinline{align-items: stretch}.
    \item \textbf{Sử dụng đơn vị Viewport (\codeinline{vh})}: Ví dụ \codeinline{height: 50vh;} (bằng đúng $50\%$ chiều cao màn hình hiển thị), hoàn toàn độc lập với phần tử cha.
    \item \textbf{Sử dụng \codeinline{min-height}}: Kết hợp co giãn tự nhiên theo nội dung mà vẫn đảm bảo độ cao tối thiểu cho khung giao diện.
\end{enumerate}
\end{tipBox}

\paragraph{d) Vấn Đề Kinh Điển: Dùng height: 100\% Với Thẻ <body> Và <html>}

Khi bạn muốn một khối con chiếm trọn vẹn $100\%$ chiều cao màn hình trình duyệt bằng đơn vị phần trăm, việc chỉ thiết lập \codeinline{height: 100\%;} cho thẻ \codeinline{<body>} hoặc khối con là \textbf{hoàn toàn không đủ}:

\begin{codeWindow}[Lỗi Thường Gặp: Chỉ Đặt height: 100\% Cho Khối Con Hoặc Body]
\begin{lstlisting}[language=HTML5]
<!DOCTYPE html>
<html>
<head>
  <style>
    body {
      /* KHONG co height ro rang cho html -> body khong co co so chieu cao */
      background: #e0e0e0;
    }
    .child {
      height: 100%;  /* KHONG co tac dung chiem tron man hinh! */
      background: #f48fb1;
    }
  </style>
</head>
<body>
  <div class="child">Child khong the chiem 100% chieu cao man hinh</div>
</body>
</html>
\end{lstlisting}
\end{codeWindow}

\begin{conceptBox}[Giải Thích Cơ Chế Chuỗi Kế Thừa (Chain of Inheritance)]
Khối \codeinline{.child} không chiếm $100\%$ chiều cao màn hình vì \codeinline{<body>} chưa có chiều cao xác định, mà bản thân \codeinline{<body>} lại phụ thuộc vào thẻ gốc \codeinline{<html>}.
\end{conceptBox}

\begin{codeWindow}[Cách Làm Đúng Chuẩn: Thiết Lập height: 100\% Đồng Thời Cho Cả html Và body]
\begin{lstlisting}[language=HTML5]
<!DOCTYPE html>
<html>
<head>
  <style>
    /* 1. Dat height: 100% cho ca html va body: */
    html, body {
      height: 100%;   /* Bat buoc phai dat cho ca hai the */
      margin: 0;      /* Xoa margin mac dinh 8px cua trinh duyet */
    }

    /* 2. Bay gio khoi con co the dung height: 100% hoan hao: */
    .child {
      height: 100%;   /* Chiem dung 100% chieu cao body (toan bo viewport) */
      background: #f48fb1;
    }
  </style>
</head>
<body>
  <div class="child">Chiem toan bo chieu cao man hinh</div>
</body>
</html>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[So Sánh: height: 100\% Khi Chưa vs Đã Khai Báo Cho html]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Cửa sổ trái (Sai - Chưa set html)
  \draw[fill=white, draw=red!40, line width=0.9pt, rounded corners=4pt] (0,0) rectangle (4.6, 3.5);
  \node[anchor=north, font=\scriptsize\bfseries, color=red!80!black] at (2.3, 3.35) {[CHƯA KHAI BÁO <html>]};
  \draw[fill=pink!60, draw=red!70!black, line width=0.8pt, rounded corners=2pt] (0.25, 2.1) rectangle (4.35, 2.9);
  \node[font=\tiny\bfseries\color{purple!90!black}] at (2.3, 2.5) {.child (Bị co rút $\approx 30\text{px}$)};
  \draw[dashed, draw=gray!30] (0.25, 0.2) rectangle (4.35, 1.9);
  \node[font=\tiny\color{gray!60}, align=center] at (2.3, 1.05) {Khoảng trắng thừa\\(body không cao $100\%$)};

  % Cửa sổ phải (Đúng - Đã set html, body 100%)
  \draw[fill=white, draw=primaryBlue!40, line width=0.9pt, rounded corners=4pt] (5.2,0) rectangle (9.8, 3.5);
  \node[anchor=north, font=\scriptsize\bfseries, color=primaryBlue] at (7.5, 3.35) {[ĐÃ KHAI BÁO html, body 100\%]};
  \draw[fill=pink!70, draw=purple!80!black, line width=1pt, rounded corners=2pt] (5.45, 0.2) rectangle (9.55, 2.9);
  \node[font=\scriptsize\bfseries\color{purple!90!black}, align=center] at (7.5, 1.55) {.child \{ height: 100\%; \}\\\textbf{Phủ kín $100\%$ màn hình!}};
\end{tikzpicture}
\end{browserWindow}

\paragraph{e) Bảng Ma Trận So Sánh Hành Vi Của Thẻ <html> Và <body> Trong CSS:}

\begin{prettyTableBox}[Bảng Đối Chiếu Quy Tắc Chiều Cao Của Thẻ html Và body]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{2.4cm}|Z|>{\sffamily\small}Z|>{\sffamily\small}Z|}
\hline
\rowcolor{primaryBlue} \thd{Thẻ} & \thd{Hành Vi Mặc Định} & \thd{Khi Dùng height: 100\%} & \thd{Khi Dùng height: 100vh} \\
\hline
html & Chiều cao tự giãn theo nội dung bên trong, không có giới hạn cứng. & Cần thiết để thẻ \codeinline{body} có thể sử dụng \codeinline{height: 100\%}. & Không bắt buộc áp dụng, vì \codeinline{vh} dựa trực tiếp trên Viewport. \\
\hline
body & Tự giãn theo nội dung, không tự động cao bằng toàn bộ màn hình. & Chiều cao bằng chiều cao của \codeinline{<html>} (chỉ khi \codeinline{<html>} có \codeinline{height: 100\%}). & Chiều cao bằng đúng chiều cao Viewport, hoàn toàn độc lập với thẻ \codeinline{<html>}. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{prettyTableBox}[Bảng Tóm Tắt Quy Trình 3 Bước Khi Dùng \% Chiều Cao Với Thẻ body]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.2cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thành Phần} & \thd{Hành Động Bắt Buộc} \\
\hline
Thẻ \codeinline{<html>} & Khai báo \codeinline{height: 100\%;} \\
\hline
Thẻ \codeinline{<body>} & Khai báo \codeinline{height: 100\%; margin: 0;} \\
\hline
Khối con (\codeinline{.child}) & Bây giờ có thể sử dụng \codeinline{height: \%} hoặc \codeinline{height: 100\%;} một cách chuẩn xác. \\
\hline
\end{tabularx}
\end{prettyTableBox}

% ==============================================================================
% CHUYÊN ĐỀ SÂU VỀ ĐƠN VỊ EM & REM (TYPOGRAPHY & SCALABLE UI MASTERCLASS)
% ==============================================================================
\subsection{Chuyên Đề Đơn Vị em (em Masterclass Trong CSS)}
\label{sec:css_em_masterclass}

Đơn vị \codeinline{em} là một trong những đơn vị đo lường tương đối lâu đời và quan trọng bậc nhất trong CSS, bắt nguồn từ thuật ngữ ngành in ấn (chiều rộng của ký tự chữ ``M'' hoa). Trong CSS hiện đại, \codeinline{em} hoạt động linh hoạt theo cơ chế kế thừa ngữ cảnh phông chữ.

\paragraph{1. Khái Niệm Cốt Lõi \& Bản Chất Đơn Vị em:}
Đơn vị \codeinline{em} là đơn vị tương đối, phụ thuộc trực tiếp vào \codeinline{font-size} của phần tử cha gần nhất (hoặc kích thước phông chữ của chính nó nếu khai báo trực tiếp cho các thuộc tính kích thước khác).

\begin{conceptBox}[Bản Chất Quy Đổi Của Đơn Vị em]
\[
\mathbf{1\text{em} = 100\% \text{ font-size của phần tử hiện tại hoặc cha trực tiếp}}
\]
\vspace{-4pt}
\textbf{Công thức quy đổi giá trị thực tế:}
\[
\text{Giá Trị Thực Tế (px)} = \text{Giá Trị em} \times \text{font-size của phần tử cha}
\]
\end{conceptBox}

\paragraph{2. Ví Dụ Đơn Giản: Kích Thước Chữ (font-size) Với Đơn Vị em:}
Xét ví dụ phần tử cha chứa cỡ chữ $20\text{px}$ và thẻ con được gán kích thước $2\text{em}$:

\begin{codeWindow}[Ví Dụ Đơn Giản: Xác Định font-size Bằng Đơn Vị em]
\begin{lstlisting}[language=HTML5]
<div class="parent">
  <p class="child">Chữ này 2em</p>
</div>
...
<style>
  ...
  .child {
    font-size: 2em; /* = 2 x 20px = 40px */
  }
</style>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng Kết Quả Render: .child Có font-size Là 40px]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!20, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=12pt, right=12pt]
  {\fontsize{13pt}{16pt}\selectfont \textcolor{darkText}{Phần tử cha (\codeinline{.parent}): Cỡ chữ chuẩn $20\text{px}$}\par}
  \vspace{6pt}
  {\fontsize{20pt}{24pt}\selectfont \textcolor{primaryBlue}{\textbf{Chữ này 2em (\codeinline{.child}): Cỡ chữ hiển thị thực tế là $40\text{px}$ ($2 \times 20\text{px}$)}}\par}
  \vspace{4pt}
  \textcolor{green!60!black}{\textbf{[Kết quả]:}} Phần tử \codeinline{.child} nhận kích thước \codeinline{font-size} là $40\text{px}$.
\end{tcolorbox}
\end{browserWindow}

\paragraph{3. Ví Dụ Rõ Ràng Hơn: Áp Dụng em Cho padding Của Phần Tử:}
Đơn vị \codeinline{em} không chỉ áp dụng cho phông chữ mà còn được sử dụng rộng rãi cho các thuộc tính Box Model như \codeinline{padding}, \codeinline{margin}, \codeinline{width}\dots Trong trường hợp này, $1\text{em}$ được tính dựa theo cỡ chữ của chính nó (hoặc kế thừa từ phần tử cha nếu phần tử con không khai báo \codeinline{font-size}).

\begin{codeWindow}[Ví Dụ Rõ Ràng Hơn: Áp Dụng em Cho Khoảng Đệm padding]
\begin{lstlisting}[language=HTML5]
<div class="outer">
  <div class="inner">Tôi có padding 2em</div>
</div>
...
<style>
  ...
  .inner {
    padding: 2em; /* = 2 x 20px = 40px padding */
...
\begin{browserWindow}[Mô Phỏng Trực Quan: Khối .inner Nhận Khoảng Đệm padding: 2em = 40px]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!20, arc=6pt, boxrule=0.8pt, top=10pt, bottom=10pt, left=12pt, right=12pt]
  \begin{tcolorbox}[enhanced, colback=red!15, colframe=red!45, arc=5pt, boxrule=0.9pt, top=18pt, bottom=18pt, left=18pt, right=18pt]
    \centering \textcolor{red!75!black}{\textbf{Tôi có padding 2em ($40\text{px}$ mọi phía xung quanh)}}
  \end{tcolorbox}
  \vspace{4pt}
  \textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: Phần tử \codeinline{.inner} có khoảng đệm xung quanh là $40\text{px}$ nhờ phép nhân $2 \times 20\text{px}$ từ cỡ chữ cha kế thừa).}
\end{tcolorbox}
\end{browserWindow}

\paragraph{4. Lưu Ý Quan Trọng: 3 Tình Huống \& Hành Vi Khi Sử Dụng em:}
Khi làm việc với đơn vị \codeinline{em}, các lập trình viên bắt buộc phải nắm rõ 3 tình huống hành vi đặc thù sau đây:

\begin{prettyTableBox}[Bảng 3 Tình Huống \& Hành Vi Đặc Thù Của Đơn Vị em]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{4.2cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Tình Huống} & \thd{Hành Vi Cụ Thể} \\
\hline
\textbf{Lồng nhiều cấp} & \codeinline{em} sẽ \textbf{nhân chồng (compounding)} qua từng tầng $\rightarrow$ có thể khiến kích thước phông chữ bị phóng đại hoặc teo nhỏ một cách khó kiểm soát. \\
\hline
\textbf{Áp dụng cho padding, margin, width\dots} & Cũng hoạt động tương tự: Độ dài được tính tỷ lệ theo \codeinline{font-size} của chính phần tử đó. \\
\hline
\textbf{Không đặt font-size cụ thể cho cha} & \codeinline{em} mặc định sẽ dựa vào kích thước phông chữ thiết lập của trình duyệt (thông thường tiêu chuẩn là $16\text{px}$). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{5. So Sánh Nhanh Yếu Tố Tham Chiếu Của 4 Đơn Vị Cốt Lõi:}
Dưới đây là bảng tổng hợp nhanh điểm tựa tham chiếu của 4 đơn vị phổ biến nhất trong CSS:

\begin{prettyTableBox}[Bảng So Sánh Nhanh Điểm Tựa Tham Chiếu: em, rem, \%, px]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\bfseries\color{primaryBlue}}K{2.5cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Đơn Vị} & \thd{Dựa Vào (Điểm Tựa Tham Chiếu)} \\
\hline
em & \codeinline{font-size} của phần tử cha gần nhất (hoặc chính nó đối với thuộc tính Box Model). \\
\hline
rem & \codeinline{font-size} của thẻ gốc \codeinline{<html>} duy nhất trên toàn bộ trang. \\
\hline
\% & Phụ thuộc vào thuộc tính cụ thể (ví dụ: \codeinline{width} theo chiều rộng cha, \codeinline{font-size} theo cỡ chữ cha). \\
\hline
px & Cố định, không bị ảnh hưởng bởi bất kỳ phần tử nào khác. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{6. Quy Tắc Hoạt Động Cốt Lõi Của Đơn Vị em:}
Điểm tham chiếu của đơn vị \codeinline{em} phụ thuộc trực tiếp vào thuộc tính CSS mà nó được áp dụng:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Khi áp dụng cho thuộc tính \codeinline{font-size}}: $1\text{em}$ bằng đúng kích thước \codeinline{font-size} của \textbf{phần tử cha trực tiếp}.
    \item \textbf{Khi áp dụng cho các thuộc tính kích thước khác} (\codeinline{padding}, \codeinline{margin}, \codeinline{width}, \codeinline{border-radius}...): $1\text{em}$ bằng đúng kích thước \codeinline{font-size} của \textbf{chính phần tử đó}.
\end{itemize}

\begin{prettyTableBox}[Bảng Điểm Tham Chiếu Của Đơn Vị em]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{3.6cm}|>{\bfseries\color{darkText}}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thuộc Tính Áp Dụng} & \thd{Điểm Tựa Tham Chiếu} & \thd{Công Thức Tính Toán Độ Dài Thực Tế} \\
\hline
font-size & \codeinline{font-size} của phần tử cha & $\text{Cỡ chữ thực tế} = \text{Giá trị em} \times \text{font-size của cha}$. \\
\hline
padding, margin, width, border-radius & \textbf{\codeinline{font-size} của chính nó} & $\text{Kích thước thực tế} = \text{Giá trị em} \times \text{font-size của chính phần tử}$. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{7. Cạm Bẫy Cần Tránh: Hiệu Ứng Nhân Dồn Kích Thước (Em Compounding Effect):}

Khi bạn lồng ghép nhiều cấp phần tử con liên tiếp và mỗi cấp đều sử dụng \codeinline{font-size} bằng đơn vị \codeinline{em}, kích thước chữ sẽ bị \textbf{nhân dồn qua từng tầng}, dẫn đến việc phần tử càng lồng sâu thì chữ càng phình to hoặc teo nhỏ một cách khó kiểm soát.

\begin{codeWindow}[Minh Họa Hiện Tượng Nhân Dồn Kích Thước Với Danh Sách Menu Lồng Nhau]
\begin{lstlisting}[language=HTML5]
<style>
/* Co chu goc cua danh sach: 16px */
ul.compounding-menu {
  font-size: 16px;
}

/* Moi cap danh sach con deu tang kich thuoc them 1.2em */
ul.compounding-menu li {
  font-size: 1.2em; /* 1.2 * font-size cua ul cha truc tiep */
}
</style>

<ul class="compounding-menu">
  <li>Cap 1 (16px * 1.2 = 19.2px)
    <ul>
      <li>Cap 2 (19.2px * 1.2 = 23.04px)
        <ul>
          <li>Cap 3 (23.04px * 1.2 = 27.65px - Bi phinh to qua muc!)</li>
        </ul>
      </li>
    </ul>
  </li>
...
\begin{conceptBox}[Phân Tích Toán Học Phép Nhân Dồn Đơn Vị em]
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Cấp 1}: $1.2\text{em} \times 16\text{px} = \mathbf{19.2\text{px}}$.
    \item \textbf{Cấp 2}: $1.2\text{em} \times 19.2\text{px} = \mathbf{23.04\text{px}}$.
    \item \textbf{Cấp 3}: $1.2\text{em} \times 23.04\text{px} = \mathbf{27.65\text{px}}$ (gần gấp đôi kích thước phông chữ gốc ban đầu).
\end{itemize}
\end{conceptBox}

\begin{browserWindow}[Mô Phỏng Trực Quan Hiệu Ứng Nhân Dồn Khi Dùng em Cho Danh Sách Lồng]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!25, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=12pt, right=12pt]
  {\fontsize{13pt}{16pt}\selectfont \textcolor{primaryBlue}{\textbf{$\bullet$ Cấp 1 ($19.2\text{px}$): Menu cha tiêu chuẩn}}\par}
  \vspace{3pt}
  \hspace{16pt}{\fontsize{15pt}{19pt}\selectfont \textcolor{accentPurple}{\textbf{$\circ$ Cấp 2 ($23.04\text{px}$): Chữ bắt đầu to hơn rõ rệt}}\par}
  \vspace{3pt}
  \hspace{32pt}{\fontsize{18pt}{22pt}\selectfont \textcolor{red!80!black}{\textbf{-- Cấp 3 ($27.65\text{px}$): Chữ bị phình to ngoài ý muốn!}}\par}
\end{tcolorbox}
\end{browserWindow}

\paragraph{8. Ứng Dụng Thực Tế Tuyệt Vời Của em: Nút Bấm Co Giãn Tự Động (Scalable Buttons):}

Mặc dù \codeinline{em} phức tạp cho \codeinline{font-size}, nhưng nó lại là \textbf{vũ khí tối thượng để thiết kế các thành phần UI có khả năng tự động co giãn tỷ lệ} (\term{Scalable Components}). Khi sử dụng \codeinline{em} cho \codeinline{padding} và \codeinline{border-radius} của nút bấm, khoảng đệm sẽ tự động mở rộng hài hòa khi bạn thay đổi cỡ chữ mà không cần viết thêm bất kỳ dòng CSS nào!

\begin{codeWindow}[Mã Nguồn Nút Bấm Co Giãn Tỷ Lệ Hoàn Hảo Với Đơn Vị em]
\begin{lstlisting}[language=HTML5]
<style>
/* Khung nut bam co ban: Padding va Radius dua theo font-size cua chinh no */
.btn {
  font-family: inherit;
  font-weight: bold;
  padding: 0.5em 1.2em;      /* 0.5em tren/duoi, 1.2em trai/phai */
  border-radius: 0.35em;     /* Bo goc ty le theo co chu */
  background: #0f4c81;
  color: white;
  border: none;
  cursor: pointer;
...
/* Nut nho (Small): */
.btn-sm { font-size: 12px; }  /* Padding tu tinh: 6px 14.4px, Radius: 4.2px */

/* Nut tieu chuan (Medium): */
.btn-md { font-size: 16px; }  /* Padding tu tinh: 8px 19.2px, Radius: 5.6px */

/* Nut lon (Large): */
.btn-lg { font-size: 22px; }  /* Padding tu tinh: 11px 26.4px, Radius: 7.7px */
</style>
...
\begin{browserWindow}[Mô Phỏng Trực Quan Bộ Nút Bấm Tự Co Giãn Tỷ Lệ Theo Cỡ Chữ (Scalable Buttons)]
\sffamily\small
\centering
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!25, arc=6pt, boxrule=0.8pt, top=10pt, bottom=10pt, left=12pt, right=12pt]
  \tcbox[on line, arc=3pt, colback=primaryBlue, colframe=primaryBlue, boxsep=2pt, left=7pt, right=7pt, top=3pt, bottom=3pt]{%
    \color{white}\bfseries\scriptsize Small Button ($12\text{px}$)
  } \hspace{10pt}
  \tcbox[on line, arc=4pt, colback=primaryBlue, colframe=primaryBlue, boxsep=3pt, left=10pt, right=10pt, top=4pt, bottom=4pt]{%
    \color{white}\bfseries\small Medium Button ($16\text{px}$)
  } \hspace{10pt}
  \tcbox[on line, arc=6pt, colback=primaryBlue, colframe=primaryBlue, boxsep=4pt, left=14pt, right=14pt, top=6pt, bottom=6pt]{%
    \color{white}\bfseries\large Large Button ($22\text{px}$)
  }
  \par\vspace{6pt}
  \textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Khoảng cách Padding và Bo góc tự động nở rộng tỷ lệ thuận theo cỡ chữ mà không cần viết lại thuộc tính padding).}
\end{tcolorbox}
\end{browserWindow}

% ------------------------------------------------------------------------------
% PHÂN TÍCH CHUYÊN SÂU 9 TRƯỜNG HỢP HOẠT ĐỘNG CỦA ĐƠN VỊ EM TRONG THỰC TẾ
% ------------------------------------------------------------------------------
\subsubsection{Cách Hoạt Động Của Đơn Vị em Qua 9 Trường Hợp Thực Chiến}

Để làm chủ hoàn toàn đơn vị \codeinline{em} và loại bỏ mọi bối rối khi thiết kế giao diện, chúng ta cùng phân tích chi tiết cách vận hành toán học của \codeinline{em} qua 9 kịch bản điển hình từ cơ bản đến phức tạp:

\paragraph{Trường Hợp 1: Khi Cả Cha Và Con Đều Không Khai Báo font-size:}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Ngữ cảnh}: Khi không khai báo \codeinline{font-size} cho cả phần tử cha và phần tử con, trình duyệt sẽ sử dụng giá trị mặc định của thẻ gốc \codeinline{<html>} (thường là $16\text{px}$).
    \item \textbf{Mặc định trình duyệt}: Thiết lập mặc định của thẻ \codeinline{<html>} là $16\text{px}$.
\end{itemize}

\begin{codeWindow}[Mặc Định Của Trình Duyệt Cho Thẻ Gốc html]
\begin{lstlisting}[language=CSS3]
html {
  font-size: 16px;
}
...
\begin{codeWindow}[Ví Dụ 1.1: Không Khai Báo font-size Cho Bất Kỳ Phần Tử Nào]
\begin{lstlisting}[language=HTML5]
<div class="box">Text</div>

<style>
  .box {
    font-size: 2em; /* 2 x 16px (mac dinh html) = 32px */
  }
</style>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Kết Quả: .box Nhận font-size Là 32px]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!20, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=12pt, right=12pt]
  {\fontsize{20pt}{24pt}\selectfont \textcolor{primaryBlue}{\textbf{Text}}\par}
  \vspace{3pt}
  \textcolor{green!60!black}{\textbf{[Kết quả]:}} Phần tử \codeinline{.box} có \codeinline{font-size} hiển thị thực tế là $\mathbf{32\text{px}}$ ($2 \times 16\text{px}$).
\end{tcolorbox}
\end{browserWindow}

\begin{codeWindow}[Ví Dụ 1.2: Dùng em Trong padding Khi Không Có font-size Khai Báo]
\begin{lstlisting}[language=HTML5]
<div class="box">Noi dung</div>

<style>
  .box {
    padding: 1.5em; /* 1.5 x 16px = 24px padding moi phia */
...
\begin{browserWindow}[Kết Quả: .box Có padding 24px Xung Quanh]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!20, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=12pt, right=12pt]
  \begin{tcolorbox}[enhanced, colback=yellow!40, colframe=black, arc=3pt, boxrule=1pt, top=16pt, bottom=16pt, left=16pt, right=16pt]
    \centering\textbf{\color{black}Nội dung}
  \end{tcolorbox}
  \vspace{3pt}
  \textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: \codeinline{.box} có khoảng đệm $24\text{px}$ xung quanh, dựa vào \codeinline{font-size} mặc định của trình duyệt là $16\text{px}$).}
\end{tcolorbox}
\end{browserWindow}

\begin{prettyTableBox}[Bảng Tổng Kết Điểm Tham Chiếu Của em Trong Trường Hợp 1]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{4.6cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Tình Huống} & \thd{em Tham Chiếu Đến Đâu?} \\
\hline
Không có \codeinline{font-size} cho cha và con & Sẽ dùng \codeinline{font-size} mặc định của thẻ \codeinline{<html>} là $16\text{px}$. \\
\hline
Có \codeinline{font-size} ở cha & \codeinline{em} sẽ nhân trực tiếp với giá trị đó. \\
\hline
Có nhiều cấp \codeinline{em} lồng nhau & Có thể gây nhân chồng, dẫn đến kích thước phóng đại không kiểm soát. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{tipBox}[Gợi Ý Kiểm Soát em \& Kỹ Thuật 62.5\% Kinh Điển]
Nếu muốn \codeinline{em} ổn định, bạn có thể đặt \codeinline{font-size} rõ ràng cho phần tử cha hoặc dùng \codeinline{rem} để tham chiếu về gốc. Để đảm bảo tính nhất quán và dễ nhẩm toán, nhiều lập trình viên CSS thường áp dụng:
\begin{lstlisting}[language=CSS3]
html {
  font-size: 62.5%; /* 62.5% x 16px = 10px -> 1em = 10px rat de tinh toan! */
}
\end{lstlisting}
\end{tipBox}

\paragraph{Trường Hợp 2: Phần Tử Con Có font-size Là px Và Dùng em Cho Thuộc Tính Box Model (width, height, padding, margin):}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Quy tắc cơ bản}: Nếu \codeinline{font-size} của phần tử được đặt bằng \codeinline{px}, thì các thuộc tính khác (\codeinline{width}, \codeinline{height}, \codeinline{padding}, \codeinline{margin}\dots) viết bằng \codeinline{em} sẽ \textbf{dựa trên chính font-size của phần tử đó} (hoàn toàn độc lập với phần tử cha).
    \item \textbf{Cách tính}:
    \[
    \text{Giá trị tính bằng px} = \text{Giá trị em} \times \text{font-size của chính phần tử}
    \]
\end{itemize}

\begin{codeWindow}[Ví Dụ 2: Xác Định Box Model Bằng em Theo font-size Của Chính Nó]
\begin{lstlisting}[language=HTML5]
<div class="box">Noi dung</div>

<style>
  .box {
    font-size: 20px;      /* Co so: 1em = 20px */
    width: 10em;          /* 10 x 20px = 200px */
    height: 5em;          /* 5 x 20px = 100px */
    padding: 1em;         /* 1 x 20px = 20px */
...
\begin{prettyTableBox}[Bảng Tính Toán Chi Tiết Thuộc Tính Box Model Theo em]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{2.4cm}|>{\bfseries\color{codeNumber}}K{2.4cm}|>{\raggedright\arraybackslash}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thuộc Tính} & \thd{Giá Trị em} & \thd{Tính Theo font-size: 20px} & \thd{Kết Quả px Thực Tế} \\
\hline
width & 10em & $10 \times 20\text{px}$ & $\mathbf{200\text{px}}$ \\
\hline
height & 5em & $5 \times 20\text{px}$ & $\mathbf{100\text{px}}$ \\
\hline
padding & 1em & $1 \times 20\text{px}$ & $\mathbf{20\text{px}}$ (mỗi phía) \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{noteBox}[3 Lưu Ý Cốt Lõi Khi Dùng Box Model Với em]
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item Nếu \codeinline{font-size} được đặt bằng \codeinline{px} $\rightarrow$ \codeinline{em} dựa trên chính giá trị đó, \textbf{không liên quan đến phần tử cha}.
    \item Đây là điểm khác biệt rõ ràng với \codeinline{em} khi dùng cho \codeinline{font-size}, vốn dựa theo cha.
    \item Nếu bạn thay đổi \codeinline{font-size}, tất cả giá trị \codeinline{em} liên quan sẽ tự động co giãn theo $\rightarrow$ cực kỳ thuận tiện cho thiết kế Responsive.
\end{enumerate}
\end{noteBox}

\begin{prettyTableBox}[So Sánh Nhanh Điểm Tựa Tham Chiếu Của 1em Trong Trường Hợp 2]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\bfseries\color{primaryBlue}}K{4.5cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Trường Hợp Khai Báo} & \thd{1em Dựa Trên Cơ Sở Nào?} \\
\hline
font-size: 20px & $20\text{px}$ (của chính phần tử đó). \\
\hline
font-size: 2em & Nhân với \codeinline{font-size} của phần tử CHA. \\
\hline
Không khai báo font-size & Mặc định $16\text{px}$ (kế thừa từ thẻ gốc \codeinline{<html>}). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{Trường Hợp 3: Phần Tử Cha Có font-size, Phần Tử Con Không Khai Báo:}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Cách hoạt động}: Nếu phần tử con không có \codeinline{font-size}, nó sẽ tự động kế thừa giá trị \codeinline{font-size} từ phần tử cha gần nhất có khai báo.
    \item \textbf{Bản chất}: Đây là tính kế thừa tự động (\term{CSS Inheritance}) — vì \codeinline{font-size} là một thuộc tính có khả năng kế thừa theo phân cấp DOM.
\end{itemize}

\begin{codeWindow}[Ví Dụ 3: Phần Tử Con Kế Thừa Tự Động font-size Từ Phần Tử Cha]
\begin{lstlisting}[language=HTML5]
<div class="parent">
  <p class="child">Noi dung cua con</p>
</div>

<style>
  .parent {
    font-size: 24px; /* Cha co font-size */
  }
  .child {
    /* KHONG khai bao font-size */
  }
</style>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Kết Quả: .child Tự Động Nhận font-size Là 24px Từ .parent]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!20, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=12pt, right=12pt]
  {\fontsize{16pt}{20pt}\selectfont \textcolor{darkText}{Nội dung của con (thừa hưởng chính xác cỡ chữ $24\text{px}$ từ phần tử cha)}\par}
  \vspace{3pt}
  \textcolor{green!60!black}{\textbf{[Kết luận]:}} Phần tử \codeinline{.child} sẽ tự động nhận \codeinline{font-size: 24px} từ \codeinline{.parent} vì \codeinline{.child} không có \codeinline{font-size}, nó kế thừa từ \codeinline{.parent}.
\end{tcolorbox}
\end{browserWindow}

\begin{prettyTableBox}[Bảng Tổng Quát Cơ Chế Kế Thừa font-size Giữa Cha Và Con]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.4cm}|>{\bfseries\color{darkText}}K{3.4cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Phần Tử Cha} & \thd{Phần Tử Con} & \thd{Kết Quả Thực Tế Ở Phần Tử Con} \\
\hline
font-size: 24px & (không có) & Con = $\mathbf{24\text{px}}$ \\
\hline
font-size: 20px & (không có) & Con = $\mathbf{20\text{px}}$ \\
\hline
(không có) & (không có) & Con = $\mathbf{16\text{px}}$ (lấy mặc định từ thẻ gốc \codeinline{<html>}) \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{tipBox}[Ứng Dụng Thực Tế Của Tính Kế Thừa font-size]
Thuộc tính \codeinline{font-size} luôn kế thừa theo mặc định nếu không khai báo cụ thể. Đây là cách dễ kiểm soát toàn bộ hệ thống chữ trong một layout: Chỉ cần đặt \codeinline{font-size} ở phần tử bao bọc lớn (ví dụ: \codeinline{body}, \codeinline{.container}, \codeinline{section}), và để tất cả các phần tử con tự động thừa kế.
\end{tipBox}

\paragraph{Trường Hợp 4: Phần Tử Cha Có font-size, Phần Tử Con Dùng Đơn Vị em:}
Nếu phần tử con sử dụng \codeinline{em} cho cả \codeinline{font-size} lẫn các thuộc tính kích thước khác (\codeinline{width}, \codeinline{padding}\dots):
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \codeinline{font-size: x em} $\rightarrow$ dựa vào \codeinline{font-size} của \textbf{phần tử cha}.
    \item Các thuộc tính khác (\codeinline{width}, \codeinline{height}, \codeinline{padding}, \codeinline{margin}) dùng \codeinline{em} $\rightarrow$ dựa trên \codeinline{font-size} của \textbf{chính phần tử con đó} (giá trị đã được tính toán xong từ cha).
\end{enumerate}

\begin{codeWindow}[Ví Dụ 4: Cha Có font-size, Con Dùng em Cho Cả Chữ Lẫn Box Model]
\begin{lstlisting}[language=HTML5]
<div class="parent">
  <div class="child">Noi dung</div>
</div>
...
<style>
  ...
  .child {
    ...
    padding: 1em;      /* 1 x 30px = 30px (vi font-size cua .child la 30px) */
    width: 10em;       /* 10 x 30px = 300px */
...
\begin{prettyTableBox}[Bảng Phân Tích Tính Toán Chi Tiết Thuộc Tính Trong Ví Dụ 4]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{3.8cm}|>{\raggedright\arraybackslash}K{4.2cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thuộc Tính} & \thd{Điểm Tựa Dựa Trên} & \thd{Tính Ra Độ Dài Thực Tế (px)} \\
\hline
font-size: 1.5em & $1.5 \times 20\text{px}$ (của \codeinline{.parent}) & $\mathbf{= 30\text{px}}$ \\
\hline
padding: 1em & $1 \times 30\text{px}$ (của chính nó) & $\mathbf{= 30\text{px}}$ \\
\hline
width: 10em & $10 \times 30\text{px}$ (của chính nó) & $\mathbf{= 300\text{px}}$ \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{prettyTableBox}[Bảng Tổng Kết Quy Tắc em Khi Cả Chữ Và Box Model Cùng Dùng em]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{4.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thuộc Tính Dùng em} & \thd{Dựa Theo font-size Của Ai?} \\
\hline
font-size: em & Phần tử cha gần nhất. \\
\hline
padding, margin, width, height & Phần tử chính nó (sau khi đã tính toán xong \codeinline{font-size}). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{warnBox}[Lưu Ý Về Tính Kế Thừa Nhân Dồn]
Đơn vị \codeinline{em} có tính kế thừa theo cấp, nên nếu lồng nhiều cấp \codeinline{font-size: 1.2em}, kích thước sẽ bị phóng đại dần. Để tránh sự phức tạp không mong muốn, bạn có thể chuyển sang dùng đơn vị \codeinline{rem} để luôn luôn bám vào thẻ gốc \codeinline{<html>}.
\end{warnBox}

\paragraph{Trường Hợp 5: Cả Cha Và Con Đều Đặt font-size Bằng px, Thuộc Tính Khác Dùng em:}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Nguyên tắc hoạt động}: Các thuộc tính như \codeinline{width}, \codeinline{height}, \codeinline{padding}, \codeinline{margin} khi dùng \codeinline{em} sẽ \textbf{luôn luôn dựa trên font-size của chính phần tử đó} (hoàn toàn không quan tâm đến \codeinline{font-size} của phần tử cha).
    \item \textbf{Điểm then chốt}: Không quan trọng \codeinline{font-size} đó được khai báo bằng \codeinline{px}, \codeinline{em} hay \codeinline{rem}\dots miễn là đã được xác định cụ thể trên phần tử đó.
\end{itemize}

\begin{codeWindow}[Ví Dụ 5: Cả Cha Và Con Đều Đặt font-size px, Box Model Dùng em]
\begin{lstlisting}[language=HTML5]
<div class="parent">
  <div class="child">Noi dung con</div>
</div>
...
<style>
  ...
  .child {
    font-size: 30px;       /* font-size rieng cua con */
    width: 10em;           /* = 10 x 30px = 300px */
    padding: 1em;          /* = 1 x 30px = 30px */
    margin: 0.5em;         /* = 0.5 x 30px = 15px */
...
\begin{prettyTableBox}[Bảng Phân Tích Tính Toán Cho Phần Tử .child Trong Ví Dụ 5]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{3.4cm}|>{\raggedright\arraybackslash}K{4.4cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thuộc Tính .child} & \thd{Đơn Vị em Tính Theo Gì?} & \thd{Tính Ra Độ Dài Thực Tế (px)} \\
\hline
font-size: 30px & Do bạn trực tiếp thiết lập & $\mathbf{30\text{px}}$ \\
\hline
width: 10em & $10 \times \text{font-size}$ của chính nó & $10 \times 30\text{px} = \mathbf{300\text{px}}$ \\
\hline
padding: 1em & $1 \times \text{font-size}$ của chính nó & $1 \times 30\text{px} = \mathbf{30\text{px}}$ \\
\hline
margin: 0.5em & $0.5 \times \text{font-size}$ của chính nó & $0.5 \times 30\text{px} = \mathbf{15\text{px}}$ \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{prettyTableBox}[Bảng Tổng Kết Nguyên Tắc Phân Định Điểm Tham Chiếu]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{4.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thuộc Tính Dùng em} & \thd{Dựa Vào font-size Của Ai?} \\
\hline
font-size & Phần tử cha gần nhất. \\
\hline
width, height, padding, margin & Phần tử chính nó (sau khi đã tính xong \codeinline{font-size}). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{noteBox}[Lưu Ý Thiết Lập Cơ Sở Đo Lường]
Khi bạn gán \codeinline{font-size} cụ thể (dù bằng \codeinline{px}, \codeinline{em} hay \codeinline{rem}) $\rightarrow$ các thuộc tính khác dùng \codeinline{em} sẽ lấy đúng giá trị đó làm cơ sở. Để việc tính toán luôn chuẩn xác, hãy luôn đặt rõ ràng \codeinline{font-size} cho phần tử nếu bạn dự định dùng \codeinline{em} để thiết lập kích thước khác cho nó.
\end{noteBox}

\paragraph{Trường Hợp 6: Thẻ Cha Có font-size Tương Đối (\%, em) Và Cơ Chế Kế Thừa:}
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{font-size tương đối là gì?} Là khi bạn dùng các đơn vị tương đối như \codeinline{em}, \codeinline{\%}, \codeinline{rem} thay vì \codeinline{px}. Ví dụ: \codeinline{.parent \{ font-size: 120\%; \}} (tương đối theo phần tử cha của nó).
    \item \textbf{Khi thẻ cha dùng đơn vị tương đối nhưng không có phần tử cấp trên đặt font-size}: Trình duyệt sẽ lấy kích thước mặc định của thẻ gốc \codeinline{<html>} làm điểm tựa (thường là $16\text{px}$). Tức là:
    \[
    \text{font-size: 120\%} \quad \rightarrow \quad 120\% \times 16\text{px} = \mathbf{19.2\text{px}}
    \]
    \item \textbf{Thẻ con kế thừa như thế nào?} Thẻ con dùng đơn vị \codeinline{\%} hoặc \codeinline{em} sẽ kế thừa dựa trên giá trị \codeinline{font-size} đã tính toán xong của thẻ cha.
\end{enumerate}

\begin{codeWindow}[Ví Dụ 6: Cha Dùng Tỷ Lệ Phần Trăm, Con Dùng Đơn Vị em]
\begin{lstlisting}[language=HTML5]
<div class="parent">
  <p class="child">Noi dung</p>
</div>

<style>
  .parent {
    font-size: 125%;     /* = 1.25 x 16px = 20px */
  }
  .child {
    font-size: 1.5em;    /* = 1.5 x 20px = 30px */
  }
...
\begin{prettyTableBox}[Bảng Tổng Kết Quy Đổi font-size Qua 3 Cấp Trong Ví Dụ 6]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{2.4cm}|>{\bfseries\color{codeNumber}}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Cấp Độ} & \thd{font-size Tính Được} & \thd{Ghi Chú \& Điểm Tựa} \\
\hline
html & $16\text{px}$ (mặc định) & Cơ sở gốc chuẩn của trình duyệt. \\
\hline
.parent & $125\% = \mathbf{20\text{px}}$ & So với giá trị mặc định của \codeinline{html} ($1.25 \times 16\text{px}$). \\
\hline
.child & $1.5\text{em} = \mathbf{30\text{px}}$ & So với giá trị đã tính của \codeinline{.parent} ($1.5 \times 20\text{px}$). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{Trường Hợp 7: Thứ Tự Ưu Tiên Kế Thừa font-size Khi Dùng em (3 Cấp Ưu Tiên):}
Khi một phần tử sử dụng đơn vị \codeinline{em}, trình duyệt sẽ duyệt qua quy tắc thứ tự ưu tiên 3 cấp sau đây để xác định kích thước:

\begin{prettyTableBox}[Bảng Thứ Tự Ưu Tiên Kế Thừa font-size Khi Dùng em]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\centering\arraybackslash\bfseries\color{primaryBlue}}K{1.8cm}|>{\bfseries\color{darkText}}K{3.6cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Ưu Tiên} & \thd{Mức Kế Thừa} & \thd{Giải Thích Cơ Chế} \\
\hline
\textbf{1} & Chính phần tử đó & Nếu phần tử có khai báo \codeinline{font-size}, thì các thuộc tính \codeinline{em} khác sẽ tính dựa trên chính phần tử đó. \\
\hline
\textbf{2} & Thẻ cha trực tiếp & Nếu phần tử không có \codeinline{font-size}, nó sẽ kế thừa từ thẻ cha gần nhất. \\
\hline
\textbf{3} & Gốc (thẻ \codeinline{<html>}) & Nếu cả phần tử và phần tử cha đều không có \codeinline{font-size}, trình duyệt sẽ dùng giá trị mặc định từ \codeinline{<html>} (thường là $16\text{px}$). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{codeWindow}[Ví Dụ 7: Minh Họa 3 Cấp Lồng Nhau Tăng Lũy Tiến Kích Thước]
\begin{lstlisting}[language=HTML5]
...
<style>
  ...
  .child       { font-size: 1.5em; }   /* = 1.5 x 24px = 36px */
</style>
\end{lstlisting}
\end{codeWindow}

\begin{conceptBox}[Tóm Tắt Bản Chất Cốt Lõi Về Đơn Vị em]
\codeinline{em} là đơn vị tương đối thông minh nhưng rất dễ gây lỗi kế thừa chồng lớp. Khi sử dụng \codeinline{font-size: 1.5em} trong nhiều cấp, bạn cần hiểu rõ nó sẽ cộng dồn theo cha; nếu cha không xác định thì sẽ kế thừa từ gốc (\codeinline{html}, thường là $16\text{px}$). Nếu bạn muốn lấy kích thước chuẩn theo gốc mà không bị phụ thuộc vào thẻ cha, giải pháp lý tưởng là sử dụng đơn vị \codeinline{rem} (\term{Root EM}), ví dụ: \codeinline{font-size: 1.2rem;} (luôn tính trực tiếp từ thẻ \codeinline{<html>}).
\end{conceptBox}

\paragraph{Trường Hợp 8: Mô Hình Thực Nghiệm 4 Cấp div Lồng Nhau (Ông -- Bố -- Con -- Cháu):}
Dưới đây là một ví dụ thực tế với 4 cấp \codeinline{div} lồng nhau: Ông $\rightarrow$ Bố $\rightarrow$ Con $\rightarrow$ Cháu, tất cả đều dùng \codeinline{font-size: 1.5em} để thấy rõ hiệu ứng kích thước bị phóng đại lũy tiến:

\begin{codeWindow}[Ví Dụ 8: 4 Cấp div Lồng Nhau Với font-size Dùng em]
\begin{lstlisting}[language=HTML5]
<div class="grandparent">
  Ong (grandparent)
  <div class="parent">
...
\begin{prettyTableBox}[Bảng Tính Toán Phóng Đại Lũy Tiến Qua 4 Cấp Lồng Nhau]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{3.5cm}|>{\raggedright\arraybackslash}K{4.5cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Phần Tử} & \thd{Công Thức Tính Toán} & \thd{font-size Thực Tế (px)} \\
\hline
.grandparent & Cố định trực tiếp & $\mathbf{16\text{px}}$ \\
\hline
.parent & $1.5\text{em} \times 16\text{px}$ & $\mathbf{24\text{px}}$ \\
\hline
.child & $1.5\text{em} \times 24\text{px}$ & $\mathbf{36\text{px}}$ \\
\hline
.grandchild & $1.5\text{em} \times 36\text{px}$ & $\mathbf{54\text{px}}$ (gấp hơn 3.3 lần kích thước ban đầu!) \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{tipBox}[Cách Khắc Phục Nếu Không Muốn Chữ Bị Phình To]
Để kiểm soát kích thước cho các phần tử lồng sâu mà không bị phóng đại theo cha, bạn có thể thiết lập kích thước cố định bằng \codeinline{px} hoặc dùng \codeinline{rem}:
\begin{lstlisting}[language=CSS3]
.grandchild {
  font-size: 1.5rem; /* 1.5 * 16px = 24px -- hoan toan khong bi ke thua tu cha! */
}
\end{lstlisting}
\end{tipBox}

\paragraph{Trường Hợp 9: Hiểm Họa Khi Dùng Cùng Một Class .box Với font-size: 1.5em Cho Nhiều Cấp:}
Nếu các phần tử con -- cháu -- chắt đều sử dụng chung một class tái sử dụng (ví dụ \codeinline{.box}), thì khi gán \codeinline{font-size} bằng đơn vị \codeinline{em}, hiện tượng phóng đại kế thừa vẫn xảy ra dữ dội vì \codeinline{em} phụ thuộc vào \codeinline{font-size} của phần tử cha trực tiếp:

\begin{codeWindow}[Ví Dụ 9: Dùng Chung Một Class .box Và font-size: 1.5em Cho Các Cấp Lồng Nhau]
\begin{lstlisting}[language=HTML5]
<div class="box">
  Ong (1)
  <div class="box">
...
\begin{prettyTableBox}[Bảng Phân Tích Phóng Đại Cấp Số Nhân Khi Dùng Cùng Class .box]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{2.8cm}|>{\raggedright\arraybackslash}K{4.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Phân Cấp} & \thd{Điểm Tựa Kế Thừa} & \thd{font-size Tính Ra (px)} \\
\hline
.box cấp 1 & Không có cha nào $\Rightarrow$ Kế thừa từ \codeinline{html} ($1\text{em} = 16\text{px}$) & $1.5 \times 16\text{px} = \mathbf{24\text{px}}$ \\
\hline
.box cấp 2 & Kế thừa từ cấp 1 ($24\text{px}$) & $1.5\text{em} \times 24\text{px} = \mathbf{36\text{px}}$ \\
\hline
.box cấp 3 & Kế thừa từ cấp 2 ($36\text{px}$) & $1.5\text{em} \times 36\text{px} = \mathbf{54\text{px}}$ \\
\hline
.box cấp 4 & Kế thừa từ cấp 3 ($54\text{px}$) & $1.5\text{em} \times 54\text{px} = \mathbf{81\text{px}}$ (chữ khổng lồ!) \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{warnBox}[Lưu Ý Sống Còn \& 2 Cách Phòng Tránh Khi Tái Sử Dụng Class]
Khi dùng chung một class như \codeinline{.box} cho nhiều cấp lồng nhau với đơn vị \codeinline{em}, hãy luôn ghi nhớ rằng \codeinline{em} kế thừa từ cha trực tiếp, do đó rất dễ gây ra hiện tượng kích thước chữ bị phóng đại mất kiểm soát.

\textbf{Hai giải pháp kiến trúc được khuyến nghị}:
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Dùng \codeinline{rem} thay cho \codeinline{em}}: Giúp kích thước luôn bất biến độc lập với cấu trúc lồng nhau:
\begin{lstlisting}[language=CSS3]
.box {
  font-size: 1.5rem; /* Luon luon co dinh bang 1.5 * 16px = 24px o moi cap! */
}
\end{lstlisting}
    \item \textbf{Chỉ định rõ cỡ chữ cho từng cấp bậc}: Tái sử dụng class \codeinline{.box} cho giao diện chung, nhưng bổ sung modifier class để kiểm soát cỡ chữ:
\begin{lstlisting}[language=CSS3]
.box.level-1 { font-size: 16px; }
.box.level-2 { font-size: 24px; }
.box.level-3 { font-size: 20px; }
\end{lstlisting}
\end{enumerate}
\end{warnBox}

% ------------------------------------------------------------------------------
% MỤC ĐỌC THÊM & KIẾN THỨC MỞ RỘNG CHUYÊN SÂU (UNDER THE HOOD, A11Y & INTERVIEW)
% ------------------------------------------------------------------------------
\subsubsection{Đọc Thêm \& Mở Rộng: Góc Chuyên Sâu Về Đơn Vị em (Under The Hood, a11y \& Phỏng Vấn)}

Để hoàn thiện kiến thức của một kỹ sư Frontend chuyên nghiệp và sẵn sàng cho các buổi phỏng vấn kỹ thuật cấp cao, mục này tổng hợp các nguyên lý vận hành tầng sâu, các bẫy mã nguồn tinh vi và tiêu chuẩn tiếp cận chuẩn quốc tế liên quan đến đơn vị \codeinline{em}.

\paragraph{1. Góc Phỏng Vấn Tuyển Dụng: Bẫy Kinh Điển line-height: 1.5 vs line-height: 1.5em:}
Một trong những câu hỏi phỏng vấn phổ biến nhất nhưng lại khiến rất nhiều lập trình viên mất điểm chính là sự khác biệt giữa việc đặt \codeinline{line-height} bằng số không đơn vị (\term{unitless}) và đặt bằng đơn vị \codeinline{em}.

\begin{conceptBox}[Bản Chất Toán Học Của Bẫy line-height]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Khi dùng \codeinline{line-height: 1.5em}}: Kích thước chiều cao dòng được trình duyệt \textbf{tính toán thành pixel cố định ngay tại phần tử cha}, sau đó truyền giá trị pixel cố định này xuống cho tất cả các phần tử con kế thừa.
    \item \textbf{Khi dùng \codeinline{line-height: 1.5} (không đơn vị)}: Phần tử cha \textbf{chỉ truyền tỷ lệ thuần túy $1.5$} xuống cho các phần tử con. Mỗi phần tử con sẽ tự nhân tỷ lệ $1.5$ này với chính cỡ chữ riêng của nó.
\end{itemize}
\end{conceptBox}

\begin{codeWindow}[Mã Nguồn Minh Họa Lỗi Đè Chữ Nghiêm Trọng Với line-height: 1.5em]
\begin{lstlisting}[language=HTML5]
<!-- TRUONG HOP NGUY HIEM: Dung 1.5em -->
<div class="parent-bad">
  Cha (20px, line-height: 1.5em -> tinh ra 30px)
  <h1 class="child-bad">Tieu De Con Co Co Chu 40px</h1>
</div>

<style>
.parent-bad {
  font-size: 20px;
  line-height: 1.5em; /* Cha tinh: 1.5 x 20px = 30px co dinh truyen xuong! */
}
.child-bad {
  font-size: 40px;    /* Co chu lon 40px nhung line-height chi co 30px! */
}
</style>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng Thực Tế: Thảm Họa Chữ Đè Lên Nhau Khi Dùng line-height: 1.5em]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=red!35, arc=6pt, boxrule=0.8pt, top=10pt, bottom=10pt, left=12pt, right=12pt]
  {\fontsize{13pt}{16pt}\selectfont \textcolor{darkText}{Đoạn văn cha (\codeinline{font-size: 20px}, \codeinline{line-height} tính ra cố định: $30\text{px}$)}\par}
  \vspace{6pt}
  \begin{tcolorbox}[enhanced, colback=red!10, colframe=red!40, arc=4pt, boxrule=0.8pt, top=4pt, bottom=4pt, left=8pt, right=8pt]
    {\fontsize{24pt}{18pt}\selectfont \textcolor{red!80!black}{\textbf{DÒNG TIÊU ĐỀ THỨ NHẤT CÓ FONT 40PX}}\par}
    {\fontsize{24pt}{18pt}\selectfont \textcolor{red!80!black}{\textbf{DÒNG THỨ HAI BỊ ĐÈ LÊN DÒNG THỨ NHẤT DO LINE-HEIGHT QUÁ NHỎ (30PX)!}}\par}
  \end{tcolorbox}
  \vspace{3pt}
  \textcolor{red!80!black}{\scriptsize (\textbf{Cảnh báo}: Vì line-height chỉ có $30\text{px}$ trong khi cỡ chữ lên tới $40\text{px}$, các dòng chữ của tiêu đề con sẽ đè bẹp lên nhau!)}
\end{tcolorbox}
\end{browserWindow}

\begin{tipBox}[Quy Tắc Vàng Tuyệt Đối Cho line-height]
\textbf{Luôn luôn sử dụng số không đơn vị (unitless) cho thuộc tính \codeinline{line-height}}:
\begin{lstlisting}[language=CSS3]
/* Chuan muc ky thuat dung: */
body {
  line-height: 1.5; /* Khong co chu 'em' phia sau! */
}
\end{lstlisting}
Khi đó, tiêu đề con $40\text{px}$ sẽ nhận \codeinline{line-height} là $1.5 \times 40\text{px} = 60\text{px}$ $\rightarrow$ văn bản giãn cách hoàn hảo, không bao giờ bị đè chữ.
\end{tipBox}

\paragraph{2. Câu Hỏi Phỏng Vấn Senior: Khi Nào Bắt Buộc NÊN Dùng em Thay Vì rem?}
Trong kỷ nguyên CSS hiện đại, mặc dù \codeinline{rem} là đơn vị thống trị cho Typography và Layout toàn trang, nhưng \codeinline{em} vẫn là \textbf{vũ khí không thể thay thế} trong các kịch bản sau:

\begin{enumerate}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Tính đóng gói thành phần (\term{Component Encapsulation})}:
    Khi bạn xây dựng một thành phần UI dùng chung (Design System Component như Button, Badge, Chip, Avatar, Tooltip), thành phần này có thể được gắn vào Sidebar (chữ nhỏ $12\text{px}$), vào Body (chữ $16\text{px}$) hoặc vào Hero Banner (chữ lớn $28\text{px}$).
    \begin{itemize}[leftmargin=1.2em, itemsep=2pt]
        \item Nếu dùng \codeinline{rem}: Khoảng đệm \codeinline{padding} và góc bo \codeinline{border-radius} sẽ cố định toàn cục, nút bấm ở Hero banner sẽ trông rất mất cân đối (chữ khổng lồ nhưng viền và đệm lại nhỏ xíu).
        \item Nếu dùng \codeinline{em}: Nút bấm tự động co giãn toàn bộ khoảng đệm, khoảng cách icon và góc bo tỷ lệ thuận tuyệt đối theo cỡ chữ của ngữ cảnh chứa nó.
    \end{itemize}
    \item \textbf{Đồng bộ kích thước biểu tượng đi kèm văn bản (Inline Icons)}:
    Khi đặt kích thước cho icon SVG nằm cạnh chữ (\codeinline{width: 1em; height: 1em;}), icon sẽ luôn luôn có kích thước bằng đúng cỡ chữ hiện tại của thẻ cha, không cần viết thêm code điều chỉnh khi thay đổi phông chữ.
\end{enumerate}

\paragraph{3. Tiêu Chuẩn Tiếp Cận Người Dùng (Accessibility -- a11y \& WCAG 2.1):}
Tiêu chuẩn tiếp cận nội dung Web quốc tế (\term{WCAG 2.1}) tại tiêu chí \textbf{1.4.4 (Resize Text)} quy định: Văn bản trên trang web phải có khả năng phóng to lên ít nhất $\mathbf{200\%}$ mà không bị vỡ giao diện hoặc mất nội dung.

\begin{prettyTableBox}[So Sánh Khả Năng Đáp Ứng Chuẩn Trợ Năng (a11y) Giữa px Và em / rem]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.4cm}|>{\raggedright\arraybackslash}K{4.6cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Hành Động Của Người Dùng} & \thd{Hành Vi Khi Dùng px Cố Định} & \thd{Hành Vi Khi Dùng em / rem} \\
\hline
Thay đổi cỡ chữ mặc định trong Cài đặt trình duyệt (từ $16\text{px}$ lên $24\text{px}$ hoặc $32\text{px}$) & \textcolor{red!80!black}{\textbf{[Thất Bại]}} Trang web giữ nguyên cỡ chữ cứng, người thị lực kém không thể đọc được. & \textcolor{green!60!black}{\textbf{[Thành Công Tuyệt Đối]}} Toàn bộ chữ và thành phần dùng \codeinline{em}/\codeinline{rem} tự động phóng to đồng bộ theo thị lực người dùng. \\
\hline
Zoom trình duyệt (Phím tắt Ctrl + / -) & Phóng to toàn bộ pixel theo tỷ lệ Viewport. & Phóng to mượt mà, hài hòa với cấu trúc bố cục tự thích ứng. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{4. Cơ Chế Bên Trong Cỗ Máy Trình Duyệt (Browser Engine \& Under The Hood):}
Trong quá trình xử lý mã nguồn của trình duyệt (Blink trên Chrome, Gecko trên Firefox, WebKit trên Safari), đơn vị \codeinline{em} trải qua vòng đời tính toán nghiêm ngặt:

\begin{conceptBox}[Vòng Đời Chuyển Hóa Của Giá Trị em Trong Trình Duyệt]
\[
\text{Declared Value (2em)} \;\longrightarrow\; \text{Cascaded Value} \;\longrightarrow\; \text{Specified Value} \;\longrightarrow\; \mathbf{Computed Value (32px)}
\]
\vspace{-4pt}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Giai đoạn Computed Style (Tái tính toán kiểu dáng)}: Trình duyệt xây dựng cây \term{CSSOM}. Tại đây, mọi đơn vị tương đối (\codeinline{em}, \codeinline{\%}) đều được nhân với giá trị phông chữ của cha để quy đổi thành giá trị số thực tuyệt đối (\codeinline{px}) gọi là \textbf{Computed Value}.
    \item \textbf{Giai đoạn Layout (Reflow)}: Khi bước vào cây hiển thị (\term{Render Tree}) và tính toán tọa độ khung hình, cỗ máy dựng hình của trình duyệt \textbf{hoàn toàn chỉ làm việc với số pixel thực tế}, không còn bất kỳ đơn vị \codeinline{em} nào tồn tại trong quá trình vẽ màn hình.
\end{itemize}
\end{conceptBox}

\paragraph{5. Bản Đồ Phân Vai Đơn Vị Chuẩn Doanh Nghiệp (Enterprise Architecture Cheat Sheet):}
Để xây dựng một hệ thống giao diện web bền vững, dễ bảo trì và có khả năng mở rộng không giới hạn, các kỹ sư phần mềm chuyên nghiệp luôn tuân theo ma trận phân vai sau:

\begin{prettyTableBox}[Ma Trận Phân Vai Các Đơn Vị Đo Lường Trong Dự Án Chuyên Nghiệp]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.2cm}|>{\centering\arraybackslash\ttfamily\bfseries\color{codeNumber}}K{2.6cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Thành Phần Giao Diện} & \thd{Đơn Vị Tối Ưu} & \thd{Lý Do Kỹ Thuật \& Trải Nghiệm Người Dùng (UX)} \\
\hline
Cỡ chữ toàn trang (\codeinline{h1-h6}, \codeinline{p}, \codeinline{span}) & rem & Giữ cho toàn bộ trang web đồng bộ theo một nguồn chân lý gốc (\term{Single Source of Truth}) từ thẻ \codeinline{<html>}. \\
\hline
Bố cục vĩ mô (Grid columns, Flexbox layout, Margin lớn) & rem / \% / vw & Giúp khung bố cục ổn định, tránh bị xô lệch khi các thành phần con thay đổi cỡ chữ. \\
\hline
Micro-components (Button, Badge, Chip, Avatar, Tooltip) & em & Cho phép toàn bộ khoảng đệm (\codeinline{padding}), bo góc (\codeinline{border-radius}) tự co giãn hài hòa theo kích thước chữ của component. \\
\hline
Biểu tượng đi kèm chữ (Inline SVG Icons) & em & Icon tự động bằng đúng chiều cao chữ (\codeinline{width: 1em; height: 1em;}), luôn thẳng hàng tuyệt đối. \\
\hline
Khoảng cách dòng (\codeinline{line-height}) & Unitless (1.5) & Ngăn ngừa triệt để bẫy đè chữ khi phần tử con kế thừa từ cha. \\
\hline
Đường viền mỏng (\codeinline{border}) & px (1px, 2px) & Đảm bảo nét vẽ siêu mảnh, không bị mờ nhòe do làm tròn số thập phân trên màn hình độ phân giải cao. \\
\hline
\end{tabularx}
\end{prettyTableBox}

% ==============================================================================
% CHUYÊN ĐỀ SÂU VỀ ĐƠN VỊ REM (ROOT EM MASTERCLASS)
% ==============================================================================
\subsection{Chuyên Đề Đơn Vị rem (rem Masterclass Trong CSS)}
\label{sec:css_rem_masterclass}

\paragraph{1. Bản Chất Đơn Vị rem \& 3 Đặc Điểm Cốt Lõi:}
\codeinline{rem} là viết tắt của thuật ngữ \term{``Root EM''}. Đây là đơn vị đo lường tương đối được tính toán dựa trên kích thước phông chữ (\codeinline{font-size}) của \textbf{phần tử gốc duy nhất của trang web là thẻ \codeinline{<html>}}. Khác biệt hoàn toàn với \codeinline{em}, \codeinline{rem} không bị ảnh hưởng bởi thẻ cha gần nhất, mà luôn luôn tính từ thẻ gốc \codeinline{html}.

\begin{conceptBox}[3 Đặc Điểm Nền Tảng Của Đơn Vị rem]
\begin{enumerate}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Tương đối với phần tử gốc (\codeinline{<html>})}: Đây là điểm khác biệt lớn nhất so với đơn vị \codeinline{em}. Trong khi \codeinline{em} tương đối với \codeinline{font-size} của phần tử cha trực tiếp, \codeinline{rem} luôn tương đối với \codeinline{font-size} của thẻ \codeinline{<html>}. Điều này giúp triệt tiêu hoàn toàn ``hiệu ứng tầng'' (\term{Cascading / Compounding Effect}) mà \codeinline{em} có thể gây ra khi các phần tử lồng nhau.
    \item \textbf{Kiểm soát kích thước đồng bộ}: Khi bạn thay đổi \codeinline{font-size} của thẻ \codeinline{<html>}, tất cả các phần tử sử dụng đơn vị \codeinline{rem} trên trang sẽ tự động điều chỉnh kích thước theo tỷ lệ. Điều này cực kỳ hữu ích cho việc thiết kế giao diện có khả năng co giãn tỷ lệ thống nhất (\term{Scalable Design}) và dễ dàng thay đổi kích thước toàn trang.
    \item \textbf{Thân thiện với Responsive và Accessibility (Trợ năng)}:
    \begin{itemize}[leftmargin=1.2em, itemsep=2pt]
        \item \textbf{Responsive}: Bạn có thể sử dụng \codeinline{rem} để định nghĩa kích thước các thành phần (như \codeinline{padding}, \codeinline{margin}, \codeinline{width}, \codeinline{height}, \codeinline{font-size}) sao cho chúng tự động thay đổi theo kích thước màn hình bằng cách điều chỉnh \codeinline{font-size} của \codeinline{<html>} trong các Media Queries.
        \item \textbf{Accessibility}: Người dùng có thể dễ dàng điều chỉnh kích thước văn bản của trang web thông qua cài đặt trình duyệt (ví dụ: phóng to / thu nhỏ văn bản). Khi sử dụng \codeinline{rem}, văn bản và các thành phần khác sẽ tự động thích ứng với cài đặt của người dùng, cải thiện trải nghiệm vượt bậc cho những người có thị lực kém.
    \end{itemize}
\end{enumerate}
\end{conceptBox}

\paragraph{2. Cơ Chế Hoạt Động \& Công Thức Quy Đổi Toán Học:}
Đơn vị \codeinline{rem} hoạt động dựa trên nguyên tắc tương đối với kích thước phông chữ của phần tử gốc duy nhất của tài liệu là thẻ \codeinline{<html>}.

\begin{conceptBox}[Quy Trình 3 Bước Vận Hành Của Đơn Vị rem]
\begin{enumerate}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Bước 1 -- Trình duyệt đọc \codeinline{font-size} của thẻ \codeinline{<html>}}: Mặc định là $16\text{px}$ nếu không bị ghi đè trong CSS.
    \item \textbf{Bước 2 -- Xác lập quy chuẩn cơ sở}: Mỗi $1\text{rem} = \text{giá trị font-size của } \codeinline{<html>}$.
    \item \textbf{Bước 3 -- Áp dụng đồng bộ}: Mọi phần tử sử dụng đơn vị \codeinline{rem} trên toàn trang sẽ luôn tính toán dựa trên thẻ gốc \codeinline{<html>}, \textbf{hoàn toàn KHÔNG bị ảnh hưởng bởi bất kỳ thẻ cha trung gian nào}.
\end{enumerate}
\end{conceptBox}

\subparagraph{a. Điểm khởi đầu -- Kích thước phông chữ của thẻ gốc \codeinline{<html>}:}
Mọi tính toán với \codeinline{rem} đều bắt đầu từ việc xác định giá trị \codeinline{font-size} của thẻ \codeinline{<html>}:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Mặc định trình duyệt}: Nếu bạn không thiết lập \codeinline{font-size} cho thẻ \codeinline{<html>}, hầu hết mọi trình duyệt (Chrome, Firefox, Edge, Safari,...) sẽ mặc định dùng $16\text{px}$ làm kích thước cơ sở. Khi đó: $\mathbf{1\text{rem} = 16\text{px}}$.
    \item \textbf{Thiết lập thủ công}: Bạn có thể tự định nghĩa \codeinline{font-size} cho thẻ \codeinline{<html>} trong file CSS để kiểm soát kích thước cơ sở:
\end{itemize}

\begin{codeWindow}[Thiết Lập Mặc Định: font-size Cơ Sở Cho Thẻ html]
\begin{lstlisting}[language=CSS3]
html {
...
Hoặc để thuận tiện trong việc nhẩm toán, nhiều lập trình viên áp dụng tỷ lệ $62.5\%$:

\begin{codeWindow}[Kỹ Thuật 62.5\%: Chuyển Đổi Hệ Quy Chiếu Về Cơ Số 10 Dễ Nhẩm]
\begin{lstlisting}[language=CSS3]
html {
  font-size: 62.5%; /* Tuong duong 10px neu mac dinh trinh duyet la 16px */
  /* Cach tinh: 16px * 0.625 = 10px */
  /* Khi do: 1rem = 10px. Tinh toan tro nen sieu de dang (vi du: 1.5rem = 15px) */
}
\end{lstlisting}
\end{codeWindow}

Dù bạn thiết lập giá trị nào, \codeinline{font-size} của thẻ \codeinline{<html>} sẽ trở thành \textbf{``hệ số nhân''} cho tất cả các giá trị \codeinline{rem} xuất hiện trên toàn bộ trang web.

\subparagraph{b. Tính toán giá trị thực tế (Pixel) trên đa dạng thuộc tính:}
Khi bạn sử dụng \codeinline{rem} cho \textbf{bất kỳ thuộc tính CSS nào} (như \codeinline{font-size}, \codeinline{padding}, \codeinline{margin}, \codeinline{width}, \codeinline{height}, \codeinline{border-radius},...), trình duyệt sẽ thực hiện phép nhân đơn giản để chuyển đổi giá trị \codeinline{rem} thành giá trị pixel thực tế mà nó render lên màn hình.

\begin{conceptBox}[Công Thức Chuyển Đổi Đơn Vị rem Sang Pixel Thực Tế]
\[
\mathbf{\text{Giá Trị Thực Tế (px)} = \text{Giá Trị rem} \times \text{font-size của thẻ gốc \codeinline{<html>}}}
\]
Theo chuẩn mặc định:
\[
1\text{rem} = 16\text{px}, \quad 2\text{rem} = 32\text{px}, \quad 0.5\text{rem} = 8\text{px}
\]
\end{conceptBox}

Xét ví dụ minh họa tính toán trên nhiều thuộc tính thuộc Box Model và Typography:

\begin{codeWindow}[Khai Báo Nhiều Thuộc Tính Box Model \& Typography Bằng Đơn Vị rem]
\begin{lstlisting}[language=CSS3]
html {
  font-size: 16px;    /* Gia tri goc tham chieu */
}

h1 {
  font-size: 2.5rem;  /* Co chu tieu de */
  margin-top: 1.25rem;/* Khoang cach le tren */
}

p {
  font-size: 1rem;    /* Co chu doan van */
  padding: 0.5rem;    /* Khoang dem noi bo */
}

.box {
  width: 10rem;       /* Chieu rong */
  height: 5rem;       /* Chieu cao */
}
\end{lstlisting}
\end{codeWindow}

Cách trình duyệt tính toán giá trị pixel thực tế kết xuất ra màn hình:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Phần tử \codeinline{<h1>}}:
    \begin{itemize}[leftmargin=1.2em]
        \item \codeinline{font-size: 2.5rem} $\rightarrow 2.5 \times 16\text{px} = \mathbf{40\text{px}}$.
        \item \codeinline{margin-top: 1.25rem} $\rightarrow 1.25 \times 16\text{px} = \mathbf{20\text{px}}$.
    \end{itemize}
    \item \textbf{Phần tử \codeinline{<p>}}:
    \begin{itemize}[leftmargin=1.2em]
        \item \codeinline{font-size: 1rem} $\rightarrow 1 \times 16\text{px} = \mathbf{16\text{px}}$.
        \item \codeinline{padding: 0.5rem} $\rightarrow 0.5 \times 16\text{px} = \mathbf{8\text{px}}$.
    \end{itemize}
    \item \textbf{Phần tử \codeinline{.box}}:
    \begin{itemize}[leftmargin=1.2em]
        \item \codeinline{width: 10rem} $\rightarrow 10 \times 16\text{px} = \mathbf{160\text{px}}$.
        \item \codeinline{height: 5rem} $\rightarrow 5 \times 16\text{px} = \mathbf{80\text{px}}$.
    \end{itemize}
\end{itemize}


\begin{prettyTableBox}[Bảng Tra Cứu Quy Đổi Nhanh Từ Pixel Sang rem (Chuẩn Gốc 16px)]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash \bfseries\color{primaryBlue}}K{2.8cm}|>{\ttfamily\color{codeNumber}\bfseries}K{2.4cm}|>{\ttfamily\color{secondaryTeal}\bfseries}K{2.4cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Mục Đích Dùng} & \thd{Pixel (px)} & \thd{Giá Trị rem} & \thd{Giải Thích Quy Đổi} \\
\hline
Chú thích nhỏ (Small Text) & $12\text{px}$ & 0.75rem & $12 / 16 = 0.75\text{rem}$. \\
\hline
Chữ phụ (Secondary Text) & $14\text{px}$ & 0.875rem & $14 / 16 = 0.875\text{rem}$. \\
\hline
Văn bản chuẩn (Body Text) & $16\text{px}$ & 1rem & $16 / 16 = 1\text{rem}$ (Chuẩn mặc định trình duyệt). \\
\hline
Tiêu đề phụ (Subheading) & $20\text{px}$ & 1.25rem & $20 / 16 = 1.25\text{rem}$. \\
\hline
Tiêu đề H2 (Heading 2) & $24\text{px}$ & 1.5rem & $24 / 16 = 1.5\text{rem}$. \\
\hline
Tiêu đề H1 (Heading 1) & $32\text{px}$ & 2rem & $32 / 16 = 2\text{rem}$. \\
\hline
Tiêu đề Hero Banner & $48\text{px}$ & 3rem & $48 / 16 = 3\text{rem}$. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{3. Các Ví Dụ Trực Quan Minh Họa Đơn Vị rem:}

\begin{codeWindow}[Ví Dụ 3.1: Sử Dụng rem Cho Kích Thước Bố Cục Và Phông Chữ]
\begin{lstlisting}[language=HTML5]
...
.box {
  width: 20rem;       /* = 20 x 16px = 320px */
  height: 10rem;      /* = 10 x 16px = 160px */
...
\begin{browserWindow}[Mô Phỏng Trực Quan: Khối .box Thiết Lập Toàn Bộ Bằng rem]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!20, arc=6pt, boxrule=0.8pt, top=10pt, bottom=10pt, left=12pt, right=12pt]
  \begin{tcolorbox}[enhanced, colback=cyan!15, colframe=cyan!50, arc=4pt, boxrule=0.8pt, width=7.5cm, height=2.8cm, valign=center]
    \centering {\fontsize{16pt}{20pt}\selectfont \textbf{\color{primaryBlue}Box ($1.5\text{rem} = 24\text{px}$)}}\par
    \vspace{2pt}
    {\scriptsize\color{darkText} $20\text{rem} \times 10\text{rem}$ ($320\text{px} \times 160\text{px}$)}
  \end{tcolorbox}
  \vspace{3pt}
  \textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: Chiều rộng $320\text{px}$, chiều cao $160\text{px}$ và cỡ chữ $24\text{px}$ đều tính chuẩn mực theo gốc thẻ \codeinline{html}).}
\end{tcolorbox}
\end{browserWindow}

\begin{codeWindow}[Ví Dụ 3.2: Thay Đổi Cỡ Chữ Root Thẻ html Ảnh Hưởng Đồng Bộ Toàn Bộ Trang]
\begin{lstlisting}[language=HTML5]
<style>
html {
  font-size: 10px;    /* Thay doi co chu goc thanh 10px */
}
.box {
  font-size: 2rem;    /* = 2 x 10px = 20px */
}
</style>
\end{lstlisting}
\end{codeWindow}

\begin{noteBox}[Nguyên Lý Vận Hành Đồng Bộ Của rem]
Khi bạn thay đổi \codeinline{font-size} của thẻ gốc \codeinline{html}, toàn bộ các kích thước xây dựng dựa trên đơn vị \codeinline{rem} trên khắp trang web sẽ tự động co giãn theo tỷ lệ mới một cách đồng nhất, giúp việc scale giao diện trở nên vô cùng nhẹ nhàng.
\end{noteBox}

\begin{codeWindow}[Ví Dụ 3.3: Sự ``Ma Thuật'' Của rem Trong Thiết Kế Đáp Ứng (Responsive Design)]
\begin{lstlisting}[language=CSS3]
/* 1. Gia tri mac dinh danh cho man hinh Desktop lon (> 768px) */
html {
  font-size: 16px;    /* 1rem = 16px */
}
h1 {
...
/* 2. Khi man hinh Tablet co chieu rong toi da 768px */
@media (max-width: 768px) {
  html {
    font-size: 14px;  /* Giam font-size goc xuong 14px */
  }
}

/* 3. Khi man hinh Mobile co chieu rong toi da 480px */
@media (max-width: 480px) {
  html {
    font-size: 12px;  /* Giam font-size goc xuong 12px */
  }
}
\end{lstlisting}
\end{codeWindow}

\begin{prettyTableBox}[Ma Trận Co Giãn Kích Thước Tự Động Theo Thiết Bị Nhờ rem]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.2cm}|>{\centering\arraybackslash}K{2.4cm}|>{\raggedright\arraybackslash}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Ngưỡng Màn Hình} & \thd{font-size Thẻ html} & \thd{Tiêu Đề h1 (2.5rem)} & \thd{Đoạn Văn p (1rem)} \\
\hline
\textbf{Màn hình lớn ($> 768\text{px}$)} & $16\text{px}$ (Chuẩn) & $2.5 \times 16\text{px} = \mathbf{40\text{px}}$ & $1 \times 16\text{px} = \mathbf{16\text{px}}$ \\
\hline
\textbf{Màn hình vừa ($481\text{px} - 768\text{px}$)} & $14\text{px}$ & $2.5 \times 14\text{px} = \mathbf{35\text{px}}$ & $1 \times 14\text{px} = \mathbf{14\text{px}}$ \\
\hline
\textbf{Màn hình nhỏ ($\le 480\text{px}$)} & $12\text{px}$ & $2.5 \times 12\text{px} = \mathbf{30\text{px}}$ & $1 \times 12\text{px} = \mathbf{12\text{px}}$ \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{tipBox}[Lợi Ích Vượt Trội Của rem Trong Kiến Trúc Giao Diện]
Bạn không cần phải viết lại CSS cho từng phần tử (\codeinline{h1}, \codeinline{p}, \codeinline{.box},...) trong mỗi Media Query. Chỉ cần điều chỉnh duy nhất \codeinline{font-size} của \codeinline{<html>}, toàn bộ hệ thống giao diện từ phông chữ đến khoảng cách bố cục sẽ tự động co giãn đồng bộ theo tỷ lệ hoàn hảo. Điều này giúp mã nguồn tinh gọn, sạch sẽ, dễ bảo trì và giảm thiểu tối đa sai sót.
\end{tipBox}

\paragraph{4. Ma Trận So Sánh Toàn Diện: rem vs em:}
Dưới đây là bảng tổng hợp đối chiếu sự khác biệt bản chất giữa hai đơn vị tương đối quan trọng nhất:

\begin{prettyTableBox}[Bảng So Sánh Nhanh Đặc Điểm Vận Hành: rem vs em]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.4cm}|>{\raggedright\arraybackslash}K{4.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Đặc Điểm So Sánh} & \thd{Đơn Vị em} & \thd{Đơn Vị rem} \\
\hline
\textbf{Phụ thuộc vào} & Phần tử cha gần nhất (với \codeinline{font-size}) hoặc chính nó (với box model). & \textbf{Phần tử gốc duy nhất thẻ \codeinline{<html>}}. \\
\hline
\textbf{Dễ dự đoán} & \textcolor{red!80!black}{\textbf{[Không]}} Khó dự đoán nếu các cấp cha lồng nhau cũng dùng \codeinline{em}. & \textcolor{green!60!black}{\textbf{[Có]}} Rất dễ dự đoán vì luôn bám theo chuẩn thẻ \codeinline{<html>}. \\
\hline
\textbf{Trường hợp thường dùng} & Dùng trong \codeinline{padding}, khoảng cách (\term{spacing}) nội bộ của component. & Dùng trong thiết lập bố cục (\term{Layout}) và hệ thống cỡ chữ (\term{Typography}) toàn trang. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{prettyTableBox}[Bảng So Sánh Đối Chiếu Chi Tiết Giữa em Và rem]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash \bfseries\color{primaryBlue}}K{3.2cm}|Z|Z|}
\hline
\rowcolor{primaryBlue} \thd{Tiêu Chí So Sánh} & \thd{Đơn Vị em} & \thd{Đơn Vị rem} \\
\hline
\textbf{Điểm tham chiếu} & Phần tử cha gần nhất (với \codeinline{font-size}) hoặc chính nó (với padding/margin). & Thẻ gốc \codeinline{<html>} duy nhất trên toàn bộ trang. \\
\hline
\textbf{Độ phức tạp kiểm soát} & \textcolor{red!80!black}{\textbf{[Phức tạp]}} Dễ bị nhân dồn kích thước khi lồng nhau nhiều cấp. & \textcolor{green!60!black}{\textbf{[Rất dễ]}} Đồng nhất, dự đoán chính xác kích thước ở mọi nơi trong DOM. \\
\hline
\textbf{Khả năng mở rộng (Scaling)} & Cục bộ theo từng thành phần (Component-level). & Toàn cục trên quy mô cả trang web (Global-level). \\
\hline
\textbf{Trường hợp tối ưu nhất} & Tạo khoảng đệm (\codeinline{padding}), bo góc (\codeinline{border-radius}) cho nút bấm co giãn. & Định cỡ phông chữ (\codeinline{font-size}), khoảng cách bố cục (\codeinline{margin}, \codeinline{gap}) chuẩn Responsive. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\subparagraph{Thực nghiệm đối chứng: Hành vi của em vs rem trên cùng một cấu trúc DOM lồng nhau:}
Để hiểu sâu sắc sự khác biệt cốt lõi, hãy quan sát cùng một cây DOM lồng nhau $3$ cấp:

\begin{codeWindow}[Cấu Trúc HTML Cây DOM Lồng Nhau 3 Cấp: .parent > .child > .grandchild]
\begin{lstlisting}[language=HTML5]
<div class="parent">
  Parent Text
  <div class="child">
...
\begin{codeWindow}[Thử Nghiệm 1: Định Cỡ Bằng Đơn Vị em (Gây Hiệu Ứng Tầng Compounding)]
\begin{lstlisting}[language=CSS3]
html {
  font-size: 16px;
}
...
.child {
  font-size: 1.2em; /* 1.2 x font-size cua .parent (1.2 x 16px = 19.2px) */
}
.grandchild {
  font-size: 1.2em; /* 1.2 x font-size cua .child (1.2 x 19.2px = 23.04px) */
}
\end{lstlisting}
\end{codeWindow}

\begin{warnBox}[Hệ Quả Của Hiệu Ứng Tầng (Compounding Effect) Khi Dùng em]
Với \codeinline{em}, kích thước phông chữ của \codeinline{.grandchild} bị ảnh hưởng bởi \codeinline{.child}, và \codeinline{.child} lại bị ảnh hưởng bởi \codeinline{.parent}. Kích thước bị nhân dồn lên thành $23.04\text{px}$, trở nên rất khó kiểm soát khi cấu trúc DOM lồng nhau phức tạp.
\end{warnBox}

\begin{codeWindow}[Thử Nghiệm 2: Định Cỡ Bằng Đơn Vị rem (Hoàn Toàn Độc Lập Khỏi Thẻ Cha)]
\begin{lstlisting}[language=CSS3]
html {
  font-size: 16px;
}
...
.child {
  font-size: 1.2rem; /* 1.2 x 16px = 19.2px */
}
.grandchild {
...
\begin{conceptBox}[Điểm Tựa Độc Lập Tuyệt Đối Của rem]
Với \codeinline{rem}, kích thước của \codeinline{.child} ($19.2\text{px}$) và \codeinline{.grandchild} ($22.4\text{px}$) \textbf{hoàn toàn độc lập} với phần tử cha của chúng. Mọi phép tính đều neo chặt vào mỏ neo duy nhất là thẻ \codeinline{<html>}, giúp kích thước luôn dễ dự đoán và ổn định tuyệt đối trong mọi tình huống.
\end{conceptBox}

\paragraph{5. Những Lưu Ý Quan Trọng Khi Sử Dụng rem:}
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \codeinline{rem} \textbf{luôn luôn tính theo} \codeinline{html \{ font-size \}}.
    \item \textbf{Cực kỳ dễ kiểm soát trong thiết kế Responsive}: Chỉ cần điều chỉnh \codeinline{font-size} của thẻ \codeinline{<html>} tại các Breakpoint, toàn bộ trang web sẽ tự động co giãn tỷ lệ hoàn hảo.
    \item \textbf{Tránh hiện tượng lồng nhau gây tăng giảm kích thước không kiểm soát}: Thay vì đau đầu tính toán cấp số nhân của \codeinline{em}, sử dụng \codeinline{rem} giúp cấu trúc mã nguồn trong sáng, dễ bảo trì và dễ hiểu hơn rất nhiều.
\end{enumerate}

\begin{conceptBox}[Tóm Lược 3 Chìa Khóa Vàng Về Cơ Chế Hoạt Động Của rem]
\begin{enumerate}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Thiết lập \codeinline{font-size} cho \codeinline{<html>}}: Đây là nền tảng khởi nguồn duy nhất (\term{Single Source of Truth}).
    \item \textbf{Mọi giá trị \codeinline{rem}}: Trình duyệt lấy giá trị \codeinline{rem} đã khai báo nhân với \codeinline{font-size} của \codeinline{<html>} để kết xuất ra giá trị pixel thực tế.
    \item \textbf{Thay đổi \codeinline{font-size} của \codeinline{<html>}}: Tự động kéo theo sự thay đổi tỷ lệ đồng bộ của tất cả các phần tử dùng \codeinline{rem} trên khắp trang web -- Đây chính là chìa khóa vàng cho Responsive Design và Trợ năng (Accessibility).
\end{enumerate}
\end{conceptBox}

\paragraph{6. Ứng Dụng Thực Tế Trong Kiến Trúc Giao Diện CSS:}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Xây dựng hệ thống kích thước nhất quán (\term{Design Tokens})}: Kết hợp \codeinline{rem} với CSS Custom Properties (biến CSS):
\end{itemize}

\begin{codeWindow}[Hệ Thống Design Tokens Với rem \& CSS Custom Properties]
\begin{lstlisting}[language=CSS3]
:root {
  --space: 1rem; /* 16px mac dinh */
}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Thiết lập dễ dàng scale toàn bộ website}: Thay đổi kích thước toàn trang linh hoạt chỉ bằng một dòng CSS điều chỉnh phông chữ của thẻ \codeinline{<html>}.
\end{itemize}

% ------------------------------------------------------------------------------
% MỤC ĐỌC THÊM & MỞ RỘNG KIẾN THỨC VỀ REM
% ------------------------------------------------------------------------------
\subsubsection{Đọc Thêm \& Mở Rộng: Kỹ Thuật rem Chuyên Sâu \& Best Practices}

Theo đúng chuẩn mực biên soạn giáo trình nâng cao, mục này mở rộng các khía cạnh chuyên sâu giúp bạn áp dụng \codeinline{rem} hiệu quả và an toàn nhất trong môi trường dự án doanh nghiệp:

\paragraph{a. Kỹ Thuật 62.5\% (The 62.5\% Trick) -- Lợi Ích \& Cảnh Báo Sống Còn:}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Lý do ra đời}: Tính nhẩm với cơ số $16\text{px}$ thường sinh ra các số lẻ khó chịu (ví dụ: $14\text{px} = 0.875\text{rem}$, $18\text{px} = 1.125\text{rem}$, $22\text{px} = 1.375\text{rem}$).
    \item \textbf{Cách thực hiện}: Thiết lập \codeinline{html \{ font-size: 62.5\%; \}} ($62.5\% \times 16\text{px} = 10\text{px}$). Khi đó, hệ quy chiếu chuyển thành cơ số $10$: $1\text{rem} = 10\text{px}$, $1.4\text{rem} = 14\text{px}$, $2.4\text{rem} = 24\text{px}$.
    \item \textbf{Cảnh báo sống còn về Accessibility}: \textcolor{red!80!black}{\textbf{Tuyệt đối KHÔNG viết \codeinline{html \{ font-size: 10px; \}}!}} Nếu bạn viết $10\text{px}$ cứng, bạn sẽ vô hiệu hóa hoàn toàn tính năng thay đổi cỡ chữ trợ năng của trình duyệt. Dùng tỷ lệ $62.5\%$ đảm bảo vừa dễ nhẩm toán, vừa tôn trọng người khiếm thị khi họ tăng kích thước chữ mặc định.
\end{itemize}

\paragraph{b. Kỹ Thuật Hiện Đại: Phối Hợp rem Với clamp() Để Tạo Fluid Typography:}
Thay vì phải viết nhiều Media Queries để đổi cỡ chữ thẻ \codeinline{<html>}, CSS hiện đại cho phép kết hợp \codeinline{rem} với \codeinline{vw} thông qua hàm \codeinline{clamp()}:

\begin{codeWindow}[Fluid Typography Hiện Đại: Kết Hợp rem Và clamp()]
\begin{lstlisting}[language=CSS3]
/* Co chu tu dong co gian muot ma: toi thieu 1.5rem, ly tuong 2.5vw + 1rem, toi da 3rem */
h1 {
...
\paragraph{c. Câu Hỏi Phỏng Vấn Chuyên Sâu: rem Hoạt Động Bất Thường Khi Nào?}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Trong thẻ \codeinline{<iframe>}}: Một trang web nhúng trong \codeinline{<iframe>} sở hữu phần tử gốc \codeinline{<html>} độc lập của riêng nó. Do đó, mọi đơn vị \codeinline{rem} bên trong khung nhúng sẽ tính toán theo gốc của trang iframe, hoàn toàn cô lập và không chịu ảnh hưởng từ cỡ chữ của trang cha bên ngoài.
    \item \textbf{Khi ghi đè font-size ở \codeinline{body} thay vì \codeinline{html}}: Rất nhiều lập trình viên mới nhầm lẫn giữa thẻ \codeinline{<body>} và thẻ \codeinline{<html>}. \codeinline{rem} \textbf{chỉ lắng nghe thẻ gốc \codeinline{<html>}}, hoàn toàn bỏ qua mọi khai báo \codeinline{font-size} trên thẻ \codeinline{<body>}.
    \item \textbf{Bẫy tính toán rem trong Media Queries (\codeinline{@media})}: Theo đặc tả chuẩn của W3C (CSS Media Queries Level 4), các đơn vị tương đối như \codeinline{rem} hoặc \codeinline{em} khi nằm trong biểu thức Media Query (ví dụ: \codeinline{@media (min-width: 48rem)}) \textbf{luôn luôn lấy giá trị mặc định ban đầu của trình duyệt ($16\text{px}$)}, hoàn toàn KHÔNG bị ảnh hưởng nếu bạn ghi đè \codeinline{html \{ font-size: 62.5\%; \}} hay bất kỳ giá trị nào khác trong CSS! Ví dụ: biểu thức \codeinline{@media (min-width: 48rem)} sẽ luôn kích hoạt tại đúng $48 \times 16\text{px} = 768\text{px}$, chứ không phải $480\text{px}$.
\end{itemize}

\begin{tipBox}[Quy Tắc Ngón Tay Cái (Rule of Thumb) Cho Kỹ Sư Frontend]
Khi phân vân giữa các đơn vị đo lường trong dự án thực tế:
\begin{itemize}[leftmargin=1.2em, itemsep=2pt]
    \item Hãy ưu tiên dùng \codeinline{rem} cho \textbf{$90\%$ các trường hợp}: toàn bộ hệ thống cỡ chữ (\codeinline{font-size}), khoảng cách bố cục vĩ mô (\codeinline{gap}, \codeinline{margin}, \codeinline{padding} của section, card, container).
    \item Chỉ sử dụng \codeinline{em} cho các thành phần vi mô (\term{micro-components}) cần khả năng tự co giãn tỷ lệ đồng bộ theo chính phông chữ của nó (như nút bấm, icon đi kèm chữ, badge, tooltip).
    \item Sử dụng \codeinline{px} cho các đường viền siêu mảnh ($1\text{px}, 2\text{px}$) hoặc các bóng đổ (\codeinline{box-shadow}) cố định.
\end{itemize}
\end{tipBox}

% ==============================================================================
% CHUYÊN ĐỀ SÂU VỀ ĐƠN VỊ VIEWPORT (VIEWPORT UNITS MASTERCLASS)
% ==============================================================================
\subsection{Chuyên Đề Đơn Vị Viewport (vw, vh, vmin, vmax Masterclass)}
\label{sec:css_viewport_masterclass}

Đơn vị Viewport trong CSS lấy \textbf{kích thước khung nhìn của cửa sổ trình duyệt (\term{Viewport})} làm thước đo tham chiếu. Điều tuyệt vời nhất của đơn vị Viewport là: \textbf{Nó hoàn toàn độc lập với phần tử cha}, bất kể phần tử nằm sâu bao nhiêu cấp trong cây DOM.

\begin{prettyTableBox}[Bảng Tra Cứu 4 Đơn Vị Viewport Chuẩn Trong CSS]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{2.4cm}|>{\bfseries\color{darkText}}K{3.6cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Đơn Vị} & \thd{Ý Nghĩa Toán Học} & \thd{Ứng Dụng Thực Chiến Phổ Biến} \\
\hline
vw & $1\text{vw} = 1\%$ chiều rộng Viewport & Banner toàn màn hình, co giãn bố cục ngang không phụ thuộc cha. \\
\hline
vh & $1\text{vh} = 1\%$ chiều cao Viewport & Khối Hero Section phủ kín $100\%$ chiều cao màn hình (\codeinline{height: 100vh}). \\
\hline
vmin & Giá trị nhỏ hơn giữa \codeinline{vw} và \codeinline{vh} & Tạo hình vuông Responsive hoàn hảo trên cả màn hình dọc và ngang. \\
\hline
vmax & Giá trị lớn hơn giữa \codeinline{vw} và \codeinline{vh} & Thiết kế phông chữ cho màn hình cực lớn hoặc chữ nền trang trí. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{1. Chuyên Đề Sâu Về Đơn Vị vw (Viewport Width Masterclass):}

\begin{conceptBox}[1.1. Bản Chất Cốt Lõi: Đơn Vị vw Là Gì?]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Định nghĩa}: \codeinline{vw} viết tắt của \term{Viewport Width} (chiều rộng khung nhìn trình duyệt).
    \item \textbf{Quy đổi chuẩn}: $\mathbf{1\text{vw} = 1\%}$ chiều rộng hiện tại của cửa sổ trình duyệt.
    \item \textbf{Ví dụ minh họa}: Giả sử độ rộng cửa sổ trình duyệt người dùng là $1000\text{px}$, khi đó:
    \begin{itemize}[leftmargin=1.2em, itemsep=2pt]
        \item $1\text{vw} = 1\% \times 1000\text{px} = \mathbf{10\text{px}}$.
        \item $50\text{vw} = 50\% \times 1000\text{px} = \mathbf{500\text{px}}$.
        \item $100\text{vw} = 100\% \times 1000\text{px} = \mathbf{1000\text{px}}$ (chiếm trọn vẹn $100\%$ bề ngang màn hình).
    \end{itemize}
\end{itemize}
\end{conceptBox}

\paragraph{1.2. Khi Nào Nên Sử Dụng Đơn Vị vw?}
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Thiết kế giao diện Responsive}: Tự động co giãn mượt mà theo kích thước màn hình mà không cần viết quá nhiều Media Queries.
    \item \textbf{Thiết kế kích thước chữ (Typography), hình ảnh, layout linh hoạt}.
    \item \textbf{Dùng thay cho đơn vị phần trăm (\%)}: Khi bạn \textbf{không muốn phụ thuộc vào kích thước của phần tử cha}, mà muốn bám sát theo độ rộng màn hình thực tế của người dùng.
\end{enumerate}

\begin{codeWindow}[Ví Dụ Cơ Bản: Khối Luôn Chiếm 50\% Bề Ngang Màn Hình]
\begin{lstlisting}[language=HTML5]
<div class="box">Box 50vw</div>

<style>
.box {
  width: 50vw;               /* Luon chiem 50% chieu rong man hinh */
  height: 100px;
  background-color: tomato;  /* Mau do cam noi bat */
  color: white;
  text-align: center;
  line-height: 100px;
  font-size: 18px;
...
\begin{browserWindow}[Mô Phỏng Trực Quan: Khối width: 50vw Chiếm Nửa Màn Hình Trình Duyệt]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung màn hình Browser Viewport (rộng 9cm)
  \draw[fill=white, draw=gray!40, line width=0.9pt, rounded corners=4pt] (0,0) rectangle (9.0, 2.2);
  \node[anchor=north west, font=\tiny\bfseries\color{gray!70}] at (0.2, 2.05) {Khung nhìn trình duyệt (Viewport: 100vw)};
  
  % Khối .box (50vw = 4.5cm)
  \draw[fill=orange!80!red, draw=red!70!black, line width=1pt, rounded corners=3pt] (0.2, 0.2) rectangle (4.7, 1.6);
  \node[anchor=center, font=\small\bfseries\color{white}] at (2.45, 0.9) {Box: 50vw};
  
  % Khoảng trống 50vw còn lại
  \draw[dashed, draw=gray!30] (4.9, 0.2) rectangle (8.8, 1.6);
  \node[anchor=center, font=\tiny\color{gray!50}] at (6.85, 0.9) {Khoảng trống 50vw còn lại};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: Phần tử tự co lại khi màn hình thu nhỏ và tự dãn ra khi màn hình mở rộng).}
\end{browserWindow}

\paragraph{1.3. Ứng Dụng Độc Đáo: Dùng vw Cho font-size (Fluid Typography):}

Khi áp dụng \codeinline{vw} cho phông chữ, chữ sẽ tự động phóng to hoặc thu nhỏ mượt mà theo từng pixel thay đổi của trình duyệt:

\begin{codeWindow}[Mã Nguồn Cỡ Chữ Tự Co Giãn Theo Màn Hình]
\begin{lstlisting}[language=CSS3]
h1 {
  font-size: 5vw; /* Man hinh 1200px -> 60px; Man hinh 400px -> 20px */
  color: #0f4c81;
}
\end{lstlisting}
\end{codeWindow}

\paragraph{1.4. Bảng Lưu Ý Quan Trọng \& Kỹ Thuật Kết Hợp An Toàn:}

\begin{prettyTableBox}[Bảng 3 Lưu Ý Cốt Lõi Khi Sử Dụng Đơn Vị vw]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Lưu Ý Kỹ Thuật} & \thd{Giải Thích Chi Tiết \& Rủi Ro Cần Tránh} \\
\hline
Không phụ thuộc vào phần tử cha & \codeinline{vw} luôn tính theo kích thước trình duyệt, không dựa vào phần tử cha gần nhất. \\
\hline
Không nên dùng đơn độc cho mọi layout & Trên màn hình cực nhỏ (Mobile) phần tử có thể bị teo quá nhỏ, trên màn hình cực lớn (4K) lại bị phình to quá mức. \\
\hline
Cần kết hợp với \codeinline{min-width} / \codeinline{max-width} & Để thiết lập ngưỡng giới hạn an toàn, tránh vỡ bố cục trên mọi độ phân giải. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{codeWindow}[Mẫu Kết Hợp Tối Ưu Nhất Trong Thực Tế: vw + max-width + min-width]
\begin{lstlisting}[language=CSS3]
.box {
  width: 80vw;         /* Co gian chiem 80% man hinh */
  max-width: 600px;    /* Khong bao gio vuot qua 600px tren man hinh lon */
  min-width: 300px;    /* Khong bao gio bi teo nho hon 300px tren mobile */
}
\end{lstlisting}
\end{codeWindow}

\paragraph{1.5. Ví Dụ Chuyên Sâu 1: Khối Con 50vw Trong Khối Cha 300px (Hiện Tượng Tràn Khung):}

\begin{codeWindow}[Mã Nguồn Thể Hiện Hiện Tượng Tràn Khung Khi Dùng vw Trong Khung Hẹp]
\begin{lstlisting}[language=HTML5]
<!DOCTYPE html>
<html>
<head>
  <style>
    .container {
      width: 300px;           /* Phan tu cha co chieu rong chi 300px */
      padding: 20px;
      background-color: #e0e0e0;
    }
    .box {
      width: 50vw;            /* 50% cua Viewport (Vi du: 1000px -> 500px) */
      height: 100px;
...
\begin{browserWindow}[Mô Phỏng: Khối .box (50vw) Vượt Ra Ngoài Khung Cha .container (300px)]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung cha .container (300px ~ 4.2cm)
  \draw[fill=gray!20, draw=gray!50, line width=1pt, rounded corners=4pt] (0,0) rectangle (4.2, 2.6);
  \node[anchor=north west, font=\scriptsize\bfseries, color=darkText] at (0.15, 2.45) {.container \textmd{(width: $300\text{px}$)}};
  
  % Khối con .box (50vw ~ 7.0cm, tràn ra khỏi 4.2cm)
  \draw[fill=red!75!black, draw=red!95!black, line width=1pt, rounded corners=3pt] (0.2, 0.3) rectangle (7.2, 1.7);
  \node[anchor=center, font=\small\bfseries\color{white}] at (2.2, 1.0) {.box (width: 50vw)};
  \node[anchor=center, font=\tiny\bfseries\color{yellow!90!white}, align=center] at (5.7, 1.0) {\textbf{Tràn ra ngoài cha}\\\textbf{do lấy theo Viewport!}};

  % Thước đo phần tràn bên ngoài
  \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.8pt, color=red!80!black] (7.2, 1.8) -- (4.2, 1.8)
    node[midway, above=5pt, font=\tiny\bfseries, color=red!80!black] {Phần tràn ($200\text{px}$)};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{red!80!black}{\scriptsize ($\rightarrow$ \textbf{Lưu ý}: .box chiếm $50\%$ màn hình ($500\text{px}$ trên viewport $1000\text{px}$), nên sẽ tràn khỏi khung cha $300\text{px}$).}
\end{browserWindow}

\paragraph{1.6. Ví Dụ Chuyên Sâu 2: Cả Cha Và Con Đều Sử Dụng Đơn Vị vw:}

\begin{codeWindow}[Mã Nguồn Cả Cha (80vw) Và Con (50vw) Dùng Đơn Vị Viewport]
\begin{lstlisting}[language=HTML5]
<!DOCTYPE html>
<html>
<head>
  <style>
    .parent {
      width: 80vw;            /* Cha chiem 80% chieu rong man hinh */
      background-color: #e0e0e0;
      padding: 15px;
      margin: 0 auto;         /* Can giua man hinh */
    }
    .child {
      width: 50vw;            /* Con chiem 50% MAN HINH (khong phai 50% cua cha!) */
      height: 90px;
...
<body>
  <div class="parent">
    <div class="child">Child: 50vw (50% màn hình)</div>
  </div>
</body>
</html>
\end{lstlisting}
...
\begin{conceptBox}[Quy Tắc Đối Chiếu Cốt Lõi: vw vs \%]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Khi con dùng \codeinline{width: 50vw}}: Phần tử con chiếm $50\%$ độ rộng của \textbf{toàn bộ màn hình}.
    \item \textbf{Khi con dùng \codeinline{width: 50\%}}: Phần tử con chỉ chiếm $50\%$ độ rộng của \textbf{phần tử cha} ($50\% \times 80\text{vw} = \mathbf{40\text{vw}}$).
\end{itemize}
\end{conceptBox}

\paragraph{1.7. Chuyên Đề Bổ Trợ Thực Chiến: Thuộc Tính \codeinline{margin: 0 auto;} Căn Giữa Phần Tử Trong CSS:}

Trong ví dụ trên, thuộc tính \codeinline{margin: 0 auto;} được dùng để căn giữa phần tử theo chiều ngang:

\begin{conceptBox}[Cơ Chế Hoạt Động Của margin: 0 auto;]
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{\codeinline{0}}: Khoảng cách lề trên và lề dưới (\codeinline{margin-top} và \codeinline{margin-bottom}).
    \item \textbf{\codeinline{auto}}: Trình duyệt tự động tính toán và chia đều khoảng trống còn thừa cho lề trái (\codeinline{margin-left}) và lề phải (\codeinline{margin-right}), giúp phần tử nằm ngay chính giữa.
\end{itemize}
\end{conceptBox}

\begin{prettyTableBox}[Điều Kiện Bắt Buộc Để margin: 0 auto; Hoạt Động]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.6cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Tiêu Chuẩn Bắt Buộc} & \thd{Chi Tiết Kỹ Thuật} \\
\hline
Phải có chiều rộng rõ ràng & Phần tử bắt buộc phải có \codeinline{width} hoặc \codeinline{max-width} cụ thể (ví dụ: \codeinline{300px}, \codeinline{80vw}). \\
\hline
Kiểu hiển thị dạng khối & Phần tử phải là \codeinline{display: block} hoặc \codeinline{display: inline-block} (thẻ \codeinline{<div>}, \codeinline{<section>}, v.v.). \\
\hline
Trường hợp KHÔNG hoạt động & Khi \codeinline{width: auto} (vì phần tử khối mặc định chiếm $100\%$ chiều ngang, không còn khoảng trống thừa để căn giữa). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{browserWindow}[Mô Phỏng Trực Quan: margin: 0 auto; Tự Chia Đều Khoảng Trống Hai Bên]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung toàn trang
  \draw[fill=white, draw=gray!40, line width=0.9pt, rounded corners=4pt] (0,0) rectangle (9.2, 2.0);
  
  % Mũi tên margin-left: auto
  \draw[{Stealth[length=4pt]}-{Stealth[length=4pt]}, primaryBlue, line width=1pt] (0.2, 1.0) -- (2.5, 1.0) node[midway, above, font=\tiny\bfseries\color{primaryBlue}] {margin: auto};
  
  % Khối được căn giữa
  \draw[fill=orange!80!red, draw=red!80!black, line width=1pt, rounded corners=3pt] (2.6, 0.2) rectangle (6.6, 1.8);
  \node[anchor=center, font=\small\bfseries\color{white}, align=center] at (4.6, 1.0) {Khối Căn Giữa\\\scriptsize (width: 300px)};
  
  % Mũi tên margin-right: auto
  \draw[{Stealth[length=4pt]}-{Stealth[length=4pt]}, primaryBlue, line width=1pt] (6.7, 1.0) -- (9.0, 1.0) node[midway, above, font=\tiny\bfseries\color{primaryBlue}] {margin: auto};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: Phần tử nằm chính giữa hoàn hảo nhờ 2 lề trái và phải tự động co giãn bằng nhau).}
\end{browserWindow}

\paragraph{2. Chuyên Đề Sâu Về Đơn Vị vh (Viewport Height Masterclass):}

\begin{conceptBox}[2.1. Khái Niệm \& Bản Chất Toán Học Của Đơn Vị vh]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Định nghĩa}: \codeinline{vh} viết tắt của \term{Viewport Height} -- tức là chiều cao của khung nhìn trình duyệt.
    \item \textbf{Quy đổi chuẩn}: $\mathbf{1\text{vh} = 1\%}$ chiều cao của trình duyệt (Viewport).
    \item \textbf{Các mốc quy đổi tương tự}:
    \begin{itemize}[leftmargin=1.2em, itemsep=2pt]
        \item $\mathbf{100\text{vh}} = 100\%$ toàn bộ chiều cao màn hình.
        \item $\mathbf{50\text{vh}} =$ chính xác một nửa chiều cao màn hình trình duyệt.
    \end{itemize}
\end{itemize}
\end{conceptBox}

\begin{codeWindow}[Ví Dụ Cơ Bản: Khối Cao 50\% Màn Hình \& Canh Giữa Dọc Bằng line-height]
\begin{lstlisting}[language=HTML5]
<div class="box">Tôi cao 50% màn hình</div>

<style>
  .box {
    height: 50vh;                  /* Chieu cao bang 50% trinh duyet */
    background-color: lightseagreen;
    color: white;
    font-size: 20px;
    text-align: center;
    line-height: 50vh;             /* Canh giua van ban theo chieu doc */
  }
</style>
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng Trực Quan: Khối height: 50vh Chiếm Nửa Chiều Cao Trình Duyệt]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Khung màn hình Browser toàn bộ (rộng 9.2cm, cao 3.8cm)
  \draw[fill=white, draw=gray!30, line width=1pt, rounded corners=5pt] (0,0) rectangle (9.2, 3.8);
  
  % Thanh tiêu đề giả lập thanh trình duyệt ở trên cùng (cao 0.5cm)
  \fill[gray!12, rounded corners=4pt] (0.05, 3.25) rectangle (9.15, 3.75);
  \fill[red!60!black] (0.3, 3.5) circle (2.2pt);
  \fill[orange!80!black] (0.55, 3.5) circle (2.2pt);
  \fill[green!60!black] (0.8, 3.5) circle (2.2pt);
  \node[anchor=west, font=\scriptsize\bfseries\color{gray!70}] at (1.1, 3.5) {Cửa sổ trình duyệt (Tổng chiều cao Viewport = 100vh)};
  
  % Khối .box chiếm 50vh nửa trên (từ y = 1.75 đến y = 3.15, cao 1.4cm)
  \draw[fill=teal!75!black, draw=teal!95!black, line width=1pt, rounded corners=3pt] (0.25, 1.75) rectangle (8.95, 3.15);
  \node[anchor=center, font=\normalsize\bfseries\color{white}] at (4.6, 2.45) {Tôi cao 50\% màn hình};
  
  % Khoảng trống 50vh còn lại nửa dưới (từ y = 0.25 đến y = 1.65, cao 1.4cm)
  \draw[dashed, draw=gray!35, fill=gray!3, rounded corners=3pt] (0.25, 0.25) rectangle (8.95, 1.65);
  \node[anchor=center, font=\scriptsize\color{gray!55}] at (4.6, 0.95) {Khoảng trống 50vh còn lại phía dưới};

  % Thước đo 50vh bên trái
  \draw[decorate, decoration={brace, amplitude=5pt}, line width=0.9pt, color=teal!90!black] (-0.25, 1.75) -- (-0.25, 3.15)
    node[midway, left=6pt, font=\scriptsize\bfseries, color=teal!90!black] {$50\text{vh}$};

  % Thước đo 100vh bên phải
  \draw[decorate, decoration={brace, amplitude=5pt}, line width=0.9pt, color=primaryBlue] (9.45, 3.15) -- (9.45, 0.25)
    node[midway, right=6pt, font=\scriptsize\bfseries, color=primaryBlue, align=left] {$100\text{vh}$\\(Toàn màn)};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Kết quả: Khối \codeinline{.box} có chiều cao chuẩn $50\%$ màn hình và chữ được căn giữa dọc hoàn hảo bằng \codeinline{line-height: 50vh}).}
\end{browserWindow}

\paragraph{2.2. Bảng Tổng Hợp Ứng Dụng Thực Tế Của Đơn Vị vh:}

\begin{prettyTableBox}[Bảng Các Trường Hợp Thực Chiến Phổ Biến Của vh]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{4.8cm}|>{\ttfamily\color{accentOrange}\bfseries}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Mục Đích Thiết Kế} & \thd{Cách Khai Báo CSS} & \thd{Hiệu Quả Giao Diện} \\
\hline
Làm Header / Hero chiếm trọn màn hình & height: 100vh; & Tạo ấn tượng thị giác mạnh mẽ ngay khi người dùng vừa truy cập website. \\
\hline
Tạo màn hình chờ (Splash Screen) & min-height: 100vh; & Đảm bảo luôn lấp đầy màn hình dù nội dung bên trong ngắn hay dài. \\
\hline
Căn giữa dọc dễ dàng & display: flex; \newline align-items: center; & Kết hợp \codeinline{100vh} với Flexbox/Grid để căn giữa nội dung vào chính tâm màn hình. \\
\hline
Hiệu ứng cuộn từng phần (Fullpage Scroll) & section \{ height: 100vh; \} & Mỗi Section chiếm trọn vẹn đúng 1 khung nhìn màn hình khi cuộn trang. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{codeWindow}[Ứng Dụng Thực Chiến: Tạo Hero Banner Toàn Màn Hình Căn Giữa Hoàn Hảo]
\begin{lstlisting}[language=HTML5]
<style>
.hero-fullscreen {
  width: 100%;
  height: 100vh;                  /* Phu kin tron ven 100% chieu cao man hinh */
  ...
  align-items: center;
  color: white;
  box-sizing: border-box;         /* Tranh sinh thanh cuon doc khi co padding */
}
</style>

<div class="hero-fullscreen">
  <h1>Chào Mừng Đến Với Khóa Học CSS Masterclass</h1>
  <button class="cta-btn">Bắt Đầu Học Ngay</button>
</div>
\end{lstlisting}
\end{codeWindow}

\begin{warnBox}[Lưu Ý Quan Trọng Trên Thiết Bị Di Động \& Cạm Bẫy Cần Tránh]
\begin{enumerate}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item \textbf{Ảnh hưởng của thanh điều hướng (URL Bar)}: Trên thiết bị di động (Safari iOS, Chrome Android), thanh điều hướng có thể tự ẩn/hiện khi cuộn trang, làm giá trị \codeinline{100vh} bị che mất một phần dưới đáy.
    \item \textbf{Giải pháp thế hệ mới}: Có thể sử dụng \codeinline{100svh} (Small Viewport Height) hoặc \codeinline{100dvh} (Dynamic Viewport Height) để khắc phục triệt để độ lệch chiều cao trên mobile.
    \item \textbf{Cạm bẫy thanh cuộn dọc}: Khi dùng \codeinline{height: 100vh;}, nếu có thêm \codeinline{padding} hoặc \codeinline{border} bắt buộc phải có \codeinline{box-sizing: border-box;} để tránh phát sinh thanh cuộn ngoài ý muốn.
\end{enumerate}
\end{warnBox}

\paragraph{3. Chuyên Đề Sâu Về Đơn Vị vmin Và vmax:}

\begin{conceptBox}[3.1. Bản Chất Toán Học Của vmin Và vmax]
\begin{itemize}[leftmargin=1.5em, itemsep=3pt, parsep=0pt]
    \item $\mathbf{1\text{vmin}} = 1\%$ của \textbf{chiều nhỏ hơn} giữa chiều rộng (\codeinline{vw}) và chiều cao (\codeinline{vh}) của khung nhìn.
    \item $\mathbf{1\text{vmax}} = 1\%$ của \textbf{chiều lớn hơn} giữa chiều rộng (\codeinline{vw}) và chiều cao (\codeinline{vh}) của khung nhìn.
\end{itemize}
\end{conceptBox}

\begin{prettyTableBox}[Bảng So Sánh Giá Trị vmin Và vmax Theo Hướng Màn Hình]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\bfseries\color{primaryBlue}}K{3.8cm}|>{\centering\arraybackslash\bfseries}K{2.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Màn Hình Thiết Bị} & \thd{1vmin Bằng} & \thd{1vmax Bằng} \\
\hline
Màn hình dọc Mobile ($375\text{px} \times 812\text{px}$) & $1\% \text{ chiều rộng} = \mathbf{3.75\text{px}}$ & $1\% \text{ chiều cao} = \mathbf{8.12\text{px}}$ \\
\hline
Màn hình ngang Desktop ($1440\text{px} \times 900\text{px}$) & $1\% \text{ chiều cao} = \mathbf{9\text{px}}$ & $1\% \text{ chiều rộng} = \mathbf{14.4\text{px}}$ \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{codeWindow}[Ứng Dụng vmin Tạo Khối Vuông Responsive Hoàn Hảo 1:1]
\begin{lstlisting}[language=CSS3]
/* Khoi vuong khong bao gio bi tran khoi man hinh tren ca Desktop va Mobile: */
.responsive-square {
  width: 40vmin;   /* Lay 40% cua chieu nho nhat */
  height: 40vmin;
...
\begin{browserWindow}[Mô Phỏng Trực Quan: Khối Vuông 40vmin x 40vmin Co Giãn An Toàn]
\sffamily\small
\centering
\begin{tcolorbox}[enhanced, colback=purple!80!black, colframe=purple!95!black, width=80pt, height=80pt, arc=8pt, boxrule=1pt, top=14pt, bottom=14pt, left=6pt, right=6pt]
  \centering
  \color{white}\bfseries\small Khối Vuông\par
  \scriptsize 40vmin $\times$ 40vmin
\end{tcolorbox}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Luôn giữ đúng tỷ lệ hình vuông $1:1$ hoàn mỹ, không bao giờ vượt quá tầm nhìn của màn hình).}
\end{browserWindow}

\subsection{Hướng Dẫn Lựa Chọn Đơn Vị \& Best Practices Thực Chiến}
\label{sec:css_units_best_practices}

\begin{prettyTableBox}[Bảng Hướng Dẫn Lựa Chọn Đơn Vị CSS Theo Tình Huống]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
...
\hline
Cỡ chữ toàn trang (\codeinline{font-size}) & rem & Giúp điều chỉnh quy mô tập trung từ thẻ gốc \codeinline{<html>} và bảo toàn Accessibility. \\
\hline
Khoảng đệm nút bấm (\codeinline{padding}) & em & Tự động phóng to/thu nhỏ tỷ lệ thuận theo cỡ chữ của từng nút bấm. \\
\hline
Chia cột bố cục (\codeinline{width}) & \%, CSS Grid, Flexbox & Co giãn mượt mà theo kích thước khung chứa cha. \\
\hline
Hero Section, Banner toàn màn hình & vh, vw, dvh & Phủ kín $100\%$ kích thước màn hình mà không cần phụ thuộc vào cây DOM cha. \\
\hline
Chi tiết nhỏ cố định (Border, Icon, Shadow) & px & Kiểm soát chính xác tuyệt đối từng điểm ảnh (\term{Pixel-Perfect}). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\paragraph{Mẹo Thực Chiến \& Lưu Ý Cốt Lõi Khi Dùng Đơn Vị CSS:}

\begin{tipBox}[Mẹo Thực Chiến \& Lưu Ý Cốt Lõi Khi Dùng Đơn Vị CSS]
\begin{itemize}[leftmargin=1.5em, itemsep=4pt, parsep=0pt]
    \item $\rightarrow$ \textbf{\codeinline{rem} là tiêu chuẩn vàng cho Typography}: Luôn dùng \codeinline{rem} cho \codeinline{font-size} của toàn bộ website để tôn trọng cài đặt trợ năng phóng to của người dùng.
    \item $\rightarrow$ \textbf{Kết hợp linh hoạt \codeinline{em} và \codeinline{rem}}: Dùng \codeinline{rem} cho khoảng cách toàn cục (\codeinline{margin}, \codeinline{gap}) và dùng \codeinline{em} cho khoảng cách cục bộ bên trong Component (\codeinline{padding} của button/badge).
    \item $\rightarrow$ \textbf{Tránh dùng \codeinline{px} cho phông chữ chính}: Cỡ chữ \codeinline{px} sẽ vô hiệu hóa tính năng Zoom mặc định trên trình duyệt của người khiếm thị.
    \item $\rightarrow$ \textbf{\codeinline{100vh} trên mobile}: Hãy ưu tiên dùng \codeinline{100dvh} thay vì \codeinline{100vh} để tránh lỗi giao diện bị che khuất bởi thanh công cụ trình duyệt di động.
\end{itemize}
\end{tipBox}

\subsection{Mục "Đọc Thêm": Các Kỹ Thuật Đơn Vị Hiện Đại Trong CSS}
\label{sec:css_modern_units_deepdive}

\begin{conceptBox}[Đọc Thêm: Các Kỹ Thuật Đơn Vị Hiện Đại Trong CSS Modern]
Mục này giới thiệu các đơn vị Viewport thế hệ mới (\codeinline{dvh}, \codeinline{svh}, \codeinline{lvh}), Container Query Units, kỹ thuật Fluid Typography và kỹ thuật Root Reset $62.5\%$.
\end{conceptBox}
...
\begin{prettyTableBox}[Bảng Đối Chiếu 3 Đơn Vị Viewport Di Động Hiện Đại]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{2.4cm}|>{\bfseries\color{darkText}}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Đơn Vị} & \thd{Trạng Thái Thanh Địa Chỉ} & \thd{Đặc Điểm \& Ứng Dụng} \\
\hline
svh (Small) & Đang \textbf{hiển thị đầy đủ} & Chiều cao nhỏ nhất của màn hình, đảm bảo không bao giờ bị thanh URL che khuất. \\
\hline
lvh (Large) & Đang \textbf{thu gọn / ẩn hoàn toàn} & Chiều cao lớn nhất của màn hình khi người dùng cuộn trang xuống. \\
\hline
dvh (Dynamic) & \textbf{Co giãn tự động theo thời gian thực} & Tự động thích ứng mượt mà khi thanh địa chỉ ẩn/hiện, tối ưu cho Hero Section mobile. \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{browserWindow}[Mô Phỏng Trực Quan 3 Trạng Thái svh, dvh và lvh Trên Màn Hình Di Động]
\sffamily\small
\centering
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!25, width=300pt, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=8pt, right=8pt]
  \begin{tabularx}{\linewidth}{Y Y Y}
    \begin{tcolorbox}[colback=red!10, colframe=red!60!black, arc=4pt, boxrule=0.8pt, top=4pt, bottom=4pt, left=4pt, right=4pt]
      \centering
      \textcolor{red!90!black}{\bfseries\scriptsize 100svh}\par
      \tiny Thanh URL hiện diện\par
      \textcolor{darkText}{\tiny (Vừa khít màn hình nhỏ)}
    \end{tcolorbox} &
    \begin{tcolorbox}[colback=green!10, colframe=green!60!black, arc=4pt, boxrule=0.8pt, top=4pt, bottom=4pt, left=4pt, right=4pt]
      \centering
      \textcolor{green!90!black}{\bfseries\scriptsize 100dvh}\par
      \tiny Động theo cuộn trang\par
      \textcolor{darkText}{\tiny (Tự co giãn mượt mà)}
    \end{tcolorbox} &
    \begin{tcolorbox}[colback=blue!10, colframe=blue!60!black, arc=4pt, boxrule=0.8pt, top=4pt, bottom=4pt, left=4pt, right=4pt]
      \centering
      \textcolor{blue!90!black}{\bfseries\scriptsize 100lvh}\par
      \tiny Thanh URL ẩn đi\par
      \textcolor{darkText}{\tiny (Toàn bộ chiều cao lớn)}
    \end{tcolorbox}
  \end{tabularx}
\end{tcolorbox}
\end{browserWindow}

\paragraph{2. Đơn Vị Container Query Units (\codeinline{cqw}, \codeinline{cqh}, \codeinline{cqi}, \codeinline{cqb}):}

Trong thiết kế giao diện dạng thành phần (Component-Driven Design), một thẻ Card khi đặt vào Sidebar hẹp sẽ cần giao diện nhỏ gọn, nhưng khi đặt vào khu vực Nội dung chính (Main Content) rộng sẽ cần tự động nở lớn. Container Queries giải quyết điều này bằng cách định cỡ dựa trên \textbf{kích thước của chính khung chứa bao bọc trực tiếp}:

\begin{prettyTableBox}[Bảng Tra Cứu Đơn Vị Container Queries]
\rowcolors{2}{white}{primaryBlue!4}
\sffamily\small
\begin{tabularx}{\linewidth}{|>{\ttfamily\color{primaryBlue}\bfseries}K{2.4cm}|>{\bfseries\color{darkText}}K{3.8cm}|Z|}
\hline
\rowcolor{primaryBlue} \thd{Đơn Vị} & \thd{Ý Nghĩa Kỹ Thuật} & \thd{Ứng Dụng Thực Tế} \\
\hline
...
\hline
cqi & $1\%$ kích thước hướng dòng (\term{Inline}) & Tự động thích ứng theo hướng đọc (trái qua phải). \\
\hline
\end{tabularx}
\end{prettyTableBox}

\begin{codeWindow}[Mã Nguồn Khai Báo Container Query Units Với container-type]
\begin{lstlisting}[language=CSS3]
/* 1. Khai bao phan tu cha la mot Container doc lap: */
.card-wrapper {
  container-type: inline-size; /* Lang nghe chieu rong cua khung chua nay */
}

/* 2. Phan tu con ben trong tu dinh co theo chieu rong cua Card: */
.card-title {
  font-size: 5cqi; /* 5% do rong cua chinh khung card-wrapper */
  padding: 2cqi;
}
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng: Cùng Một Card Tự Thích Ứng Khi Nằm Trong Sidebar vs Main Content]
\sffamily\small
\centering
\begin{tikzpicture}[font=\sffamily]
  % Cột Sidebar hẹp (rộng 3.2cm)
  \draw[fill=gray!15, draw=gray!40, line width=0.8pt, rounded corners=3pt] (0,0) rectangle (3.2, 2.6);
  \node[anchor=north, font=\tiny\bfseries\color{gray!70}] at (1.6, 2.45) {Cột Sidebar Hẹp ($200\text{px}$)};
  \draw[fill=white, draw=primaryBlue!40, rounded corners=2pt] (0.2, 0.3) rectangle (3.0, 2.0);
  \node[anchor=center, font=\tiny\bfseries\color{primaryBlue}] at (1.6, 1.4) {Tiêu Đề Card ($10\text{px}$)};
  \node[anchor=center, font=\tiny\color{darkText}] at (1.6, 0.8) {font-size: 5cqi};

  % Cột Main Content rộng (rộng 5.8cm)
  \draw[fill=gray!15, draw=gray!40, line width=0.8pt, rounded corners=3pt] (3.8,0) rectangle (9.6, 2.6);
  \node[anchor=north, font=\tiny\bfseries\color{gray!70}] at (6.7, 2.45) {Cột Main Content Rộng ($600\text{px}$)};
  \draw[fill=white, draw=primaryBlue!40, rounded corners=2pt] (4.0, 0.3) rectangle (9.4, 2.0);
  \node[anchor=center, font=\small\bfseries\color{primaryBlue}] at (6.7, 1.4) {Tiêu Đề Card Tự Nở Lớn ($30\text{px}$)};
  \node[anchor=center, font=\scriptsize\color{darkText}] at (6.7, 0.8) {font-size: 5cqi};
\end{tikzpicture}
\par\vspace{4pt}
\textcolor{primaryBlue}{\scriptsize ($\rightarrow$ Cùng chung một dòng CSS \codeinline{font-size: 5cqi;}, Card tự động hiển thị nhỏ trong Sidebar và nở lớn trong Main Content).}
\end{browserWindow}
...
\begin{conceptBox}[Cấu Trúc 3 Tham Số Của Hàm clamp()]
\[
\text{font-size: } \mathbf{\text{clamp}}(\text{Cỡ Nhỏ Nhất (Min)}, \ \text{Giá Trị Tuyến Tính (vw + rem)}, \ \text{Cỡ Lớn Nhất (Max)});
\]
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt]
    \item \textbf{Min}: Ngưỡng chặn dưới, cỡ chữ không bao giờ nhỏ hơn giá trị này (ví dụ: $1\text{rem} = 16\text{px}$).
    \item \textbf{Val}: Giá trị co giãn động theo Viewport ($2.5\text{vw} + 0.5\text{rem}$).
    \item \textbf{Max}: Ngưỡng chặn trên, cỡ chữ không bao giờ vượt quá giá trị này (ví dụ: $2\text{rem} = 32\text{px}$).
\end{itemize}
\end{conceptBox}
...
\begin{conceptBox}[Bản Chất Toán Học Của Kỹ Thuật Root Reset 62.5\%]
Trình duyệt mặc định có phông chữ gốc là $16\text{px}$. Khi ta thiết lập:
\[
\text{html} \{ \text{font-size: } 62.5\%; \} \implies \text{Cỡ chữ gốc mới} = 16\text{px} \times 62.5\% = \mathbf{10\text{px}}
\]
Quy tắc nhẩm siêu tốc: \textbf{Bao nhiêu pixel thì chỉ cần chia cho 10 để ra số rem!}
\end{conceptBox}

\begin{codeWindow}[Mã Nguồn Áp Dụng Kỹ Thuật html \{ font-size: 62.5\%; \}]
\begin{lstlisting}[language=CSS3]
/* 1. Reset font-size goc ve 10px: */
html {
  font-size: 62.5%; /* 1rem bay gio bang dung 10px! */
}

/* 2. Tra lai co chu 16px tieu chuan cho body: */
body {
  font-size: 1.6rem; /* 1.6 * 10px = 16px */
}

/* 3. Nham tinh cac co chu khac cuc ky de dang: */
h1 { font-size: 3.2rem; } /* 3.2 * 10px = 32px */
h2 { font-size: 2.4rem; } /* 2.4 * 10px = 24px */
p  { font-size: 1.4rem; } /* 1.4 * 10px = 14px */
\end{lstlisting}
\end{codeWindow}

\begin{browserWindow}[Mô Phỏng Trực Quan Quy Đổi Siêu Tốc Với Kỹ Thuật 62.5\% Reset]
\sffamily\small
\begin{tcolorbox}[enhanced, colback=white, colframe=primaryBlue!25, arc=6pt, boxrule=0.8pt, top=8pt, bottom=8pt, left=12pt, right=12pt]
  {\fontsize{20pt}{24pt}\selectfont \textcolor{primaryBlue}{\textbf{Tiêu đề H1: 3.2rem ($32\text{px}$)}}\par}
  \vspace{3pt}
  {\fontsize{16pt}{20pt}\selectfont \textcolor{accentPurple}{\textbf{Tiêu đề H2: 2.4rem ($24\text{px}$)}}\par}
  \vspace{3pt}
  {\fontsize{11pt}{15pt}\selectfont \color{darkText} Đoạn văn p: 1.4rem ($14\text{px}$) -- Nhẩm tính chuyển đổi cực kỳ trực quan và chuẩn xác.\par}
\end{tcolorbox}
\end{browserWindow}
